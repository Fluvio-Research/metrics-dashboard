...
const getServiceStyles = (theme: GrafanaTheme2) => {
  return {
    ...
    divider: {
      base: css({
        color: theme.isDark ? 'rgba(148, 163, 184, 1)' : 'rgba(71, 85, 105, 1)',
        display: 'flex',
  ...
  return (
    <div className={styles.divider.base}>
      ...
      <div className={styles.divider.text}>
        {!config.disableLoginForm && <Trans i18nKey="login.divider.connecting-text">or</Trans>}
      </div>
      <div className={styles.divider.line} />
    </div>
  );
...
function getButtonStyleFor(service: LoginService, styles: ReturnType<typeof getServiceStyles>, theme: GrafanaTheme2) {
  return cx(
    ...
    css({
      ...
      ['&:hover']: {
        backgroundColor: theme.colors.emphasize(service.bgColor, 0.12),
      ...
      ['&:active']: {
        backgroundColor: theme.colors.emphasize(service.bgColor, -0.05),
      },
    })
  );
}
