import { css, cx } from '@emotion/css';
import { FC, useEffect, useState } from 'react';

import { useTheme2 } from '@grafana/ui';
import loginBackground from 'img/login_background.svg';
import mainSiwisLogo from 'img/MAIN_SIWIS.png';
import cascadeLogoSvg from 'img/cascade_logo.svg';

// ====================================
// 🎨 CENTRALIZED LOGO CONFIGURATION
// ====================================
// Change these paths to update ALL logos and icons across the entire application
const LOGO_CONFIG = {
  // Main logo used throughout the app (login, navigation, etc.)
  mainLogo: mainSiwisLogo,
  // Sidebar logo (SVG for better quality)
  sidebarLogo: cascadeLogoSvg,
  // Alternative logo (if needed for specific contexts)
  altLogo: mainSiwisLogo,
  
  // App branding & titles
  appTitle: 'SIWIS',
  loginTitle: '',
  
  // Icon files for favicon, apple-touch-icon, etc.
  iconFiles: {
    favicon: 'public/img/MAIN_SIWIS.png',
    appleTouchIcon: 'public/img/MAIN_SIWIS.png',
    faviconIco: 'public/favicon.ico',
  },
  
  // Organization/Company info
  company: {
    name: 'SIWIS',
    fullName: 'SIWIS Analytics Platform',
    shortName: 'SIWIS',
  }
};

export interface BrandComponentProps {
  className?: string;
  children?: JSX.Element | JSX.Element[];
}

export const LoginLogo: FC<BrandComponentProps & { logo?: string }> = ({ className, logo }) => {
  return <img className={className} src={`${logo ? logo : LOGO_CONFIG.mainLogo}`} alt={LOGO_CONFIG.appTitle} />;
};

const LoginBackground: FC<BrandComponentProps> = ({ className, children }) => {
  const theme = useTheme2();

  const background = css({
    position: 'relative',
    minHeight: '100vh',
    overflow: 'hidden',
    
    '&:before': {
      content: '""',
      position: 'fixed',
      left: 0,
      right: 0,
      bottom: 0,
      top: 0,
      background: `url(${loginBackground})`,
      backgroundPosition: 'center',
      backgroundSize: 'cover',
      backgroundRepeat: 'no-repeat',
      zIndex: 0,
      
      opacity: 0,
      transition: 'opacity 2s ease-in-out',

      [theme.breakpoints.up('md')]: {
        backgroundPosition: 'center',
        backgroundSize: 'cover',
      },
    },
    
    // Removed overlay for cleaner background appearance
    '&:after': {
      content: '""',
      position: 'fixed',
      left: 0,
      right: 0,
      bottom: 0,
      top: 0,
      background: 'transparent',
      zIndex: 1,
    },
  });

  return (
    <div className={cx(background, className)}>
      <div style={{ position: 'relative', zIndex: 2, minHeight: '100vh' }}>
        {children}
      </div>
    </div>
  );
};

const MenuLogo: FC<BrandComponentProps> = ({ className }) => {
  const [isAnimating, setIsAnimating] = useState(true);
  const theme = useTheme2();
  
  useEffect(() => {
    // Animation plays only once on mount
    const timer = setTimeout(() => {
      setIsAnimating(false);
    }, 1000); // Animation duration
    
    return () => clearTimeout(timer);
  }, []);
  
  const animationStyles = css({
    animation: isAnimating ? 'logoIntro 1s cubic-bezier(0.34, 1.56, 0.64, 1) forwards' : 'none',
    transformOrigin: 'center center',
    
    '@keyframes logoIntro': {
      '0%': {
        transform: 'scale(0.5)',
        opacity: 0,
      },
      '50%': {
        transform: 'scale(1.3)',
        opacity: 0.8,
        filter: theme.isDark 
          ? 'drop-shadow(0 0 12px rgba(77, 172, 255, 0.6))' 
          : 'drop-shadow(0 0 8px rgba(77, 172, 255, 0.4))',
      },
      '100%': {
        transform: 'scale(1)',
        opacity: 1,
        filter: 'drop-shadow(0 0 0 transparent)',
      },
    },
  });
  
  return (
    <img 
      className={cx(className, animationStyles)} 
      src={LOGO_CONFIG.sidebarLogo} 
      alt={LOGO_CONFIG.appTitle} 
    />
  );
};

const LoginBoxBackground = () => {
  const theme = useTheme2();
  return css({
    // Ultra-transparent glass morphism effect
    background: theme.isDark 
      ? 'rgba(20, 25, 40, 0.35)' 
      : 'rgba(255, 255, 255, 0.25)',
    backdropFilter: 'blur(40px) saturate(150%)',
    WebkitBackdropFilter: 'blur(40px) saturate(150%)',
    
    border: theme.isDark
      ? '1px solid rgba(100, 120, 180, 0.15)'
      : '1px solid rgba(200, 220, 255, 0.2)',
    
    boxShadow: theme.isDark
      ? '0 25px 70px 0 rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05)'
      : '0 25px 70px 0 rgba(50, 80, 150, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.4)',
  });
};

export class Branding {
  static LoginLogo = LoginLogo;
  static LoginBackground = LoginBackground;
  static MenuLogo = MenuLogo;
  static LoginBoxBackground = LoginBoxBackground;
  static AppTitle = LOGO_CONFIG.appTitle;
  static LoginTitle = LOGO_CONFIG.loginTitle;
  static HideEdition = false;
  static GetLoginSubTitle = (): null | string => {
    return null;
  };
}
