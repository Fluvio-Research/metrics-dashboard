...
export async function initLayer(
  ...
  // For LayerGroup, register child layers instead of the group itself
  // This prevents duplicate tooltip detection
  if (layer instanceof LayerGroup) {
    layer
      .getLayers()
      .getArray()
      .forEach((childLayer: BaseLayer) => {
        // Register all child layer types for tooltip detection
        layerStateMap.set(childLayer, state);
      });
  } else {
    // For non-group layers, register the layer itself
