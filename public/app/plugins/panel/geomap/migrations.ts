...
function migrateLayerTooltipFlag(layer: any) {
  if (!layer) {
    return;
  }
  const legacyTooltip = layer['layer-tooltip'];
  if (legacyTooltip !== undefined && layer.tooltip === undefined) {
    layer.tooltip = legacyTooltip;
  }
  delete layer['layer-tooltip'];
}

function migrateTooltipOptions(options?: Partial<Options>) {
  if (!options) {
    return;
  }

  migrateLayerTooltipFlag(options.basemap);
  options.layers?.forEach((layer) => migrateLayerTooltipFlag(layer));
}
  ...
  if (pluginVersion.startsWith('8.1') || pluginVersion.startsWith('8.2')) {
    ...
    if (layers?.length === 1) {
      ...
      if (layer?.type === 'markers' && layer.config) {
        ...
        const migratedOptions = { ...panel.options, layers: [{ ...layer, config }] };
        migrateTooltipOptions(migratedOptions);
        return migratedOptions;
      }
    }
  }
  migrateTooltipOptions(panel.options);
