  ...
  const rowCategory = useMemo(
    () =>
      new OptionsPaneCategoryDescriptor({ title: '', id: 'row-options' })
        ...
        .addItem(
          new OptionsPaneItemDescriptor({
            title: t('dashboard.rows-layout.row-options.row.collapsed-icon', 'Collapsed icon'),
            description: t('dashboard.rows-layout.row-options.row.collapsed-icon-desc', 'Image path shown when row is collapsed'),
            render: () => <CollapsedIconInput row={model} />,
          })
        )
        .addItem(
          new OptionsPaneItemDescriptor({
            title: t('dashboard.rows-layout.row-options.row.row-group', 'Row group'),
            description: t('dashboard.rows-layout.row-options.row.row-group-desc', 'Group name for side-by-side display when collapsed'),
            render: () => <RowGroupInput row={model} />,
          })
        )
        .addItem(
          new OptionsPaneItemDescriptor({
...
function CollapsedIconInput({ row }: { row: RowItem }) {
  const { collapsedIcon } = row.useState();

  return (
    <>
      <Input
        value={collapsedIcon || ''}
        placeholder="/public/img/icon.png"
        onChange={(e) => row.onChangeCollapsedIcon(e.currentTarget.value || undefined)}
      />
      {collapsedIcon && (
...
function RowGroupInput({ row }: { row: RowItem }) {
  const { rowGroup } = row.useState();

  return (
    <Input
      value={rowGroup || ''}
      placeholder="primary_station"
      onChange={(e) => row.onChangeRowGroup(e.currentTarget.value || undefined)}
    />
  );
}
