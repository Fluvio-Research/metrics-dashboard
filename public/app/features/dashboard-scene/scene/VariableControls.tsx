...
import { CollapsibleFilterBarScene } from './CollapsibleFilterBarScene';
...
export function VariableControls({ dashboard }: { dashboard: DashboardScene }) {
  ...
  // Check if dashboard has the "filter" tag
  const hasFilterTag = dashboardState.tags && dashboardState.tags.includes('filter');

  // Use collapsible filter bar for dashboards with "filter" tag
  if (hasFilterTag && variables.variables.length > 0) {
    return <CollapsibleFilterBarScene dashboard={dashboard} />;
  }

  // Default: traditional inline variables
  return (
    <>
