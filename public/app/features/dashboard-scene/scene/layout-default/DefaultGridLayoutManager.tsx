...
import { GroupedRowsManager } from './GroupedRowsManager';
...
function DefaultGridLayoutManagerRenderer({ model }: SceneComponentProps<DefaultGridLayoutManager>) {
  ...
  // Extract rows from children for grouped rows management
  const rows = children.filter((child): child is SceneGridRow => child instanceof SceneGridRow);
  ...
  return (
    <div className={cx(styles.container, isEditing && styles.containerEditing)}>
      {/* Grouped rows manager for side-by-side display */}
      <GroupedRowsManager rows={rows} />
