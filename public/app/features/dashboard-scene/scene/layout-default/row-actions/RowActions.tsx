...
export class RowActions extends SceneObjectBase<RowActionsState> {
  ...
  public onUpdate(title: string, repeat: string | null | undefined, collapsedIcon?: string, rowGroup?: string) {
    ...
    // Update title, collapsedIcon, and rowGroup together
    const stateUpdate: Record<string, unknown> = {};
    if (title !== row.state.title) {
      stateUpdate.title = title;
    }
    // Store collapsedIcon in row state (custom property)
    if (collapsedIcon !== (row.state as any).collapsedIcon) {
      stateUpdate.collapsedIcon = collapsedIcon;
    }
    // Store rowGroup in row state (custom property)
    if (rowGroup !== (row.state as any).rowGroup) {
      stateUpdate.rowGroup = rowGroup;
    }
    if (Object.keys(stateUpdate).length > 0) {
      row.setState(stateUpdate as any);
    }
