...
export function DashboardGridItemRenderer({ model }: SceneComponentProps<DashboardGridItem>) {
  ...
  if (!variableName) {
    if (body instanceof VizPanel) {
      return (
        <div className={panelWrapper} ref={model.containerRef} data-export-key={body.state.key}>
  ...
  return (
    <div className={layoutStyle} ref={model.containerRef}>
      {repeatedPanels.map((panel) => (
        <div className={panelWrapper} key={panel.state.key} data-export-key={panel.state.key}>
