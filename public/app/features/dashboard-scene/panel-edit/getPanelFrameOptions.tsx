...
export function getPanelFrameOptions(panel: VizPanel): OptionsPaneCategoryDescriptor {
  ...
  descriptor
    ...
    .addItem(
      new OptionsPaneItemDescriptor({
        title: t('dashboard-scene.get-panel-frame-options.title.hide-title', 'Hide title'),
        id: 'hide-title',
        description: t('dashboard-scene.get-panel-frame-options.description.hide-title', 'Hide the panel title (only shows on hover)'),
        render: function renderHideTitle(descriptor) {
          return <PanelHideTitleSwitch id={descriptor.props.id} panel={panel} />;
        },
      })
    )
    .addCategory(
...
export function PanelHideTitleSwitch({ panel, id }: { panel: VizPanel; id?: string }) {
  const { hoverHeader = false, title } = panel.useState();

  // When hoverHeader is true and there's a title, the title is hidden
  // This is distinct from having no title at all
  const isHidden = hoverHeader && !!title;

  const onChange = () => {
    const newHoverHeader = !isHidden;

    dashboardEditActions.edit({
      description: t('dashboard.edit-actions.panel-hide-title', 'Toggle panel title visibility'),
      source: panel,
      perform: () => panel.setState({ hoverHeader: newHoverHeader }),
      undo: () => panel.setState({ hoverHeader: hoverHeader }),
    });
  };

  return <Switch value={isHidden} id={id} onChange={onChange} />;
}
