...
function createRowFromPanelModel(row: PanelModel, content: SceneGridItemLike[]): SceneGridItemLike {
  ...
  // Load collapsedIcon if present
  if ((row as any).collapsedIcon) {
    gridRowState.collapsedIcon = (row as any).collapsedIcon;
  }

  // Load rowGroup if present
  if ((row as any).rowGroup) {
    gridRowState.rowGroup = (row as any).rowGroup;
  }

  return new SceneGridRow(gridRowState);
}

function createRowItemFromLegacyRow(row: PanelModel, panels: DashboardGridItem[]): RowItem {
  const rowItemState: any = {
    key: getVizPanelKeyForPanelId(row.id),
    title: row.title,
  ...
  // Load collapsedIcon if present
  if (row.collapsedIcon) {
    rowItemState.collapsedIcon = row.collapsedIcon;
  }

  // Load rowGroup if present
  if (row.rowGroup) {
    rowItemState.rowGroup = row.rowGroup;
  }

  const rowItem = new RowItem(rowItemState);
  return rowItem;
}
...
export function buildGridItemForPanel(panel: PanelModel): DashboardGridItem {
  ...
  // hideTitle allows users to hide the panel title while keeping it in the config
  // When hideTitle is true, hoverHeader is set to true (title only shows on hover)
  const shouldHideTitle = panel.hideTitle === true;
  const hoverHeader = shouldHideTitle || (!panel.title && !timeOverrideShown);
  ...
  const vizPanelState: VizPanelState = {
    ...
    // hideTitle controls whether the header is always visible or only on hover
    hoverHeader,
    hoverHeaderOffset: 0,
  ...
  // Only add menu for admin users (non-admin users use Export Panels footer)
  const isAdmin = contextSrv.hasRole('Admin') || contextSrv.isGrafanaAdmin;
  if (!config.publicDashboardAccessToken && isAdmin) {
