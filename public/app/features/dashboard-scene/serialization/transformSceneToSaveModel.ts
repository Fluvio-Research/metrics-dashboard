...
export function vizPanelToPanel(
  ...
  // Save hideTitle if the panel has hoverHeader enabled but still has a title
  // (original behavior sets hoverHeader=true only when title is empty)
  if (vizPanel.state.hoverHeader && vizPanel.state.title) {
...
export function gridRowToSaveModel(gridRow: SceneGridRow, panelsArray: Array<Panel | RowPanel>, isSnapshot = false) {
  ...
  // Save collapsedIcon if present
  const collapsedIcon = (gridRow.state as any).collapsedIcon;
  if (collapsedIcon) {
    (rowPanel as any).collapsedIcon = collapsedIcon;
  }

  // Save rowGroup if present
  const rowGroup = (gridRow.state as any).rowGroup;
  if (rowGroup) {
    (rowPanel as any).rowGroup = rowGroup;
  }
