...
export function serializeRow(row: RowItem): RowsLayoutRowKind {
  ...
  // Add collapsedIcon if present
  if (row.state.collapsedIcon) {
    (rowKind.spec as any).collapsedIcon = row.state.collapsedIcon;
  }

  // Add rowGroup if present
  if (row.state.rowGroup) {
    (rowKind.spec as any).rowGroup = row.state.rowGroup;
  }
...
export function deserializeRow(
  ...
  return new RowItem({
    ...
    collapsedIcon: (row.spec as any).collapsedIcon,
    rowGroup: (row.spec as any).rowGroup,
