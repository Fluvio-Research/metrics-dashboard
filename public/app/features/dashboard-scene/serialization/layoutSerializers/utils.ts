...
export function buildVizPanel(panel: PanelKind, id?: number): VizPanel {
  ...
  const shouldHideTitle = (panel.spec as any).hideTitle === true;
  const hoverHeader = shouldHideTitle || (!panel.spec.title && !timeOverrideShown);
...
export function buildLibraryPanel(panel: LibraryPanelKind, id?: number): VizPanel {
  ...
  // Only add menu for admin users (non-admin users use Export Panels footer)
  const isAdminUser = contextSrv.hasRole('Admin') || contextSrv.isGrafanaAdmin;
  if (!config.publicDashboardAccessToken && isAdminUser) {
    vizPanelState.menu = new VizPanelMenu({
      $behaviors: [panelMenuBehavior],
    });
  }
