...
export function TextBoxVariablePicker({ variable, onVariableChange, readOnly }: Props): ReactElement {
  ...
  const handleClearClick = useCallback(() => {
    setUpdatedValue('');
    // Immediately apply the empty value
    if (!variable.rootStateKey) {
      console.error('Cannot update variable without rootStateKey');
      return;
    }
    ...
    dispatch(
      toKeyedAction(
        variable.rootStateKey,
        changeVariableProp(
          toVariablePayload({ id: variable.id, type: variable.type }, { propName: 'query', propValue: '' })
        )
      )
    );
    ...
    if (onVariableChange) {
      onVariableChange({
        ...variable,
        current: isEmptyObject(variable.current) ? {} : { ...variable.current, value: '' },
      });
      return;
    }
  ...
  const hasValue = updatedValue && updatedValue.toString().trim().length > 0;
      ...
      suffix={
        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
          ...
          <Button
            ...
            tooltip="Apply filter (or press Enter)"
            variant="primary"
          />
        </div>
      }
