...
export function getPanelFrameCategory(props: OptionPaneRenderProps): OptionsPaneCategoryDescriptor {
  ...
  return descriptor
    ...
    .addItem(
      new OptionsPaneItemDescriptor({
        title: t('dashboard.get-panel-frame-category.title.hide-title', 'Hide title'),
        id: 'hide-title',
        description: t('dashboard.get-panel-frame-category.description.hide-title', 'Hide the panel title (only shows on hover)'),
        render: function renderHideTitle(descriptor) {
          return (
            <Switch
              data-testid={selectors.components.PanelEditor.OptionsPane.fieldInput('Hide title')}
              value={panel.hideTitle}
              id={descriptor.props.id}
              onChange={(e) => onPanelConfigChange('hideTitle', e.currentTarget.checked)}
            />
          );
        },
      })
