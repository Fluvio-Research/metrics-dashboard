...
import { CustomPanelExportModal } from './CustomPanelExportModal';
...
function getTabs(canEditDashboard: boolean, panel?: PanelModel, activeTab?: string) {
  ...
  // For non-admin users with panels, use custom export component
  if (!isAdmin && panel) {
    const tabs: ShareModalTabModel[] = [];

    // Use custom panel export component
    const exportLabel = t('share-modal.tab-title.export', 'Export Panel');
    tabs.push({ label: exportLabel, value: shareDashboardType.link, component: CustomPanelExportModal });
