...
interface PanelOption extends SelectableValue<number> {
  panel: PanelModel;
  size: number;
  width: number;
  height: number;
...
export function ExportPanelsFooter({ dashboard }: Props) {
  ...
  const panelOptions: PanelOption[] = (() => {
    const collected: PanelModel[] = [];
    const seenIds = new Set<number>();

    // Collect panels from top level and any collapsed rows to avoid missing nested panels
    const collectPanels = () => {
    ...
    const enriched: PanelOption[] = [];

    for (const panel of collected) {
      // Skip structural row panels
      if (!panel || panel.type === 'row') {
        continue;
      }

      // Avoid duplicates when panels are repeated or attached to rows
      if (seenIds.has(panel.id)) {
        continue;
      }
      seenIds.add(panel.id);
      
      const width = panel.gridPos?.w ?? 12;
      const height = panel.gridPos?.h ?? 8;
    ...
    return enriched.sort((a, b) => b.size - a.size);
  })();

  // Find panel element by ID
  const findPanelElement = useCallback((panelId: number, panelTitle: string): HTMLElement | null => {
    // Strategy 1: Try to find by data-export-key
    let element = document.querySelector(`[data-export-key="panel-${panelId}"]`) as HTMLElement;
    if (element) {
      return element;
    }
  ...
  const buildRenderUrl = useCallback(
    (panelId: number, opts?: { width?: number; height?: number; theme?: 'dark' | 'light' }) => {
      try {
        ...
        if ((dashboard as any).uid) {
          const uid = (dashboard as any).uid;
  ...
  const captureScreenshot = useCallback(async (panelId: number, panelTitle: string): Promise<Blob> => {
    const element = findPanelElement(panelId, panelTitle);
    ...
    const renderUrl = buildRenderUrl(panelId, {
    ...
    const canvas = await html2canvas(element, {
      ...
      onclone: (doc) => {
        ...
        const el = doc.querySelector(`[data-export-key="panel-${panelId}"]`) as HTMLElement | null;
        if (el) {
  ...
  const handleExport = async () => {
    ...
    try {
      const blob = await captureScreenshot(selectedPanel.panelId, selectedPanel.originalTitle);
  ...
  const formatOptionLabel = (item: SelectableValue<number>) => {
