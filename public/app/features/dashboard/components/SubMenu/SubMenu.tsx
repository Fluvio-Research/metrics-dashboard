...
class SubMenuUnConnected extends PureComponent<Props> {
  ...
  render() {
    ...
    const hasFilterTag = dashboard.tags && dashboard.tags.includes('filter');

    return (
      <div className={styles.submenu}>
        {hasFilterTag && variables.length > 0 ? (
          // Use modern collapsible filter bar for dashboards with "filter" tag
          <CollapsibleFilterBar variables={variables} readOnly={readOnlyVariables} />
        ) : (
          // Use traditional inline variables for other dashboards
          <form
