...
} from 'app/plugins/panel/geomap/components/AdvancedTooltip';
...
export const DataHoverRow = ({ feature, onClose }: Props) => {
  let data: DataFrame;
  let rowIndex = 0;
  if (!feature) {
    return null;
  ...
  const advancedConfig = feature.get('advancedTooltip') as AdvancedTooltipConfig | undefined;
  if (advancedConfig && hasAdvancedTooltipContent(feature, advancedConfig)) {
    return <AdvancedTooltip feature={feature} config={advancedConfig} onClose={onClose} />;
  }
...
function hasAdvancedTooltipContent(feature: FeatureLike, config: AdvancedTooltipConfig): boolean {
  if (config.imageUrl) {
    return true;
  }
