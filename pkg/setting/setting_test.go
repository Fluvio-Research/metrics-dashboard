...
func TestLoadingSettings(t *testing.T) {
	...
	t.Run("grafana.com API URL can be set separately from grafana.com URL", func(t *testing.T) {
		t.Setenv("GF_GRAFANA_NET_URL", "https://metrics-dashboard-dev.com")
		t.Setenv("GF_GRAFANA_COM_API_URL", "http://metrics-dashboard-dev.internal/api")
		cfg := NewCfg()
		err := cfg.Load(CommandLineArgs{HomePath: "../../", Config: "../../conf/defaults.ini"})
		require.Nil(t, err)
		require.Equal(t, "https://metrics-dashboard-dev.com", cfg.GrafanaComURL)
		require.Equal(t, "http://metrics-dashboard-dev.internal/api", cfg.GrafanaComAPIURL)
	})
