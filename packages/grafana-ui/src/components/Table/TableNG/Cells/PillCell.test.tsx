...
describe('PillCell', () => {
  ...
  describe('Color by hash (classic palette)', () => {
    ...
    it('JSON array with objects (should convert to strings)', () => {
      // This test ensures objects in JSON arrays are converted to strings
      // to prevent React error #310 ("Objects are not valid as a React child")
      const result = render(<PillCell {...props} value='[{"name":"test"},{"id":123}]' />);
      const spans = result.container.querySelectorAll('span');
      expect(spans.length).toBe(2);
      expect(spans[0].textContent).toBe('{"name":"test"}');
      expect(spans[1].textContent).toBe('{"id":123}');
    });

    it('JSON array with mixed types (should convert all to strings)', () => {
      const result = render(<PillCell {...props} value='["string",123,true,null]' />);
      const spans = result.container.querySelectorAll('span');
      expect(spans.length).toBe(4);
      expect(spans[0].textContent).toBe('string');
      expect(spans[1].textContent).toBe('123');
      expect(spans[2].textContent).toBe('true');
