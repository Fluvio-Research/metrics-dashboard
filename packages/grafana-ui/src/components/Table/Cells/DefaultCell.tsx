...
export const DefaultCell = (props: TableCellProps) => {
  ...
  // Column-level styling from field config (set per-column in Field overrides)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const columnStyling = (field.config.custom as any)?.columnStyling as
    | {
  ...
  // Font weight from column styling — applied as inline style since buildCellContainerStyle doesn't support it
  const colFontWeight =
    columnStyling?.fontWeight === 'bold' ? '700' : columnStyling?.fontWeight === 'light' ? '300' : undefined;
  ...
  return (
    ...
    <div
      ...
      style={{
        ...
        ...(colFontWeight && { fontWeight: colFontWeight }),
      }}
...
function getCellStyle(
  ...
  // Get colors from cell display options (thresholds, colour modes, etc.)
  const colors = getCellColors(tableStyles.theme, cellOptions, displayValue);
  textColor = colors.textColor;
  bgColor = colors.bgColor;
  bgHoverColor = colors.bgHoverColor;

  // Column-level styling overrides (highest priority — explicit user setting)
  if (columnStyling) {
    if (columnStyling.gradientEnabled && columnStyling.gradientStart && columnStyling.gradientEnd) {
      bgColor = `linear-gradient(180deg, ${columnStyling.gradientStart}, ${columnStyling.gradientEnd})`;
    } else if (columnStyling.backgroundColor) {
      bgColor = columnStyling.backgroundColor;
    }
    if (columnStyling.textColor) {
      textColor = columnStyling.textColor;
    }
  }
