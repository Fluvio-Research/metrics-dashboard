...
Form component provides a way to build simple forms at Metrics Dashboard. It is built on top of [react-hook-form](https://react-hook-form.com/) library and incorporates the same concepts while adjusting the API slightly.

**Note: This component is deprecated and will be removed in the future versions of metrics-dashboard/ui. Use the `useForm` hook from react-hook-form instead.**
...
```jsx
import { Form, Field, InputControl } from '@metrics-dashboard/ui';
...
Version 8 of Metrics Dashboard-UI is using version 7 of `react-hook-form` (previously version 5 was used), which introduced a few breaking changes to the `Form` API. The detailed list of changes can be found in the library's migration guides:
