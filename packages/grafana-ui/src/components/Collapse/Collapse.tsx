...
const getStyles = (theme: GrafanaTheme2) => ({
  ...
  collapsedIcon: css({
    label: 'collapse__collapsed-icon',
    display: 'flex',
    alignItems: 'center',
...
export interface Props {
  ...
  /** Element or text to show when collapsed (e.g., icon/image). If not provided, uses label */
...
export const Collapse = ({
  ...
  // Use collapsedLabel when collapsed, otherwise use label
  const displayLabel = !isOpen && collapsedLabel !== undefined ? collapsedLabel : label;
  const isCollapsedWithCustomLabel = !isOpen && collapsedLabel !== undefined;
  ...
  return (
    <div className={panelClass}>
      <button type="button" className={cx(buttonStyles, headerClass)} onClick={onClickToggle}>
        ...
        <div className={cx([style.headerLabel], isCollapsedWithCustomLabel && style.collapsedIcon)}>
          {displayLabel}
        </div>
      </button>
      {isOpen && (
