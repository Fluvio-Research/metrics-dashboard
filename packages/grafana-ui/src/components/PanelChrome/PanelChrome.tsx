...
interface BaseProps {
  ...
  /**
   * Show a centered spinner overlay during loading instead of just the loading bar.
   * Useful for panels that need a full reload indicator (like chart panels).
   * Default: false (shows loading bar only)
   */
...
export function PanelChrome({
  ...
  return (
    ...
    >
      ...
      {/* Centered spinner overlay for loading state - with delay to avoid flash on fast loads */}
      {showLoadingSpinner && loadingState === LoadingState.Loading && (
        <DelayRender delay={400}>
          <div className={styles.loadingSpinnerOverlay}>
            <div className={styles.loadingSpinnerContainer}>
              <Spinner size="lg" />
              <span className={styles.loadingText}>
                {t('grafana-ui.panel-chrome.loading-text', 'Loading...')}
              </span>
            </div>
          </div>
        </DelayRender>
...
const getStyles = (theme: GrafanaTheme2) => {
  ...
  return {
    ...
    loadingSpinnerOverlay: css({
      ...
      alignItems: 'center',
      justifyContent: 'center',
      backgroundColor: theme.isDark ? 'rgba(17, 18, 23, 0.8)' : 'rgba(255, 255, 255, 0.85)',
      zIndex: 2,
      borderRadius: theme.shape.radius.default,
