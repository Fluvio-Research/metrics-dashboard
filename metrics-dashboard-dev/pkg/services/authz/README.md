...
The `listen` setting determines whether the authorization server should listen for incoming requests. When set to `true`, the authorization service will be registered to the Metrics Dashboard GRPC server.

The default configuration does not register the authorization service on the Metrics Dashboard GRPC server and binds the client to it `inproc`:
...
Here is an example to register the authorization service on the Metrics Dashboard GRPC server and connect the client to it through grpc.
