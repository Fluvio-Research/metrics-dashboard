...
The unified storage projects aims to provide a simple and extensible backend to unify the way we store different objects within the Metrics Dashboard app platform.

It provides generic storage for k8s objects, and can store data either within dedicated tables in the main Metrics Dashboard database, or in separate storage.

By default it runs in-process within Metrics Dashboard, but it can also be run as a standalone GRPC service (`storage-server`).
...
[feature_toggles]
metrics-dashboardAPIServerWithExperimentalAPIs = true
kubernetesClientDashboardsFolders = true

[unified_storage.folders.folder.metrics-dashboard.app]
dualWriterMode = 4

[unified_storage.dashboards.dashboard.metrics-dashboard.app]
dualWriterMode = 4
...
With this configuration, you can run everything in-process. Run the Metrics Dashboard backend with:
...
The default kubeconfig sends requests directly to the apiserver, to authenticate as a metrics-dashboard user, create `metrics-dashboard.kubeconfig`:
```yaml
apiVersion: v1
clusters:
- cluster:
...
- name: default
  user:
    ...
    password: <password>
```
Where `<username>` and `<password>` are credentials for basic auth against Metrics Dashboard. For example, with the [default credentials](https://github.com/metrics-dashboard/metrics-dashboard/blob/HEAD/contribute/developer-guide.md#backend):
```yaml
    username: admin
    password: admin
```
...
In this mode, you can interact with the k8s api. Make sure you are in the directory where you created `metrics-dashboard.kubeconfig`. Then run:
```sh
kubectl --kubeconfig=./metrics-dashboard.kubeconfig get playlist
```
...
```yaml
apiVersion: playlist.metrics-dashboard.app/v0alpha1
kind: Playlist
metadata:
  ...
  labels:
    foo: bar
  annotations:
    metrics-dashboard.app/slug: "slugger"
    metrics-dashboard.app/updatedBy: "updater"
spec:
...
then run:
```sh
kubectl --kubeconfig=./metrics-dashboard.kubeconfig create -f playlist-generate.yaml
```

For example, a successful response would be:
```sh
playlist.playlist.metrics-dashboard.app/u394j4d3-s63j-2d74-g8hf-958773jtybf2 created
```
...
```sh
kubectl --kubeconfig=./metrics-dashboard.kubeconfig patch playlist <NAME> --patch-file playlist-generate.yaml
```
...
Make sure you are in the directory where you created `metrics-dashboard.kubeconfig`. Then run:
```sh
kubectl --kubeconfig=./metrics-dashboard.kubeconfig get folder
```
...
then run:
```sh
kubectl --kubeconfig=./metrics-dashboard.kubeconfig create -f folder-generate.yaml
```
...
Make sure you have the gRPC address in the `[metrics-dashboard-apiserver]` section of your config file:
```ini
[metrics-dashboard-apiserver]
...
; http url to Metrics Dashboard's signing keys to validate incoming id tokens
...
```sh
GF_DEFAULT_TARGET=storage-server ./bin/metrics-dashboard server target
```
...
To run metrics-dashboard against the storage-server, override the `storage_type` setting:
```sh
GF_METRICS_DASHBOARD_APISERVER_STORAGE_TYPE=unified-grpc ./bin/metrics-dashboard server
```
...
```ini
[feature_toggles]
; Used by the Metrics Dashboard instance
...
## Running load tests
Load tests and instructions can be found [here](https://github.com/metrics-dashboard/metrics-dashboard-api-tests/tree/main/simulation/src/unified_storage).
...
```sh
docker run -d --name db -e "MYSQL_DATABASE=metrics-dashboard" -e "MYSQL_USER=metrics-dashboard" -e "MYSQL_PASSWORD=metrics-dashboard" -e "MYSQL_ROOT_PASSWORD=root" -p 3306:3306 docker.io/bitnami/mysql:8.0.31
```
...
db_name = metrics-dashboard ; or whatever you defined in your currently running database
db_user = metrics-dashboard ; or whatever you defined in your currently running database
db_pass = metrics-dashboard ; or whatever you defined in your currently running database
...
Example metrics-dashboard ini file:
...
```sh
./bin/metrics-dashboard server target --config conf/distributor.ini
```

Repeat for the other services.

```sh
./bin/metrics-dashboard server target --config conf/storage-api-1.ini
./bin/metrics-dashboard server target --config conf/storage-api-2.ini
./bin/metrics-dashboard server target --config conf/storage-api-3.ini

./bin/metrics-dashboard server target --config conf/metrics-dashboard1.ini
./bin/metrics-dashboard server target --config conf/metrics-dashboard2.ini
./bin/metrics-dashboard server target --config conf/metrics-dashboard3.ini
```
...
If all is well, you will be able to visit every metrics-dashboard stack you started and use it normally. Visit
