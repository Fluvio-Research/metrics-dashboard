...
aliases:
  - ../../http_api/user/
canonical: /docs/metrics-dashboard/latest/developers/http_api/user/
description: Metrics Dashboard User HTTP API
keywords:
  - metrics-dashboard
  - http
  - documentation
...
Alternatively, you can use the [Organization HTTP API](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/developers/http_api/org/#current-organization-api) with service account tokens to manage users in a specific organization
...
For Metrics Dashboard Cloud customers, refer to [Organization HTTP API](/docs/metrics-dashboard/latest/developers/http_api/org/) for finding users with the org Admin role.

> If you are running Metrics Dashboard Enterprise, for some endpoints you'll need to have specific permissions. Refer to [Role-based access control permissions](ref:role-based-access-control-permissions) for more information.
...
Default value for the `perpage` parameter is `1000` and for the `page` parameter is `1`. Requires basic authentication and that the authenticated user is a Metrics Dashboard Admin.
...
Requires basic authentication and that the authenticated user is a Metrics Dashboard Admin.
...
curl -X PUT -H "Content-Type: application/json" -d '{
  ...
  "newPassword": "newpass",
  "confirmNew": "newpass"
}' http://admin:oldpass@<your_metrics-dashboard_host>:3000/api/user/password
```
...
Switch user context to the given organization. Requires basic authentication and that the authenticated user is a Metrics Dashboard Admin.
