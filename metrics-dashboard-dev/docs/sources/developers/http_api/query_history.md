...
canonical: /docs/metrics-dashboard/latest/developers/http_api/query_history/
description: Metrics Dashboard Query History HTTP API
keywords:
  - metrics-dashboard
  - http
  - documentation
...
- **from/to** - Specifies time range for the query history search. The time can be either epoch timestamps in milliseconds or relative using Metrics Dashboard time units. For example, now-5m.
