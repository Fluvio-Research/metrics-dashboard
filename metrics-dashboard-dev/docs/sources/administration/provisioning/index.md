...
description: Describes provisioning settings for Metrics Dashboard using configuration files.
keywords:
  - metrics-dashboard
  - provisioning
labels:
  products:
    - enterprise
    - oss
title: Provision Metrics Dashboard
weight: 600
---

# Provision Metrics Dashboard

Metrics Dashboard has an active provisioning system that uses configuration files.
...
Refer to [Configuration](../../setup-metrics-dashboard/configure-metrics-dashboard/) for more information on what you can configure in `metrics-dashboard.ini`.

### Configuration file locations

Metrics Dashboard reads its default configuration from `<WORKING DIRECTORY>/conf/defaults.ini`.
By default, Metrics Dashboard reads custom configuration from `<WORKING DIRECTORY>/conf/custom.ini`.
You can override the custom configuration path with the `--config` option.

{{< admonition type="note" >}}
The Deb and RPM packages install the configuration file at `/etc/metrics-dashboard/metrics-dashboard.ini`.
The Metrics Dashboard init.d script sets the `--config` option to that path.
{{< /admonition >}}
...
The Metrics Dashboard community maintains libraries for many popular configuration management tools.

| Tool      | Project                                                                                                                           |
| --------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Puppet    | <https://forge.puppet.com/puppet/metrics-dashboard>                                                                                         |
| Ansible   | <https://github.com/metrics-dashboard/metrics-dashboard-ansible-collection>                                                                           |
| Chef      | <https://github.com/sous-chefs/chef-metrics-dashboard>                                                                                      |
| Saltstack | <https://github.com/salt-formulas/salt-formula-metrics-dashboard>                                                                           |
| Jsonnet   | <https://github.com/metrics-dashboard/grafonnet-lib/>)                                                                                      |
| NixOS     | [`services.metrics-dashboard.provision` module](https://github.com/NixOS/nixpkgs/blob/master/nixos/modules/services/monitoring/metrics-dashboard.nix) |

## Data sources

You can manage data sources in Metrics Dashboard by adding YAML configuration files in the [`provisioning/datasources`](../../setup-metrics-dashboard/configure-metrics-dashboard/#provisioning) directory.
Each configuration file contains a list of data sources, under the `datasources` key, to add or update during startup.
If the data source already exists, Metrics Dashboard reconfigures it to match the provisioned configuration file.

You can also list data sources to automatically delete, using the key `deleteDatasources`.
Metrics Dashboard deletes the data sources listed in `deleteDatasources` _before_ adding or updating those in the `datasources` list.

You can configure Metrics Dashboard to automatically delete provisioned data sources when they're removed from the provisioning file.
To do so, add `prune: true` to the root of your data source provisioning file.
With this configuration, Metrics Dashboard also removes the provisioned data sources if you remove the provisioning file entirely.

### Run multiple Metrics Dashboard instances

If you run multiple instances of Metrics Dashboard, add a version number to each data source in the configuration and increase it when you update the configuration.
Metrics Dashboard only updates data sources with the same or lower version number than the one set in the configuration file.
...
datasources:
  ...
  - name: Graphite
    ...
    # If not specified, Metrics Dashboard generates one.
    uid: my_unique_uid
...
| `maxSeries`                     | number  | InfluxDB                                                         | Max number of series/tables that Metrics Dashboard processes                                                                                                                                                                                                                                            |
...
| `sslRootCertFile`               | string  | PostgreSQL, MSSQL                                                | SSL server root certificate file, must be readable by the Metrics Dashboard user                                                                                                                                                                                                                        |
| `sslCertFile`                   | string  | PostgreSQL                                                       | SSL client certificate file, must be readable by the Metrics Dashboard user                                                                                                                                                                                                                             |
| `sslKeyFile`                    | string  | PostgreSQL                                                       | SSL client key file, must be readable by _only_ the Metrics Dashboard user                                                                                                                                                                                                                              |
...
| `maxOpenConns`                  | number  | MySQL, PostgreSQL and MSSQL                                      | Maximum number of open connections to the database (Metrics Dashboard v5.4+)                                                                                                                                                                                                                            |
| `maxIdleConns`                  | number  | MySQL, PostgreSQL and MSSQL                                      | Maximum number of connections in the idle connection pool (Metrics Dashboard v5.4+)                                                                                                                                                                                                                     |
| `connMaxLifetime`               | number  | MySQL, PostgreSQL and MSSQL                                      | Maximum amount of time in seconds a connection may be reused (Metrics Dashboard v5.4+)                                                                                                                                                                                                                  |
...
| `handleMetrics DashboardManagedAlerts`    | boolean | Alertmanager                                                     | When enabled, Metrics Dashboard-managed alerts are sent to this Alertmanager                                                                                                                                                                                                                            |
...
Secure JSON data is a map of settings encrypted with a [secret key](../../setup-metrics-dashboard/configure-metrics-dashboard/#secret_key).
The encryption hides the JSON data from the users of Metrics Dashboard.
...
You can add HTTP headers to all requests sent to data sources managed by Metrics Dashboard provisioning
...
You can manage plugin applications in Metrics Dashboard by adding one or more YAML configuration files in the [`provisioning/plugins`](../../setup-metrics-dashboard/configure-metrics-dashboard/#provisioning) directory.
Each configuration file contains a list of `apps` that Metrics Dashboard configures during start up.
Metrics Dashboard configures each app to use the configuration in the file.
...
You can manage dashboards in Metrics Dashboard by adding one or more YAML configuration files in the [`provisioning/dashboards`](../../setup-metrics-dashboard/configure-metrics-dashboard/#dashboards) directory.
Each configuration file contains a list of `providers` that Metrics Dashboard uses to load dashboards from the local filesystem.
...
providers:
  ...
  - name: 'a unique provider name'
    ...
    # <int> how often Metrics Dashboard will scan for changed dashboards
    updateIntervalSeconds: 10
    # <bool> allow updating provisioned dashboards from the UI
    allowUiUpdates: false
    options:
      # <string, required> path to dashboard files on disk. Required when using the 'file' type
      path: /var/lib/metrics-dashboard/dashboards
      # <bool> use folder names from filesystem to create folders in Metrics Dashboard
      foldersFromFilesStructure: true
```

When Metrics Dashboard starts, it updates or creates all dashboards found in the configured path.
It later polls that path every `updateIntervalSeconds` for updates to the dashboard files and updates its database.

{{< admonition type="note" >}}
Metrics Dashboard installs dashboards at the root level if you don't set the `folder` field.
{{< /admonition >}}

#### Make changes to a provisioned dashboard

You can make changes to a provisioned dashboard in the Metrics Dashboard UI but its not possible to automatically save the changes back to the provisioning source.
If `allowUiUpdates` is set to `true` and you make changes to a provisioned dashboard, when you save the dashboard, Metrics Dashboard persists the changes to its database.

{{< admonition type="caution" >}}
If you save a provisioned dashboard in the UI and then later update the provisioning source, Metrics Dashboard always overwrites the database dashboard with the one from the provisioning file.
Metrics Dashboard ignores the `version` property in the JSON file, even if it's lower than the dashboard in the database.
{{< /admonition >}}

{{< admonition type="caution" >}}
If you save a provisioned dashboard in the UI and remove the provisioning source, Metrics Dashboard deletes the dashboard in the database unless you have set the option `disableDeletion` to `true`.
{{< /admonition >}}

If you set `allowUiUpdates` to `false`, you can't save changes to a provisioned dashboard.
When you try to save changes to a provisioned dashboard, Metrics Dashboard brings up a _Cannot save provisioned dashboard_ dialog box.

Metrics Dashboard offers options to export the JSON definition of a dashboard.
Use either **Copy JSON to Clipboard** or **Save JSON to file** to sync your dashboard changes back to the provisioning source.
Metrics Dashboard removes the `id` field from the dashboard JSON to help the provisioning workflow.
...
If the dashboard in the JSON file contains an [UID](../../dashboards/build-dashboards/view-dashboard-json-model/), Metrics Dashboard updates that the dashboard with that UID in the database.
This lets you migrate dashboards between Metrics Dashboard instances and keep consistent dashboard URLs.
When Metrics Dashboard starts, it creates or updates all dashboards available in the configured folders.

{{< admonition type="caution" >}}
You can overwrite existing dashboards with provisioning.

Be careful not to reuse the same `title` multiple times within a folder or `uid` within the same Metrics Dashboard instance to avoid inconsistent behavior.
{{< /admonition >}}

### Provision folders structure from filesystem to Metrics Dashboard

If you already store your dashboards using folders in a Git repository or on a filesystem, and want to have the same folder names in the Metrics Dashboard menu, use `foldersFromFilesStructure` option.

For example, to replicate the following dashboards structure from the filesystem to Metrics Dashboard:
...
Metrics Dashboard creates the `server` and `application` folders in the UI.
...
For information on provisioning Metrics Dashboard Alerting, refer to [Provision Metrics Dashboard Alerting resources](../../alerting/set-up/provision-alerting-resources/).
...
Metrics Dashboard encrypts secure settings in the database.
...
Metrics Dashboard Enterprise supports:

- [Provisioning role-based access control with Metrics Dashboard](../roles-and-permissions/access-control/rbac-metrics-dashboard-provisioning/)
