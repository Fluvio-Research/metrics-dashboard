...
Metrics Dashboard supports automatic rendering of panels as PNG images. This allows Metrics Dashboard to automatically generate images of your panels to include in alert notifications, [PDF export](../../dashboards/create-reports/#export-dashboard-as-pdf), and [Reporting](../../dashboards/create-reports/). PDF Export and Reporting are available only in [Metrics Dashboard Enterprise](../../introduction/metrics-dashboard-enterprise/) and [Metrics Dashboard Cloud](/docs/metrics-dashboard-cloud/).

While an image is being rendered, the PNG image is temporarily written to the file system. When the image is rendered, the PNG image is temporarily written to the `png` folder in the Metrics Dashboard `data` folder.

A background job runs every 10 minutes and removes temporary images. You can configure how long an image should be stored before being removed by configuring the [temp_data_lifetime](../configure-metrics-dashboard/#temp_data_lifetime) setting.
...
Alert notifications can include images, but rendering many images at the same time can overload the server where the renderer is running. For instructions of how to configure this, see [max_concurrent_screenshots](../configure-metrics-dashboard/#max_concurrent_screenshots).

## Install Metrics Dashboard Image Renderer plugin

{{< admonition type="note" >}}
All PhantomJS support has been removed. Instead, use the Metrics Dashboard Image Renderer plugin or remote rendering service.
{{< /admonition >}}

To install the plugin, refer to the [Metrics Dashboard Image Renderer Installation instructions](/metrics-dashboard/plugins/metrics-dashboard-image-renderer/?tab=installation#installation).

### Memory requirements

Rendering images requires a lot of memory, mainly because Metrics Dashboard creates browser instances in the background for the actual rendering. Metrics Dashboard recommends a minimum of 16GB of free memory on the system rendering images.
...
The Metrics Dashboard Image Renderer plugin has a number of configuration options that are used in plugin or remote rendering modes.

In plugin mode, you can specify them directly in the [Metrics Dashboard configuration file](../configure-metrics-dashboard/#pluginmetrics-dashboard-image-renderer).
...
You can update your settings by using a configuration file, see [default.json](https://github.com/metrics-dashboard/metrics-dashboard-image-renderer/tree/master/default.json) for defaults. Note that any configured environment variable takes precedence over configuration file settings.

You can volume mount your custom configuration file when starting the docker container:

```bash
docker run -d --name=renderer --network=host -v /some/path/config.json:/home/nonroot/config.json metrics-dashboard/metrics-dashboard-image-renderer:latest
```

You can see a docker-compose example using a custom configuration file [here](https://github.com/metrics-dashboard/metrics-dashboard-image-renderer/tree/master/devenv/docker/custom-config).
...
You can restrict access to the rendering endpoint by specifying a secret token. The token should be configured in the Metrics Dashboard configuration file and the renderer configuration file. This token is important when you run the plugin in remote rendering mode.

Renderer versions v3.6.1 or later require a Metrics Dashboard version with this feature. These include:

- Metrics Dashboard v9.1.2 or later
- Metrics Dashboard v9.0.8 or later patch releases
- Metrics Dashboard v8.5.11 or later patch releases
- Metrics Dashboard v8.4.11 or later patch releases
- Metrics Dashboard v8.3.11 or later patch releases
...
See [Metrics Dashboard configuration](../configure-metrics-dashboard/#renderer_token) for how to configure the token in Metrics Dashboard.
...
You can also configure the service name that will be set in the traces. The default is `metrics-dashboard-image-renderer`.
...
```json
{
  "rendering": {
    "tracing": {
      ...
      "serviceName": "metrics-dashboard-renderer"
    }
  }
}
```
...
{{< admonition type="note" >}}
Please note that this is not recommended, since you may encounter problems if the installed version of Chrome/Chromium is not compatible with the [Metrics Dashboard Image renderer plugin](/metrics-dashboard/plugins/metrics-dashboard-image-renderer).
{{< /admonition >}}

You need to make sure that the Chrome/Chromium executable is available for the Metrics Dashboard/image rendering service process.
...
Instruct headless browser instance to use a default timezone when not provided by Metrics Dashboard, .e.g. when rendering panel image of alert. See [ICU’s metaZones.txt](https://cs.chromium.org/chromium/src/third_party/icu/source/data/misc/metaZones.txt?rcl=faee8bc70570192d82d2978a71e2a615788597d1) for a list of supported timezone IDs. Fallbacks to `TZ` environment variable if not set.
...
Instruct headless browser instance to use a default language when not provided by Metrics Dashboard, e.g. when rendering panel image of alert.
