...
By exporting usage logs to Loki, you can directly query them and create dashboards of the information that matters to you most, such as dashboard errors, most active organizations, or your top-10 most-used queries. This configuration is done for you in Metrics Dashboard Cloud, with provisioned dashboards. Read about them in the [Metrics Dashboard Cloud documentation](/docs/metrics-dashboard-cloud/usage-insights/).
...
| `username`\* | string | Name of the Metrics Dashboard user that made the request. |
| `userId`\* | number | ID of the Metrics Dashboard user that made the request. |
...
To export your logs, enable the usage insights feature and [configure](../../configure-metrics-dashboard/) an export location in the configuration file:
...
The options for storage type are `loki` and `logger` (added in Metrics Dashboard Enterprise 8.2).

If the storage type is set to `loki` you'll need to also configure Metrics Dashboard
...
Using `logger` will print usage insights to your [Metrics Dashboard server log](../../configure-metrics-dashboard/#log).
There is no option for configuring the `logger` storage type.

## Visualize Loki usage insights in Metrics Dashboard

If you export logs into Loki, you can build Metrics Dashboard dashboards to understand your Metrics Dashboard instance usage.

1. Add Loki as a data source. Refer to [Metrics Dashboard fundamentals tutorial](/tutorials/metrics-dashboard-fundamentals/#6).
1. Import one of the following dashboards:
   - [Usage insights](/metrics-dashboard/dashboards/13785)
   - [Usage insights datasource details](/metrics-dashboard/dashboards/13786)
