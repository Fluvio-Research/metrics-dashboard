...
description: Learn how to use AWS KMS to encrypt secrets in the Metrics Dashboard database.
labels:
  products:
    - enterprise
    - oss
...
You can use an encryption key from AWS Key Management Service to encrypt secrets in the Metrics Dashboard database.
...
5. Add your AWS KMS details to the Metrics Dashboard configuration file; depending on your operating system, it is usually named `metrics-dashboard.ini`:
   ...
   An example of an AWS KMS provider section in the `metrics-dashboard.ini` file is as follows:
...
8. (Optional) From the command line and the root directory of Metrics Dashboard, re-encrypt all of the secrets within the Metrics Dashboard database with the new key using the following command:
