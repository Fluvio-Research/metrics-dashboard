...
labels:
  products:
    - cloud
    - enterprise
title: Audit a Metrics Dashboard instance
weight: 800
---

# Audit a Metrics Dashboard instance

Auditing allows you to track important changes to your Metrics Dashboard instance. By default, audit logs are logged to file but the auditing feature also supports sending logs directly to Loki.

{{< admonition type="note" >}}
To enable sending Metrics Dashboard Cloud audit logs to your Metrics Dashboard Cloud Logs instance, please [file a support ticket](/profile/org/tickets/new). Note that standard ingest and retention rates apply for ingesting these audit logs.
{{< /admonition >}}
...
| `user.userId`           | number  | ID of the Metrics Dashboard user that made the request.                                                                                                                                                                            |
...
| `user.name`             | string  | Name of the Metrics Dashboard user that made the request.                                                                                                                                                                          |
| `user.authTokenId`      | number  | ID of the user authentication token.                                                                                                                                                                                     |
| `user.apiKeyId`         | number  | ID of the Metrics Dashboard API key used to make the request.                                                                                                                                                                      |
...
| `metrics-dashboardVersion`\*      | string  | Current version of Metrics Dashboard when this log is created.                                                                                                                                                                     |
...
| ---------- | ------ | ----------- |
| `loginUsername` | `login` | Login used in the Metrics Dashboard authentication form. |
...
```json {hl_lines=4}
{
  ...
  "metrics-dashboardVersion": "8.3.0-pre"
}
```
...
\* Where `AUTH-MODULE` is the name of the authentication module: `metrics-dashboard`, `saml`,
`ldap`, etc. \
...
Audit logs can be saved into files, sent to a Loki instance or sent to the Metrics Dashboard default logger. By default, only the file exporter is enabled.
You can choose which exporter to use in the [configuration file](../../configure-metrics-dashboard/).
...
{{< admonition type="note" >}}
The HTTP option for the Loki exporter is available only in Metrics Dashboard Enterprise version 7.4 and later.
{{< /admonition >}}
...
If you have multiple Metrics Dashboard instances sending logs to the same Loki service or if you are using Loki for non-audit logs, audit logs come with additional labels to help identifying them:

- **host** - OS hostname on which the Metrics Dashboard instance is running.
- **metrics-dashboard_instance** - Application URL.
- **kind** - `auditing`
...
Audit logs are sent to the Metrics Dashboard default logger. The audit logs use the `auditing.console` logger and are logged on `debug`-level, learn how to enable debug logging in the [log configuration](../../configure-metrics-dashboard/#log) section of the documentation. Accessing the audit logs in this way is not recommended for production use.
