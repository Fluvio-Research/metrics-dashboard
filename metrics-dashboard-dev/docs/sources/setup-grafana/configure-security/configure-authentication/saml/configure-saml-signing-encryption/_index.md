...
Metrics Dashboard supports signed and encrypted responses, and _only_ supports signed requests.

## Certificate and private key

Commonly, the certificate and key are embedded in the IdP metadata and refreshed as needed by Metrics Dashboard automatically. However, if your IdP expects signed requests, you must supply a certificate and private key.

The SAML SSO standard uses asymmetric encryption to exchange information between the SP (Metrics Dashboard) and the IdP. To perform such encryption, you need a public part and a private part. In this case, the X.509 certificate provides the public part, while the private key provides the private part. The private key needs to be issued in a [PKCS#8](https://en.wikipedia.org/wiki/PKCS_8) format.

If you are directly supplying the certificate and key, Metrics Dashboard supports two ways of specifying both the `certificate` and `private_key`:

- Without a suffix (`certificate` or `private_key`), the configuration assumes you've supplied the base64-encoded file contents.
- With the `_path` suffix (`certificate_path` or `private_key_path`), then Metrics Dashboard treats the value entered as a path and attempts to read the file from the file system.
...
The SAML standard requires digital signatures for security-critical messages such as authentication and logout requests. When you configure the `signature_algorithm` option, Metrics Dashboard automatically signs these SAML requests using your configured private key and certificate.
...
- Metrics Dashboard uses the key and certificate specified in `private_key` and `certificate` options for signing
