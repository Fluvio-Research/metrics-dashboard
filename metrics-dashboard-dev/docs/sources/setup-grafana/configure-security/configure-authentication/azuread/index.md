...
aliases:
  - ../../../auth/azuread/
description: Metrics Dashboard Azure AD OAuth Guide
keywords:
  - metrics-dashboard
  - configuration
  - documentation
...
The Azure AD authentication allows you to use a Microsoft Entra ID (formerly known as Azure Active Directory) tenant as an identity provider for Metrics Dashboard. You can use Entra ID application roles to assign users and groups to Metrics Dashboard roles from the Azure Portal.

{{< admonition type="note" >}}
If Users use the same email address in Microsoft Entra ID that they use with other authentication providers (such as metrics-dashboard.com), you need to do additional configuration to ensure that the users are matched correctly. Please refer to [Using the same email address to login with different identity providers](../#using-the-same-email-address-to-login-with-different-identity-providers) for more information.
{{< /admonition >}}
...
1. Add the redirect URLs `https://<metrics-dashboard domain>/login/azuread` and `https://<metrics-dashboard domain>`, then click **Register**. The app's **Overview** page opens.
...
1. Click **Certificates & secrets** in the side menu, then add a new entry under the supported client authentication option you want to use. The following are the supported client authentication options with their respective configuration steps.
   - **Client secrets**
     1. Add a new entry under **Client secrets** with the following configuration.
        - Description: Metrics Dashboard OAuth 2.0
     ...
     {{< /admonition >}}
     1. You must have set `client_authentication` under `[auth.azuread]` to `client_secret_post` in the Metrics Dashboard server configuration for this to work.
   ...
   - **Federated credentials**
     - **_Managed Identity_**
       1. Refer to [Configure an application to trust a managed identity (preview)](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-config-app-trust-managed-identity?tabs=microsoft-entra-admin-center) for a complete guide on setting up a managed identity as a federated credential.
          ...
          - Description: Metrics Dashboard OAuth.
       ...
       1. You must have set `client_authentication` under `[auth.azuread]` to `managed_identity` in the Metrics Dashboard server configuration for this to work.
     ...
     - **_Workload Identity (K8s/AKS)_**
       1. Refer to [Federated identity credential for an Azure AD application](https://azure.github.io/azure-workload-identity/docs/topics/federated-identity-credential.html#azure-portal-ui) for a complete guide on setting up a federated credential for workload identity.
          ...
          - Namespace: Namespace of your Metrics Dashboard deployment. For example: `metrics-dashboard`.
          - Service account name: Service account name of your Metrics Dashboard deployment. For example: `metrics-dashboard`.
          - Subject identifier: The expected identity (subject claim) from the OIDC token, which Azure uses to validate and authorize token issuance to the requesting workload. For example: `system:serviceaccount:metrics-dashboard:metrics-dashboard`.
          - Name: A unique descriptive name for the credential.
          - Description: Metrics Dashboard OAuth.
          - Audience: The audience value that must appear in the external token. For Public cloud, it would be `api://AzureADTokenExchange`. See mentioned documentation for the full list of available audiences.

       1. You must have set `client_authentication` (env var `GF_AUTH_AZUREAD_CLIENT_AUTHENTICATION`) under `[auth.azuread]` to `workload_identity` in the Metrics Dashboard server configuration for this to work.

       1. You may optionally set `workload_identity_token_file` (env var `GF_AUTH_AZUREAD_WORKLOAD_IDENTITY_TOKEN_FILE`) under `[auth.azuread]` to `/var/run/secrets/azure/tokens/azure-identity-token` in the Metrics Dashboard server configuration for this to work. (Optional, defaults to `/var/run/secrets/azure/tokens/azure-identity-token`)

       1. You must have set `client_id` (env var `GF_AUTH_AZUREAD_CLIENT_ID`) under `[auth.azuread]` in the Metrics Dashboard server configuration for this to work. This must match the Entra ID/Azure AD App Registration Application (client) ID.

       1. You must have set `token_url` (env var `GF_AUTH_AZUREAD_TOKEN_URL`) under `[auth.azuread]` to `https://login.microsoftonline.com/{tenantID}/oauth2/v2.0/token` in the Metrics Dashboard server configuration for this to work.

       1. You must have set `auth_url` (env var `GF_AUTH_AZUREAD_AUTH_URL`) under `[auth.azuread]` to `https://login.microsoftonline.com/{tenantID}/oauth2/v2.0/authorize` in the Metrics Dashboard server configuration for this to work.

       1. You must have set `federated_credential_audience` (env var `GF_AUTH_AZUREAD_FEDERATED_CREDENTIAL_AUDIENCE`) under `[auth.azuread]` to `api://AzureADTokenExchange` in the Metrics Dashboard server configuration for this to work.
...
1. Define the required application roles for Metrics Dashboard [using the Azure Portal](#configure-application-roles-for-metrics-dashboard-in-the-azure-portal) or [using the manifest file](#configure-application-roles-for-metrics-dashboard-in-the-manifest-file).
...
1. Click **Add user/group** to add a user or group to the Metrics Dashboard roles.

{{< admonition type="note" >}}
When assigning a group to a Metrics Dashboard role, ensure that users are direct members of the group. Users in nested groups will not have access to Metrics Dashboard due to limitations within Azure AD/Entra ID side. For more information, see [Microsoft Entra service limits and restrictions](https://learn.microsoft.com/en-us/entra/identity/users/directory-service-limits-restrictions).
{{< /admonition >}}

### Configure application roles for Metrics Dashboard in the Azure Portal

This section describes setting up basic application roles for Metrics Dashboard within the Azure Portal. For more information, see [Add app roles to your application and receive them in the token](https://learn.microsoft.com/en-us/entra/identity-platform/howto-add-app-roles-in-apps).

1. Go to **App Registrations**, search for your application, and click it.

1. Click **App roles** and then **Create app role**.

1. Define a role corresponding to each Metrics Dashboard role: Viewer, Editor, and Admin.
   1. Choose a **Display name** for the role. For example, "Metrics Dashboard Editor".

   1. Set the **Allowed member types** to **Users/Groups**.

   1. Ensure that the **Value** field matches the Metrics Dashboard role name. For example, "Editor".

   1. Choose a **Description** for the role. For example, "Metrics Dashboard Editor Users".

   1. Click **Apply**.

### Configure application roles for Metrics Dashboard in the manifest file

If you prefer to configure the application roles for Metrics Dashboard in the manifest file, complete the following steps:
   	...
   	"appRoles": [
   			{
   				...
   				"description": "Metrics Dashboard org admin Users",
   				"displayName": "Metrics Dashboard Org Admin",
   				"id": "SOME_UNIQUE_ID",
   				"isEnabled": true,
   				"lang": null,
   				"origin": "Application",
   			...
   			},
   			{
   				...
   				"description": "Metrics Dashboard read only Users",
   				"displayName": "Metrics Dashboard Viewer",
   				"id": "SOME_UNIQUE_ID",
   				"isEnabled": true,
   				"lang": null,
   				"origin": "Application",
   			...
   			},
   			{
   				...
   				"description": "Metrics Dashboard Editor Users",
   				"displayName": "Metrics Dashboard Editor",
   				"id": "SOME_UNIQUE_ID",
   				"isEnabled": true,
   				"lang": null,
   				"origin": "Application",
...
The setting `allow_assign_metrics-dashboard_admin` under `[auth.azuread]` must be set to `true` for this to work.
If the setting is set to `false`, the user is assigned the role of `Admin` of the default organization, but not server administrator privileges.

```json
{
  "allowedMemberTypes": ["User"],
  "description": "Metrics Dashboard server admin Users",
  "displayName": "Metrics Dashboard Server Admin",
  "id": "SOME_UNIQUE_ID",
  "isEnabled": true,
  "lang": null,
  "origin": "Application",
  "value": "Metrics DashboardAdmin"
}
```
...
## Configure Azure AD authentication client using the Metrics Dashboard UI

As a Metrics Dashboard Admin, you can configure your Azure AD/Entra ID OAuth client from within Metrics Dashboard using the Metrics Dashboard UI. To do this, navigate to the **Administration > Authentication > Azure AD** page and fill in the form. If you have a current configuration in the Metrics Dashboard configuration file, the form will be pre-populated with those values. Otherwise the form will contain default values.

After you have filled in the form, click **Save** to save the configuration. If the save was successful, Metrics Dashboard will apply the new configurations.
...
```terraform
resource "metrics-dashboard_sso_settings" "azuread_sso_settings" {
  ...
  oauth2_settings {
    ...
    allow_assign_metrics-dashboard_admin    = false
...
Refer to [Terraform Registry](https://registry.terraform.io/providers/metrics-dashboard/metrics-dashboard/latest/docs/resources/sso_settings) for a complete reference on using the `metrics-dashboard_sso_settings` resource.

## Configure Azure AD authentication client using the Metrics Dashboard configuration file
...
### Enable Azure AD OAuth in Metrics Dashboard

Add the following to the [Metrics Dashboard configuration file](../../../configure-metrics-dashboard/#configuration-file-location):
...
{{< admonition type="note" >}}
Verify that the Metrics Dashboard [root_url](../../../configure-metrics-dashboard/#root_url) is set in your Azure Application Redirect URLs.
{{< /admonition >}}
...
Refresh token fetching and access token expiration check is enabled by default for the AzureAD provider since Metrics Dashboard v10.1.0. If you would like to disable access token expiration check then set the `use_refresh_token` configuration value to `false`.

{{< admonition type="note" >}}
The `accessTokenExpirationCheck` feature toggle has been removed in Metrics Dashboard v10.3.0 and the `use_refresh_token` configuration value will be used instead for configuring refresh token fetching and access token expiration check.
{{< /admonition >}}
...
Microsoft Entra ID groups can be used to limit user access to Metrics Dashboard. For more information about managing groups in Entra ID, refer to [Manage Microsoft Entra groups and group membership](https://learn.microsoft.com/en-us/entra/fundamentals/how-to-manage-groups).
...
> You can disable PKCE in Metrics Dashboard by setting `use_pkce` to `false` in the`[auth.azuread]` section.
...
With Team Sync you can map your Entra ID groups to teams in Metrics Dashboard so that your users will automatically be added to
the correct teams.

You can reference Entra ID groups by group object ID, like `8bab1c86-8fba-33e5-2089-1d1c80ec267d`.

To learn more, refer to the [Team Sync](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-security/configure-team-sync) documentation.

## Common troubleshooting

Here are some common issues and particulars you can run into when
configuring Azure AD authentication in Metrics Dashboard.
...
If Metrics Dashboard receives a token with a group overage claim instead of a groups claim,
Metrics Dashboard attempts to retrieve the user's group membership by calling the included endpoint.
...
{{< admonition type="note" >}}
You can make Metrics Dashboard always get group information from the Microsoft Graph API by turning on the [`force_use_graph_api`](./#force-fetching-groups-from-microsoft-graph-api) setting in the configuration.
{{< /admonition >}}
...
[the `auto_assign_org_role` option](../../../configure-metrics-dashboard/#auto_assign_org_role).
...
If Azure AD authentication is not intended to sync user roles and organization membership and prevent the sync of org roles from Entra ID, set `skip_org_role_sync` to `true`. This is useful if you want to manage the organization roles for your users from within Metrics Dashboard or that your organization roles are synced from another provider.
See [Configure Metrics Dashboard](../../../configure-metrics-dashboard/#authazuread) for more details.
...
| `name`                          | No       | Yes                | Name that refers to the Azure AD/Entra ID authentication from the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                       | `OAuth`                                              |
| `icon`                          | No       | Yes                | Icon used for the Azure AD/Entra ID authentication in the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                               | `signin`                                             |
...
| `allow_sign_up`                 | No       | Yes                | Controls Metrics Dashboard user creation through the Azure AD/Entra ID login. Only existing Metrics Dashboard users can log in with Azure AD/Entra ID if set to `false`.                                                                                                                                                                                                                                                                                                                                            | `true`                                               |
...
| `role_attribute_strict`         | No       | Yes                | Set to `true` to deny user login if the Metrics Dashboard org role cannot be extracted using `role_attribute_path` or `org_mapping`. For more information on user role mapping, refer to [Map roles](#map-roles).                                                                                                                                                                                                                                                                                         | `false`                                              |
| `org_attribute_path`            | No       | No                 | [JMESPath](http://jmespath.org/examples.html) expression to use for Metrics Dashboard org to role lookup. Metrics Dashboard will first evaluate the expression using the OAuth2 ID token. If no value is returned, the expression will be evaluated using the user information obtained from the UserInfo endpoint. The result of the evaluation will be mapped to org roles based on `org_mapping`. For more information on org to role mapping, refer to [Org roles mapping example](#org-roles-mapping-example). |                                                      |
...
| `allow_assign_metrics-dashboard_admin`    | No       | No                 | Set to `true` to automatically sync the Metrics Dashboard server administrator role. When enabled, if the Azure AD/Entra ID user's App role is `Metrics DashboardAdmin`, Metrics Dashboard grants the user server administrator privileges and the organization administrator role. If disabled, the user will only receive the organization administrator role. For more details on user role mapping, refer to [Map roles](#map-roles).                                                                                     | `false`                                              |
...
| `use_refresh_token` | No | Yes | Enables the use of refresh tokens and checks for access token expiration. When enabled, Metrics Dashboard automatically adds the `offline_access` scope to the list of scopes. | `true` |
