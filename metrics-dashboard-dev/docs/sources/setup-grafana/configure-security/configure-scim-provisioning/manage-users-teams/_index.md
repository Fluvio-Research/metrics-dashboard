---
description: Learn how to implement SCIM provisioning in Metrics Dashboard for automated user and team synchronization. SCIM integrates with identity providers like Okta and Azure AD to streamline user management, automate team provisioning, and replace Team Sync.
keywords:
  - metrics-dashboard
  - scim
  - provisioning
...
SCIM streamlines identity management in Metrics Dashboard by automating user lifecycle and team membership operations. This guide explains how SCIM works with existing Metrics Dashboard setups, handles user provisioning, and manages team synchronization.
...
SCIM provisioning works in conjunction with existing user management methods in Metrics Dashboard. While SCIM automates user provisioning from the identity provider, users can still be created through SAML just-in-time provisioning when they log in, manually through the Metrics Dashboard UI, or via automation tools like Terraform and the Metrics Dashboard API. For the most consistent user management experience, we recommend centralizing user provisioning through SCIM.

{{< admonition type="note" >}}
User provisioning requires `user_sync_enabled = true` in the SCIM configuration. See [Configure SCIM in Metrics Dashboard](../../configure-scim-provisioning#configure-scim-in-metrics-dashboard) for more information.
{{< /admonition >}}

{{< admonition type="warning" >}}
After a user is provisioned through SCIM, they cannot be deleted from Metrics Dashboard - they can only be deactivated through the identity provider. This is important to consider when planning your user management strategy, especially for compliance and data retention requirements.
{{< /admonition >}}
...
SCIM uses a specific process to establish and maintain user identity between the identity provider and Metrics Dashboard:

1. **Initial user lookup:**
   - The administrator configures SCIM at the Identity Provider, defining the **Unique identifier field**
   - The identity provider looks up each user in Metrics Dashboard using this unique identifier field as a filter
   - The identity provider expects a single result from Metrics Dashboard for each user

2. **Identity linking based on lookup results:**
   - **If there's a single matching result:** The identity provider retrieves the user's unique ID at Metrics Dashboard, saves it, confirms it can fetch the user's information, and updates the user's information in Metrics Dashboard
   - **If there are no matching results:** The identity provider attempts to create the user in Metrics Dashboard. If successful, it retrieves and saves the user's unique ID for future operations. If a user with the same email address already exists in Metrics Dashboard, the user is updated and will be managed by SCIM from that point forward.
   - The identity provider learns the relationship between the found Metrics Dashboard user and the Metrics Dashboard internal ID
   - The identity provider updates Metrics Dashboard with the External ID
   - Metrics Dashboard updates the authentication validations to expect this External ID

3. **Matching the User During Login:**
   When a user logs in via SAML, Metrics Dashboard needs to securely match them to the correct user account provisioned by SCIM. This requires using a consistent, unique identifier across both processes (for example, the user's `objectId` in Azure AD).
   ...
   - **Configure Metrics Dashboard SAML:** In the Metrics Dashboard SAML settings, use the `assertion_attribute_login` setting to specify which incoming SAML attribute contains this unique identifier.
   - **Configure SCIM Mapping:** To complete the link, ensure your SCIM attribute mapping in the identity provider sets the user's Metrics Dashboard **externalId** attribute to be the _same_ unique identifier provided via SAML (for example, the user's `objectId` in Azure AD).
...
### Existing Metrics Dashboard users

For users who already exist in the Metrics Dashboard instance:
...
For users who don't yet exist in Metrics Dashboard:

- SCIM creates accounts when users are assigned to Metrics Dashboard in the identity provider
- Sets up initial access based on identity provider group memberships and SAML role mapping
- No manual Metrics Dashboard account creation needed
...
If you have an existing Metrics Dashboard instance with manually created users and want to migrate to IDP-based SCIM provisioning, you can leverage the SCIM identification mechanism to seamlessly link existing users with their IDP identities.

### Migration overview

The migration process uses the same [user identification mechanism](#how-scim-identifies-users) described earlier, but focuses on linking existing Metrics Dashboard users with their corresponding IDP identities rather than creating new users.
...
1. **Prepare the identity provider:**
   - Ensure all existing Metrics Dashboard users have corresponding accounts in your IDP
...
2. **Configure SCIM in Metrics Dashboard:**
   - Set up SCIM endpoint and authentication as described in [Configure SCIM in Metrics Dashboard](../../configure-scim-provisioning#configure-scim-in-metrics-dashboard)
...
3. **Test the matching mechanism:**
   ...
   ```bash
   curl --location 'https://{$METRICS_DASHBOARD_URL}/apis/scim.metrics-dashboard.app/v0alpha1/namespaces/{$STACK_ID}/Users?filter=userName eq "existing.user@company.com"' \
...
4. **Assign users in the IDP:**
   - Begin assigning existing users to the Metrics Dashboard application in your IDP
   - The SCIM identification process will automatically link existing Metrics Dashboard users with their IDP identities
   - Monitor the process for any conflicts or errors

5. **Verify the migration:**
   - Check that users can still access Metrics Dashboard with their existing permissions
...
- **Backup your Metrics Dashboard database** - Always have a recovery plan
...
- **Monitor logs** - Watch for SCIM errors or conflicts during the linking process in Metrics Dashboard and your Identity Provider
...
- **Regular audits** - Periodically verify that IDP and Metrics Dashboard users remain in sync
...
- Check for duplicate accounts in Metrics Dashboard or the IDP
...
- Check that the user exists in both Metrics Dashboard and the IDP
...
SCIM provides automated team management capabilities that go beyond what Team Sync offers. While Team Sync only maps identity provider groups to existing Metrics Dashboard teams, SCIM can automatically create and delete teams based on group changes in the identity provider.

{{< admonition type="note" >}}
Team provisioning requires `group_sync_enabled = true` in the SCIM configuration. See [Configure SCIM in Metrics Dashboard](../../configure-scim-provisioning#configure-scim-in-metrics-dashboard) for more information.
{{< /admonition >}}

{{< admonition type="warning" >}}
Teams provisioned through SCIM cannot be deleted manually from Metrics Dashboard - they can only be deleted by removing their corresponding groups from the identity provider.
{{< /admonition >}}
...
- **Automatic team creation:** SCIM automatically creates Metrics Dashboard teams when new groups are added to the identity provider
...
- **Simplified management:** No need to manually create teams in Metrics Dashboard before mapping them
...
- Creates Metrics Dashboard teams for new identity provider groups
...
```bash
https://{$METRICS_DASHBOARD_URL}/apis/scim.metrics-dashboard.app/v0alpha1/namespaces/{$STACK_ID}/Users
```

Where:

- `{$METRICS_DASHBOARD_URL}` is your Metrics Dashboard URL (subdomain format)
- `{$STACK_ID}` is your Metrics Dashboard stack ID:
  - **Metrics Dashboard Cloud:** Format like `stack-123` (found in your Metrics Dashboard Cloud dashboard)
...
2. **Verify permissions:** Ensure the service account has `Editor` or `Admin` role in the Metrics Dashboard instance

#### Error: "HTTP 401 Unauthorized"

**Cause:** Invalid or expired authentication token.

**Solution:** Generate a new token from the Service Account details page in Metrics Dashboard.
