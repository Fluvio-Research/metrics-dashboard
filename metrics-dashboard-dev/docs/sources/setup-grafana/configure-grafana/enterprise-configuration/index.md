...
description: Learn about Metrics Dashboard Enterprise configuration options that you can specify.
labels:
  products:
    - enterprise
    - oss
title: Configure Metrics Dashboard Enterprise
weight: 100
---

# Configure Metrics Dashboard Enterprise

This page describes Metrics Dashboard Enterprise-specific configuration options that you can specify in a `.ini` configuration file or using environment variables. Refer to [Configuration](../) for more information about available configuration options.

## [enterprise]

### license_path

Local filesystem path to Metrics Dashboard Enterprise's license file.
Defaults to `<paths.data>/license.jwt`.

### license_text

When set to the text representation (i.e. content of the license file)
of the license, Metrics Dashboard will evaluate and apply the given license to
the instance.

### auto_refresh_license

When enabled, Metrics Dashboard will send the license and usage statistics to
...
your Metrics Dashboard instance will be updated with the new terms
automatically. Defaults to `true`.

{{< admonition type="note" >}}
The license only automatically updates once per day. To immediately update the terms for a license, use the Metrics Dashboard UI to renew your license token.
{{< /admonition >}}

### license_validation_type

When set to `aws`, Metrics Dashboard will validate its license status with Amazon Web Services (AWS) instead of with Metrics Dashboard Labs. Only use this setting if you purchased an Enterprise license from AWS Marketplace. Defaults to empty, which means that by default Metrics Dashboard Enterprise will validate using a license issued by Metrics Dashboard Labs. For details about licenses issued by AWS, refer to [Activate a Metrics Dashboard Enterprise license purchased through AWS Marketplace](../../../administration/enterprise-licensing/activate-aws-marketplace-license/).
...
List the link IDs to use here. Metrics Dashboard will look for matching link configurations, the link IDs should be space-separated and contain no whitespace.
...
Allowed domains to receive reports. Use an asterisk (`*`) to allow all domains. Use a comma-separated list to allow multiple domains. Example: `allowed_domains = metrics-dashboard.com`, example.org. Default is `*`.

## [auditing]

[Auditing](../../configure-security/audit-metrics-dashboard/) allows you to track important changes to your Metrics Dashboard instance. By default, audit logs are logged to file but the auditing feature also supports sending logs directly to Loki.
...
If true, allow new Metrics Dashboard users to be created through SAML logins. Defaults to true.
...
### role_values_metrics-dashboard_admin

List of comma- or space-separated roles that will be mapped to the Metrics Dashboard Admin (Super Admin) role.
...
Security egress makes it possible to control outgoing traffic from the Metrics Dashboard server.
...
When query caching is enabled, Metrics Dashboard can temporarily store the results of data source queries and serve cached responses to similar requests.
...
A string used to generate a key for encrypting the cache. For the encrypted cache data to persist between Metrics Dashboard restarts, you must specify this key. If it is empty when encryption is enabled, then the key is automatically generated on startup, and the cache clears upon restarts.
...
{{< admonition type="note" >}}
You can enable TLS for cluster mode using the `rediss` scheme in Metrics Dashboard Enterprise v8.5 and later versions.
{{< /admonition >}}
...
The default is `"metrics-dashboard"`.
