---
aliases:
  - ../../manage-notifications/template-notifications/reference/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/manage-notifications/template-notifications/reference/
canonical: https://metrics-dashboard.com/docs/metrics-dashboard/latest/alerting/configure-notifications/template-notifications/reference/
...
refs:
  label-types:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/fundamentals/alert-rules/annotation-label/#label-types
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/alerting-and-irm/alerting/fundamentals/alert-rules/annotation-label/#label-types
  ...
  template-language-functions:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/configure-notifications/template-notifications/language/#functions
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/alerting-and-irm/alerting/configure-notifications/template-notifications/language/#functions
---

# Notification template reference

By default, Metrics Dashboard provides predefined templates to format notification messages.
...
| `ExternalURL`       | string            | A link to Metrics Dashboard, or the Alertmanager that sent this notification if using an external Alertmanager.   |
...
| `GeneratorURL` | string        | A link to Metrics Dashboard, or the source of the alert if using an external alert generator.                                                                 |
| `Fingerprint`  | string        | A unique string that identifies the alert.                                                                                                          |

Metrics Dashboard-managed alerts include these additional properties:

| Name           | Type      | Description                                                                                                                                          |
| -------------- | --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DashboardURL` | string    | A link to the Metrics Dashboard Dashboard if the alert has a Dashboard UID annotation, with time range from `1h` before alert start to end (or now if firing). |
...
{{ range .Alerts }}
  ...
  {{/* Only available for Metrics Dashboard-managed alerts */}}
  {{ .DashboardURL }}
  {{ .PanelURL }}
  {{ .SilenceURL }}
  {{ .Values }}
...
```go
{{ define "custom_template" }}
  {{ .CommonLabels.metrics-dashboard_folder }}
{{ end }}
```
...
{{ define "custom_template" }}
  ...
  {{ .CommonLabels.Names }}
  {{ .CommonLabels.Values }}
  {{ .CommonLabels.Remove (stringSlice "metrics-dashboard_folder") }}
{{ end }}
```
...
Namespaced Functions are not yet [generally available](https://metrics-dashboard.com/docs/release-life-cycle/#general-availability) in Metrics Dashboard Cloud.
