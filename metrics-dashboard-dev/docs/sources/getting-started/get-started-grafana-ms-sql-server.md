...
aliases:
  ...
  - getting-started-sql/
description: Learn how to build your first MS SQL Server dashboard in Metrics Dashboard.
labels:
  products:
    - enterprise
    - oss
title: Get started with Metrics Dashboard and MS SQL Server
weight: 500
---

# Get started with Metrics Dashboard and MS SQL Server

Microsoft SQL Server is a popular relational database management system that is widely used in development and production environments. This topic walks you through the steps to create a series of dashboards in Metrics Dashboard to display metrics from a MS SQL Server database.
...
You can install MS SQL Server on the host running Metrics Dashboard or on a remote server. To install the software from the [downloads page](https://www.microsoft.com/en-us/sql-server/sql-server-downloads), follow their setup prompts.

If you are on a Windows host but want to use Metrics Dashboard and MS SQL data source on a Linux environment, refer to the [WSL to set up your Metrics Dashboard development environment](/blog/2021/03/03/how-to-set-up-a-metrics-dashboard-development-environment-on-a-windows-pc-using-wsl) blog post. This will allow you to leverage the resources available in [metrics-dashboard/metrics-dashboard](https://github.com/metrics-dashboard/metrics-dashboard) GitHub repository. Here you will find a collection of supported data sources, including MS SQL Server, along with test data and pre-configured dashboards for use.
...
Windows Active Directory (Kerberos) authentication is not supported in Metrics Dashboard Cloud at the moment.
{{< /admonition >}}
...
To fetch data from your own instance of MS SQL Server, add the data source using instructions in Step 4 of this topic. In Metrics Dashboard [Explore](../../explore/) build queries to experiment with the metrics you want to monitor.

Once you have a curated list of queries, create [dashboards](../../dashboards/) to render metrics from the SQL Server database. For troubleshooting, user permissions, known issues, and query examples, refer to [Using Microsoft SQL Server in Metrics Dashboard](../../datasources/mssql/).
