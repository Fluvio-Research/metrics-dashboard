...
labels:
  products:
    ...
    - enterprise
    - oss
description: Configure options for Metrics Dashboard's logs visualization
title: Logs
weight: 100
refs:
  log-levels:
...
![Logs visualization](/media/docs/metrics-dashboard/panels-visualizations/screenshot-logs-v11.3.png)

{{< docs/play title="Logs Panel" url="https://play.metrics-dashboard.org/d/6NmftOxZz/" >}}
...
![Logs Example](/media/docs/metrics-dashboard/panels-visualizations/screenshot-metrics-dashboard-12.1-logs-example.png 'Logs Example')

## Log level

For logs where a **level** label is specified, we use the value of the label to determine the log level and update color accordingly. If the log doesn't have a level label specified, we try to find out if its content matches any of the supported expressions (see below for more information). The log level is always determined by the first match. In case Metrics Dashboard is not able to determine a log level, it will be visualized with **unknown** log level. See [supported log levels and mappings of log level abbreviation and expressions](ref:log-levels).
