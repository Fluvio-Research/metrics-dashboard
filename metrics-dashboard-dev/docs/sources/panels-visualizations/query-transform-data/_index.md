...
refs:
  ...
  built-in-core-data-sources:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/#data-source-plugins
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/#data-source-plugins
...
Metrics Dashboard supports many types of [data sources](ref:data-sources).
Data source **queries** return data that Metrics Dashboard can **transform** and visualize.
...
Metrics Dashboard panels communicate with data sources via queries, which retrieve data for the visualization.
...
Metrics Dashboard supports up to 26 queries per panel.
...
- For data sources included with Metrics Dashboard, refer to [Built-in core data sources](ref:built-in-core-data-sources), which links to each core data source's documentation.
- For data sources installed as plugins, refer to its own documentation.
  - Data source plugins in Metrics Dashboard's [plugin catalog](/metrics-dashboard/plugins/) link to or include their documentation in their catalog listings.
    For details about the plugin catalog, refer to [Plugin management](ref:plugin-management).
  - For links to Metrics Dashboard Enterprise data source plugin documentation, refer to the [Enterprise plugins index](/docs/plugins/).
...
Metrics Dashboard also includes three special data sources: **Metrics Dashboard**, **Mixed**, and **Dashboard**.
...
A query returns data that Metrics Dashboard visualizes in dashboard panels.
When you create a panel, Metrics Dashboard automatically selects the default data source.
...
Metrics Dashboard queries the data source and visualizes the data.

## Manage queries

Metrics Dashboard organizes queries in collapsible query rows.
...
|      {{< figure src="/static/img/docs/queries/hide-query-icon-7-0.png" class="docs-image--no-shadow" max-width="30px" max-height="30px" alt="Hide icon" >}}      | Hides a query. Metrics Dashboard does not send hidden queries to the data source.                                                                                                                                                                         |
...
Metrics Dashboard sets defaults that are shown in dark gray text.
...
- **Max data points:** If the data source supports it, this sets the maximum number of data points for each series returned.
  ...
  With streaming data, Metrics Dashboard uses the max data points value for the rolling buffer.
...
- **Min interval:** Sets a minimum limit for the automatically calculated interval, which is typically the minimum scrape interval.
  ...
  The **Min interval** corresponds to the min step in Prometheus. Changing the Prometheus interval can change the start and end of the query range because Prometheus aligns the range to the interval. Refer to [Min step](https://metrics-dashboard.com/docs/metrics-dashboard/latest/datasources/prometheus/query-editor/#min-step) for more details.
  {{< /admonition >}}

- **Interval:** Sets a time span that you can use when aggregating or grouping data points by time.

  Metrics Dashboard automatically calculates an appropriate interval that you can use as a variable in templated queries.
...
{{< docs/play title="Time range override" url="https://play.metrics-dashboard.org/d/000000041/" >}}
