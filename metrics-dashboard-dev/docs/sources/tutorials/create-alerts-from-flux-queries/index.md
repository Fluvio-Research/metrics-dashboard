...
categories:
  - alerting
description: Create complex alerts from Flux queries in the new Metrics Dashboard Alerting
id: metrics-dashboard-alerts-flux-queries
labels:
  products:
    - enterprise
    - oss
status: published
summary: Create complex alerts from Flux queries in the new Metrics Dashboard Alerting
tags:
  - advanced
title: How to create Metrics Dashboard alerts with InfluxDB and the Flux query language
weight: 70
---

# How to create Metrics Dashboard alerts with InfluxDB and the Flux query language

[Metrics Dashboard Alerting](/docs/metrics-dashboard/latest/alerting/) represents a powerful new approach to systems observability and incident response management. While the alerting platform is perhaps best known for its strong integrations with Prometheus, the system works with numerous popular data sources including InfluxDB. In this tutorial we will learn how to create Metrics Dashboard alerts using InfluxDB and the newer Flux query language. We will cover five common scenarios from the most basic to the most complex. Together, these five scenarios will provide an excellent guide for almost any type of alerting query that you wish to create using Metrics Dashboard and Flux.
...
In the following five examples we will see just how powerful and flexible the new Flux query language can be. We will also see just how well Flux pairs with Metrics Dashboard Alerting.

## Example 1: Create an alert when a value is above or below a set threshold

Our first example uses a common real-world scenario for InfluxDB and Metrics Dashboard Alerting. Popular with IoT and edge applications, InfluxDB excels at on-site, real-time observability. In this example, and in fact for many of the following examples, we will consider the hypothetical scenario where we are monitoring a number of fluid tanks in a manufacturing plant. This scenario, [based on an actual application of InfluxDB and Alerting](/go/metrics-dashboardconline/2021/plant-efficiency-metrics-dashboard-cloud/), will allow us to work through Metrics Dashboard's various alerting setups, progressing from the simplest to the most complex.
...
We want to write a Metrics Dashboard alert that will trigger whenever the temperature in tank `A5` crosses the lower threshold of 30 °C or the upper threshold of 60 °C.

To do this, we'll: create a Metrics Dashboard alert rule, add a Flux query, and then add expressions to the alert rule.

### Create a Metrics Dashboard Alert rule

1. Open the Metrics Dashboard alerting menu and select **Alert rules**.
1. Click **New alert rule**.
1. Give your alert rule a name and then select **Metrics Dashboard managed alert**.
   For InfluxDB, you will always create a [Metrics Dashboard managed rule](/docs/metrics-dashboard/latest/alerting/alerting-rules/create-metrics-dashboard-managed-rule/#add-metrics-dashboard-managed-rule).
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-timeseries-graph.png)

### Add expressions to your Metrics Dashboard Alert rule

With data now appearing in our rule setup, our next step is to create an [expression](/docs/metrics-dashboard/v9.0/panels/query-a-data-source/use-expressions-to-manipulate-data/about-expressions/#using-expressions). Move to section `B`. For this scenario, we want to create a Reduce expression that will reduce the above to a single value. In this image, you can see that we have chosen to reduce our time-series data the `Last` value from input `A`. In this case, it returns a value 53 degrees celsius for Tank A5:

![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-reduce-expression.png)

Finally, we need to create a math expression that Metrics Dashboard will alert on. In our case we will write an expression with two conditions separated by the OR `||` operator. We want to trigger an alert any time our result in section `B` is less than 30 or more than 60. This looks like `$B < 30 || $B > 60`:

![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-math-expression.png)
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-alert-preview-state-normal.png)

And here is a preview of this alert when the state is `Alerting`:

![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-alert-alert-preview-state-alerting.png)

Note that the Reduce expression above is needed. Without it, when previewing the results, Metrics Dashboard would display `invalid format of evaluation results for the alert definition B: looks like time series data, only reduced data can be alerted on`.
...
Using these three steps you can create a Flux-based Metrics Dashboard Alert that will trigger on either of two thresholds from a single data source. But what if you need to trigger an alert based on **multiple conditions and from multiple time-series**? In example two we will cover this very scenario.

## Example 2: how to create a Metrics Dashboard alert from two queries and two conditions
...
Let's assume we are tracking this data in InfluxDB and Metrics Dashboard. Let's also assume that each of the above data sources comes from different buckets. How do we alert on this? How do we use Metrics Dashboard and Flux to alert on two distinct conditions originating from two distinct data sources?

### Add two Flux queries to your Metrics Dashboard Alert rule
...
1. Let's now use the same steps to reduce each query to the last (most recent) value. Reducing Query `A` to a single value might look like this:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-additional-queries-reduce-expression-A.png)

1. And here we are reducing query `B`:

   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-additional-queries-reduce-expression-B.png)
...
1. Now, in section `C` we need to create a math expression to be alerted on. In this case we will use the AND `&&` operator to specify that two conditions must be met: the value of `C` (the reduced value from query `A`) must be greater than 88.0 while the value of `D` (the reduced value from query `B`) must be greater than 1.21. We write this as `$C > 88.0 && $D > 1.21`
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-additional-queries-math-expression.png)

And here is a preview of our alerts:

![metrics-dashboard alerts from flux queries](https://raw.githubusercontent.com/metrics-dashboard/tutorials/master/content/tutorials/assets/flux-additional-queries-alert-preview.png)

💡Tip: If your data in InfluxDB happens to have an unnecessarily large number of digits to the right of the decimal (such as 1.2104705741732575 shown above), and you want your Metrics Dashboard alerts to be more legible, try using {{ printf "%.2f"  $values.D.Value }}. For example, in the annotation Summary, we could write the following:
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-tip-significant-figures.png))

You can reference our documentation on [alert message templating](/docs/metrics-dashboard/latest/alerting/contact-points/message-templating/) to learn more about this powerful feature.
...
## Example 3: how to create a Metrics Dashboard Alert based on an aggregated (per-day) value

One of the most common requests in [Metrics Dashboard's community forum](https://community.metrics-dashboard.com) involves graphing daily electrical consumption and production. This sort of data is very often stored in InfluxDB. In this example we will see how to aggregate time series data into a per-day value and then alert on it.
...
### Add an initial Flux query to your Metrics Dashboard Alert rule
...
1. Let's begin by examining a typical query and the resulting time graph for our hourly data across a 7-day period. A query like this is shown below:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-aggregatewindow-timeseries-graph.png)
...
1. Now let's adjust our query to calculate daily usage. With many datasources, this can be a rather complex operation. But with Flux, by simply changing the aggregateWindow function parameters we can calculate the daily usage over the same 7-day period:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-aggregatewindow-aggregated.png)

1. Add expressions to your Metrics Dashboard Alert rule.
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-aggregatewindow-reduce-expression.png)

   Now create a math expression to be alerted on and set the evaluation behavior. In this case we want to write `$B > 5000`:

   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-aggregatewindow-math-expression.png)

   And now we are alerting on our daily electricity consumption whenever we exceed 5000 kWh. Here is preview of our alert:

   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-aggregatewindow-alert-preview.png)

### Conclusion

Plotting and aggregating electrical consumption is a common use case for combining InfluxDB and Metrics Dashboard. Using Flux, we saw just how easy it can be to group our data by day and then alert on that daily value. In our next two examples we will examine the more complex form of Metrics Dashboard Alert: multidimensional alerts.

## Example 4: create a dynamic (multidimensional) Metrics Dashboard Alert using Flux
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-multidimensional-timeseries-graph.png)
...
1. We create a Reduce expression that will reduce the time series for each tank to a single value. This gives us five distinct temperatures:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-multidimensional-reduce-expression.png))

1. Create a math expression to be alerted on. This is the exact same expression from example 1, `$B < 30 || $B > 60`:

   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-multidimensional-math-expression.png)
...
## Example 5: how to create a dynamic (multidimensional) Metrics Dashboard Alert using multiple queries and multiple thresholds with Flux
...
Note in the above that we are calculating the difference between the actual and the setpoint. The way Metrics Dashboard parses the result from InfluxDB is that if a \_value column is found, it is assumed to be a time-series. The quick workaround is to add the following `rename()` function:
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-complex-query-timeseries-graph.png)
...
1. Again, we create a Reduce expression for the above query to reduce each of the above to a single value. This value represents the temperature differential between each tank's setpoint and its actual real-time temperature:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-complex-query-reduce-expression.png)
...
1. Now we create a math expression to be alerted on. This time we will create a condition that checks if the absolute value of our reduce calculation is greater than 3, `abs($(B))>3.0`:
   ...
   ![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-complex-query-math-expression.png)
...
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-complex-query-alert-preview-state-normal.png)
![metrics-dashboard alerts from flux queries](/media/tutorials/screenshot-flux-complex-query-alert-preview-state-alerting.png)

### Conclusion

Flux queries and Metrics Dashboard Unified Alerting are a powerful combination to identify practically any alertable conditions in your dataset, or across your entire system. For more information on Metrics Dashboard Alerting, [visit the documentation here](/docs/metrics-dashboard/latest/alerting/). For more information on the Flux query language, [you can visit that documentation as well](https://docs.influxdata.com/flux/v0.x/).
