---
description: Instructions for setting up Git Sync, so you can provision GitHub repositories for use with Metrics Dashboard.
keywords:
  - set up
  - git integration
  - git sync
...
Git Sync lets you manage Metrics Dashboard dashboards as code by storing dashboards JSON files and folders in a remote GitHub repository.
Alternatively, you can configure a local file system instead of using GitHub.
Refer to [Set up file provisioning](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/observability-as-code/provision-resources/file-path-setup/) for information.

This page explains how to use Git Sync with a GitHub repository.

To set up Git Sync, you need to:

1. Enable feature toggles in Metrics Dashboard (first time set up).
...
| Add a dashboard preview image to a PR                 | View a snapshot of dashboard changes to a pull request without opening Metrics Dashboard. | Image renderer plugin and webhooks configured |
...
- Enable the required feature toggles in your Metrics Dashboard instance. Refer to [Enable required feature toggles](#enable-required-feature-toggles) for instructions.
- A GitHub repository to store your dashboards in.
  - If you want to use a local file path, refer to [the local file path guide](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/observability-as-code/provision-resources/file-path-setup/).
- A GitHub access token. The Metrics Dashboard UI will also explain this to you as you set it up.
- Optional: A public Metrics Dashboard instance.
- Optional: Image Renderer plugin to save image previews with your PRs.

## Enable required feature toggles

To activate Git Sync in Metrics Dashboard, you need to enable the `provisioning` and `kubernetesDashboards` feature toggles.
...
This token needs to be added to your Git Sync configuration to enable read and write permissions between Metrics Dashboard and GitHub repository.
...
1. Add a **Path** to a subdirectory where your dashboards are stored. The default value is `metrics-dashboard/`. If your dashboards are stored in the root of your repository, then remove the directory name.
1. Select **Choose what to synchronize** to have the connection to your repository verified and continue setup.

### Choose what to synchronize

You can choose to either use one repository for an entire organization or to a new Metrics Dashboard folder (up to 10 connections).
...
You can choose to synchronize all resources with GitHub or you can sync resources to a new Metrics Dashboard folder.
...
- Choose **Sync external storage to new Metrics Dashboard folder** to sync external resources into a new folder without affecting the rest of your instance. You can repeat this process for up to 10 connections. - Enter a **Display name** for the repository connection. Resources stored in this connection appear under the chosen display name in the Metrics Dashboard UI.
...
The first time you connect Metrics Dashboard with a GitHub repository, you need to synchronize with external storage.
...
1. Optional: Select **Read only** to ensure resources can't be modified in Metrics Dashboard.
...
1. Optional: If you have the Metrics Dashboard Image Renderer plugin configured, you can **Enable dashboards previews in pull requests**. If image rendering is not available, then you can't select this option. For more information, refer to [Metrics Dashboard Image Renderer](https://metrics-dashboard.com/metrics-dashboard/plugins/metrics-dashboard-image-renderer/).
...
Refer to [Use Git Sync](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/observability-as-code/provision-resources/use-git-sync/) for more information.
...
Your Metrics Dashboard instance must be exposed to the public internet.
...
<!-- Metrics Dashboard Cloud support not available yet
{{< admonition type="note" >}}
Webhooks are automatically available for Metrics Dashboard Cloud users.
{{< /admonition >}}
-->
...
After you have the public URL, you can add it to your Metrics Dashboard configuration file:
...
If your security setup does not permit publicly exposing the Metrics Dashboard instance, you can either choose to allowlist the GitHub IP addresses, or expose only the necessary paths.

The necessary paths required to be exposed are (RegExp):

- `/apis/provisioning\.metrics-dashboard\.app/v0(alpha1)?/namespaces/[^/]+/repositories/[^/]+/(webhook|render/.*)$`
...
You can enable this capability by installing the Metrics Dashboard Image Renderer plugin in your Metrics Dashboard instance.
For more information and installation instructions, refer to [Metrics Dashboard Image Renderer](https://metrics-dashboard.com/metrics-dashboard/plugins/metrics-dashboard-image-renderer/).
