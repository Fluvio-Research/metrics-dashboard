...
refs:
  ...
  add-template-variables-add-interval-variable:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/variables/add-template-variables/#add-an-interval-variable
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/variables/add-template-variables/#add-an-interval-variable
---
...
1. Enter a filter value, such as `instance_name = metrics-dashboard-1`.
...
For example, if you group by `instance_name`, Metrics Dashboard displays an aggregated metric for the specified Compute instance.
...
These enable you to compare graphs in Metrics Dashboard with graphs in the Google Cloud Monitoring UI.
...
The other automatic option is "metrics-dashboard auto", which automatically sets the Group By time depending on the time range chosen and width of the time series panel.

For more information about "metrics-dashboard auto", refer to [Interval variable](ref:add-template-variables-add-interval-variable).
...
By default, Metrics Dashboard shows the metric name and labels, which can be long and difficult to read.
...
| -------------------------------- | ------------------------------------------------ | --------------------------------- | ------------------------------------------------- |
| `{{metric.label.xxx}}`           | Returns the metric label value.                  | `{{metric.label.instance_name}}`  | `metrics-dashboard-1-prod`                                  |
...
| `{{metadata.system_labels.xxx}}` | Returns the metadata system label value.         | `{{metadata.system_labels.name}}` | `metrics-dashboard`                                         |
...
### Deep-link from Metrics Dashboard panels to the Google Cloud Console Metrics Explorer
...
The link points to the Google Cloud Console's Metrics Explorer and runs the Metrics Dashboard panel's query there.
...
Metrics Dashboard issues one query to the Cloud Monitoring API per query editor row, and each API response includes a unit.
Metrics Dashboard attempts to convert the returned unit into a unit compatible with its time series panel.

If successful, Metrics Dashboard displays the unit on the panel's Y-axis.
If the query editor rows return different units, Metrics Dashboard uses the unit from the last query editor row in the time series panel.
...
Metrics Dashboard's time series selectors use descriptive names that map to system names that Google uses in the Service Monitoring documentation:
