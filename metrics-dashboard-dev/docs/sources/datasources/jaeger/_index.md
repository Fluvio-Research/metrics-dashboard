...
description: Guide for using Jaeger in Metrics Dashboard
keywords:
  - metrics-dashboard
  - jaeger
  - guide
...
Metrics Dashboard ships with built-in support for Jaeger, which provides open source, end-to-end distributed tracing. This topic explains configuration and queries specific to the Jaeger data source.
...
Once you've added the data source, you can [configure it](#configure-the-data-source) so your Metrics Dashboard instance's users can create queries in the [query editor](#query-the-data-source) when the users [build dashboards](ref:build-dashboards) and use [Explore](ref:explore).
...
If you use Metrics Dashboard Cloud, open a [support ticket in the Cloud Portal](/profile/org#support) to access this feature.
{{< /admonition >}}

The **Trace to logs** setting configures the [trace to logs feature](ref:explore-trace-integration) that is available when you integrate Metrics Dashboard with Jaeger.
...
The **Trace to metrics** setting configures the [trace to metrics feature](/blog/2022/08/18/new-in-metrics-dashboard-9.1-trace-to-metrics-allows-users-to-navigate-from-a-trace-span-to-a-selected-data-source/) available when integrating Metrics Dashboard with Jaeger.
...
Once enabled, Metrics Dashboard displays the Node Graph above the trace view.
...
If the file has multiple traces, Metrics Dashboard visualizes its first trace.
...
If service dependency information is available in Jaeger, it can be visualized in Metrics Dashboard.
