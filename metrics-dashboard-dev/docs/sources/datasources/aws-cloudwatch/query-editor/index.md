...
refs:
  ...
  query-transform-data-navigate-the-query-tab:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/panels-visualizations/query-transform-data/#navigate-the-query-tab
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/panels-visualizations/query-transform-data/#navigate-the-query-tab
---
...
#### Deep-link Metrics Dashboard panels to the CloudWatch console
...
The provided link is valid for any account but displays the expected metrics only if you're logged in to the account that corresponds to the selected data source in Metrics Dashboard.
...
Metrics Dashboard constructs a SQL query based on your selections.
...
> **Alias pattern deprecation:** Since Metrics Dashboard v10.0, the alias field has been deprecated and replaced by dynamic
> labels.
> Any existing alias pattern is migrated upon upgrade to a corresponding dynamic label pattern.
> For details on this change, refer to [issue #48434](https://github.com/metrics-dashboard/metrics-dashboard/issues/48434).
