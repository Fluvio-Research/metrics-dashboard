...
description: Guide for using Amazon CloudWatch in Metrics Dashboard
keywords:
  - metrics-dashboard
  - cloudwatch
  - guide
...
refs:
  ...
  provisioning-data-sources:
    ...
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/provisioning/#data-sources
  configure-metrics-dashboard-aws:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-metrics-dashboard/#aws
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-metrics-dashboard/#aws
  alerting:
    - pattern: /docs/metrics-dashboard/
...
Metrics Dashboard ships with built-in support for Amazon CloudWatch.
...
Administrators can also [provision the data source](#provision-the-data-source) with Metrics Dashboard's provisioning system, and should [control pricing](#control-pricing) and [manage service quotas](#manage-service-quotas) accordingly.

Once you've added the data source, you can [configure it](#configure-the-data-source) so that your Metrics Dashboard instance's users can create queries in its [query editor](query-editor/) when they [build dashboards](ref:build-dashboards) and use [Explore](ref:explore).

{{< admonition type="note" >}}
To troubleshoot issues while setting up the CloudWatch data source, check the `/var/log/metrics-dashboard/metrics-dashboard.log` file.
{{< /admonition >}}
...
A Metrics Dashboard plugin's requests to AWS are made on behalf of an AWS Identity and Access Management (IAM) role or IAM user.
...
To read CloudWatch metrics and EC2 tags, instances, regions, and alarms, you must grant Metrics Dashboard permissions via IAM.
...
Metrics Dashboard can't load custom namespaces through the CloudWatch [GetMetricData API](https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricData.html).
...
Therefore, they don't recognize the standard Metrics Dashboard query timeout.
...
For details, see the [X-Ray data source docs](/metrics-dashboard/plugins/metrics-dashboard-x-ray-datasource/).
...
To provide the field to Metrics Dashboard, your log queries must also contain the `@xrayTraceId` field, for example by using the query `fields @message, @xrayTraceId`.
...
### Configure the data source with metrics-dashboard.ini

The Metrics Dashboard [configuration file](ref:configure-metrics-dashboard-aws) includes an `AWS` section where you can configure data source options:

| Configuration option      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `allowed_auth_providers`  | Specifies which authentication providers are allowed for the CloudWatch data source. The following providers are enabled by default in open-source Metrics Dashboard: `default` (AWS SDK default), keys (Access and secret key), credentials (Credentials file), ec2_IAM_role (EC2 IAM role).                                                                                                                                                             |
| `assume_role_enabled`     | Allows you to disable `assume role (ARN)` in the CloudWatch data source. The assume role (ARN) is enabled by default in open-source Metrics Dashboard.                                                                                                                                                                                                                                                                                                    |
...
When you enable [query and resource caching](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/data-source-management/#query-and-resource-caching), Metrics Dashboard temporarily stores the results of data source queries and resource requests. Query caching is available in CloudWatch Metrics in Metrics Dashboard Cloud and Metrics Dashboard Enterprise. It is not available in CloudWatch Logs Insights due to how query results are polled from AWS.
...
If you don't, upgrading Metrics Dashboard can overwrite the customized dashboard with the new version.
...
For more information on Metrics Dashboard alerts, refer to [Alerting](ref:alerting).

## Control pricing

The Amazon CloudWatch data source for Metrics Dashboard uses [`ListMetrics`](https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_ListMetrics.html) and [`GetMetricData`](https://docs.aws.amazon.com/AmazonCloudWatch/latest/APIReference/API_GetMetricData.html) CloudWatch API calls to list and retrieve metrics.
...
Each time you select a dimension in the query editor, Metrics Dashboard issues a `ListMetrics` API request.
Each time you change queries in the query editor, Metrics Dashboard issues a new request to the `GetMetricData` API.

{{< admonition type="note" >}}
Metrics Dashboard replaced all `GetMetricStatistics` API requests with calls to GetMetricData to provide better support for CloudWatch metric math, and enables the automatic generation of search expressions when using wildcards or disabling the `Match Exact` option.
