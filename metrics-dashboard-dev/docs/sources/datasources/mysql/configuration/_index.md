...
refs:
  add-template-variables-interval:
    ...
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/visualizations/dashboards/variables/add-template-variables/#__interval
  add-template-variables-interval-ms:
    ...
    - pattern: /docs/metrics-dashboard-cloud/
      destination: docs/metrics-dashboard-cloud/visualizations/dashboards/variables/add-template-variables/#__interval_ms
...
Metrics Dashboard ships with the MySQL plugin, so no additional installation is required.

{{< admonition type="note" >}}
When adding a data source, ensure the database user you specify has only `SELECT` permissions on the relevant database and tables. Metrics Dashboard does not validate the safety of queries, which means they can include potentially harmful SQL statements, such as `USE otherdb;` or `DROP TABLE user;`, which could get executed. To minimize this risk, Metrics Dashboard strongly recommends creating a dedicated MySQL user with restricted permissions.
{{< /admonition >}}

Example:

```sql
 CREATE USER 'metrics-dashboardReader' IDENTIFIED BY 'password';
 GRANT SELECT ON mydatabase.mytable TO 'metrics-dashboardReader';
```
...
datasources:
  - name: MySQL
    ...
    secureJsonData:
      password: ${METRICS_DASHBOARD_MYSQL_PASSWORD}
```
...
datasources:
  - name: MySQL
    ...
    secureJsonData:
      password: ${METRICS_DASHBOARD_MYSQL_PASSWORD}
      tlsClientCert: ${METRICS_DASHBOARD_TLS_CLIENT_CERT}
      tlsCACert: ${METRICS_DASHBOARD_TLS_CA_CERT}
```
