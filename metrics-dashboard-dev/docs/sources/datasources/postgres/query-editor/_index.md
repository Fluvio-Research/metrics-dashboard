---
description: This document describes the PostgreSQL query editor in Metrics Dashboard.
keywords:
  - metrics-dashboard
  - postgresql
  - guide
...
refs:
  ...
  templates:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/variables/#templates
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/visualizations/dashboards/variables/#templates
---

# PostgreSQL query editor

Metrics Dashboard query editors are unique for each data source.

For general information on Metrics Dashboard query editors, refer to [Query editors](ref:query-editor).

For general information on querying data sources in Metrics Dashboard, refer to [Query and transform data](ref:query-transform-data).
...
To create advanced queries, switch to **Code mode** by clicking **Code** in the upper right of the editor window. Code mode supports the auto-completion of tables, columns, SQL keywords, standard SQL functions, Metrics Dashboard template variables, and Metrics Dashboard macros. Columns cannot be completed before a table has been specified.
...
| `$__timeGroup(dateColumn,'5m', 0)`                    | Same as the `$__timeGroup(dateColumn,'5m')` macro, but includes a fill parameter to ensure missing points in the series are added by Metrics Dashboard, using 0 as the default value. **This applies only to time series queries.** |
...
Time series query results are returned in [wide data frame format](https://metrics-dashboard.com/developers/plugin-tools/key-concepts/data-frames#wide-format). In the data frame query result, any column, except for time or string-type columns, transforms into value fields. String columns, on the other hand, become field labels.
...
A query can return multiple columns, and Metrics Dashboard will automatically generate a list based on the query results. For example, the following query returns a list with values from `hostname` and `hostname2`.
...
Note that you must enclose the `__searchFilter` expression in quotes as Metrics Dashboard does not add them automatically.
...
Metrics Dashboard automatically formats multi-value variables as a quoted, comma-separated string. For example, if `server01` and `server02` are selected, they are formatted as `'server01'`, `'server02'`. To remove the quotes, enable the CSV formatting option for the variables:
