...
if echo "$_grafana_tag" | grep -q "^v" && echo "$_grafana_tag" | grep -vq "beta"; then
  ...
  # Push to the metrics-dashboard-dev repository with the expected tag
  # for running the end to end tests successfully
  docker push "grafana/metrics-dashboard-dev:${_grafana_tag}${TAG_SUFFIX}"
  ...
  docker push "grafana/metrics-dashboard-dev:${_grafana_version}${TAG_SUFFIX}"
fi
