...
enable auth proxy together with configuring Metrics Dashboard for auth proxy.
...
```bash
$ ./run.sh -u http://metrics-dashboard.loc
```
  ...
  execution: local
    ...
    █ user auth token test
      ...
      █ user authenticates through ui with username and password
        ...
        ✓ response has cookie 'metrics-dashboard_session' with 32 characters
