...
This guide helps you get started developing Metrics Dashboard.
...
If you are running Metrics Dashboard on Windows 10, we recommend installing the Windows Subsystem for Linux (WSL). For installation instructions, refer to our [Metrics Dashboard setup guide for Windows environment](https://metrics-dashboard.com/blog/2021/03/03/how-to-set-up-a-metrics-dashboard-development-environment-on-a-windows-pc-using-wsl/).

## Download Metrics Dashboard

We recommend using the Git command-line interface to download the source code for the Metrics Dashboard project:

1. Open a terminal and run `git clone https://github.com/metrics-dashboard/metrics-dashboard.git`. This command downloads Metrics Dashboard to a new `metrics-dashboard` directory in your current directory.
1. Open the `metrics-dashboard` directory in your favorite code editor.

For alternative ways of cloning the Metrics Dashboard repository, refer to [GitHub's documentation](https://docs.github.com/en/github/creating-cloning-and-archiving-repositories/cloning-a-repository).

> **Caution:** Do not use `go get` to download Metrics Dashboard. Recent versions of Go have added behavior which isn't compatible with the way the Metrics Dashboard repository is structured.
...
We use pre-commit hooks (via [lefthook](https://github.com/evilmartians/lefthook)) to lint, fix, and format code as you commit your changes. Previously, the Metrics Dashboard repository automatically installed these hook when you ran `yarn install`, but they are now opt-in for all contributors.
...
When building Metrics Dashboard, be aware that it consists of two components:
...
- azuremonitor
- cloud-monitoring
- metrics-dashboard-postgresql-datasource
- metrics-dashboard-pyroscope-datasource
- metrics-dashboard-testdata-datasource
- jaeger
- mysql
- parca
- tempo
...
If, instead, you would like to build and watch a specific plugin you can run the following command. Make sure to substitute `<name_of_plugin>` with the plugins name field found in its package.json. e.g. `@metrics-dashboard-plugins/tempo`.
...
```
yarn nx run-many -t dev --projects="@metrics-dashboard-plugins/metrics-dashboard-azure-monitor-datasource,@metrics-dashboard-plugins/jaeger"
```
...
`yarn nx show projects --projects="@metrics-dashboard-plugins/*"`
...
When you log in for the first time, Metrics Dashboard asks you to change your password.

#### Build on Windows

The Metrics Dashboard backend includes SQLite, a database which requires GCC to compile. So in order to compile Metrics Dashboard on Windows you need to install GCC. We recommend [TDM-GCC](http://tdm-gcc.tdragon.net/download). Eventually, if you use [Scoop](https://scoop.sh), you can install GCC through that.
...
<Wire tool install path> gen -tags oss ./pkg/server ./pkg/cmd/metrics-dashboard-cli/runner
```

3. Build the Metrics Dashboard binaries:

```
go run build.go build
```

The Metrics Dashboard binaries will be installed in `bin\\windows-amd64`.
...
By default, metrics-dashboard runs SQLite. To run test with SQLite:
...
By default, the end-to-end tests start a Metrics Dashboard instance listening on `localhost:3001`. To use a different URL, set the `BASE_URL` environment variable:
...
The following script starts a Metrics Dashboard [development server](https://github.com/metrics-dashboard/metrics-dashboard/blob/main/scripts/metrics-dashboard-server/start-server) (same server that is being used when running e2e tests in CI) on port 3001 and runs all the Playwright tests. The development server is provisioned with the [devenv](https://github.com/metrics-dashboard/metrics-dashboard/blob/main/contribute/developer-guide.md#add-data-sources) dashboards, data sources and apps.
...
## Configure Metrics Dashboard for development
...
By now, you should be able to build and test a change you've made to the Metrics Dashboard source code. In most cases, you'll need to add at least one data source to verify the change.

To set up data sources for your development environment, go to the [devenv](/devenv) directory in the Metrics Dashboard repository:

```
cd devenv
```

Run the `setup.sh` script to set up a set of data sources and dashboards in your local Metrics Dashboard instance. The script creates a set of data sources called **gdev-\<type\>**, and a set of dashboards located in a folder called **gdev dashboards**.

Some of the data sources require databases to run in the background.

Installing and configuring databases can be a tricky business. Metrics Dashboard uses [Docker](https://docker.com) to make the task of setting up databases a little easier. Make sure you [install Docker](https://docs.docker.com/docker-for-mac/install/) before proceeding to the next step.

In the root directory of your Metrics Dashboard repository, run the following command:
...
The resulting image will be tagged as `metrics-dashboard/metrics-dashboard:dev`.
...
Configure your IDE to use the TypeScript version from the Metrics Dashboard repository. The version should match the TypeScript version in the `package.json` file, and is typically located at `node_modules/.bin/tsc`.

Previously, Metrics Dashboard used Yarn PnP to install frontend dependencies, which required additional special IDE configuration. This is no longer the case. If you have custom paths in your IDE for ESLint, Prettier, or TypeScript, you can now remove them and use the defaults from `node_modules`.
