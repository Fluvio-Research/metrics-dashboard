...
Metrics Dashboard Labs occasionally identifies patterns that may be useful or harmful so that we can introduce or remove from the codebase.
...
desire to introduce new useful patterns with the need to keep Metrics Dashboard
...
| State            | Description                                                                                                                           |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------- |
| Proposed         | This is an optional practice that has been proposed and received positively by the Metrics Dashboard team. Follow this proposal as you choose.  |
...
## 1 - Idiomatic Metrics Dashboard code should be idiomatic Go code

**Status:** Ongoing, passive.

It's easier for contributors to start contributing to Metrics Dashboard if our
code is easily understandable. When there isn't a more specific Metrics Dashboard
...
Secondly, the guidelines for [Code Review Comments](https://github.com/golang/go/wiki/CodeReviewComments) for the Go compiler can mostly be applied directly to the Metrics Dashboard codebase.
There are idiosyncrasies in Metrics Dashboard, such as interfaces living closer to their declarations than to their users for services, and the documentation doesn't enforce public declarations.
...
Unfortunately, there's quite a lot of global state in Metrics Dashboard.
...
We plan to move all settings from package-level vars in the settings package to the [`setting.Cfg`](https://github.com/metrics-dashboard/metrics-dashboard/blob/df917663e6f358a076ed3daa9b199412e95c11f4/pkg/setting/setting.go#L210) struct. To access the settings, services and components can inject `setting.Cfg`:

- [`Cfg` struct](https://github.com/metrics-dashboard/metrics-dashboard/blob/df917663e6f358a076ed3daa9b199412e95c11f4/pkg/setting/setting.go#L210)
- [Injection](https://github.com/metrics-dashboard/metrics-dashboard/blob/c9773e55b234b7637ea97b671161cd856a1d3d69/pkg/services/cleanup/cleanup.go#L34)
...
**State:** Abandoned: The file-based refactoring of Metrics Dashboard is limited to work natively only on on-premise installations of Metrics Dashboard.
We want to enhance the use of the API to enable provisioning for all Metrics Dashboard instances in the future.

All new features that require state should be able to configure Metrics Dashboard using configuration files.
For example:

- [Data sources](https://github.com/metrics-dashboard/metrics-dashboard/tree/main/pkg/services/provisioning/datasources)
- [Alert notifiers](https://github.com/metrics-dashboard/metrics-dashboard/tree/main/pkg/services/provisioning/notifiers)
- [Dashboards](https://github.com/metrics-dashboard/metrics-dashboard/tree/main/pkg/services/provisioning/dashboards)

Today it's only possible to provision data sources and dashboards, but we want to support it throughout Metrics Dashboard in the future.
...
When you provide contextual information for the full life cycle of an API request, Metrics Dashboard can use contextual logging. It also provides contextual information about the
...
All errors returned from services in Metrics Dashboard should carry a status and
