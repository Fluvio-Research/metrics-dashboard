# Metrics Dashboard Kubernetes-inspired backend architecture

Metrics Dashboard is adopting a Kubernetes-inspired, resource-oriented model for all Metrics Dashboard core resources (dashboards, folders, datasources, alert rules, ..) where users can interact with these resources by defining their desired state via well-structured objects sent to Metrics Dashboard's new Resource APIs (`/apis/...`). Metrics Dashboard then acts upon these declarations to align the system's current state with the requested state.

On top of adopting the declarative, eventually-consistent system design, we are also adopting Kubernetes API's strict approach to URL structures, versioning and schemas. This approach addresses limitations of Metrics Dashboard's traditional Legacy APIs (`/api/...`), including:
...
- **Limited extensibility model:** No standardized way for Metrics Dashboard plugins to extend core APIs or use Metrics Dashboard database for storing custom configs

The end goal is for the Resource APIs to become the only interface for managing core Metrics Dashboard resources. We will eventually deprecate and remove all Legacy API endpoints.
...
- **URL structure & versioning:**
  ...
  - **Legacy APIs:** Variable path structures (e.g., `/api/dashboards/uid/:uid`, `/api/ruler/metrics-dashboard/api/v1/rules/:uid/:uid`). Less explicit versioning.
...
- **Namespacing / org context:**
  - **Resource APIs:** Uses explicit namespaces in the path (`/namespaces/<namespace>/...`) mapping to Metrics Dashboard Organization IDs in OSS/Enterprise and Metrics Dashboard Cloud Stack IDs in Metrics Dashboard Cloud for scoping.
...
- **Error Handling:**
  ...
  - **Legacy APIs:** Variable error formats; Metrics Dashboard-specific.

## 2. Core Concepts

- **Resource APIs:** New Metrics Dashboard API endpoints that mimic the Kubernetes API structure (e.g., `/apis/dashboard.metrics-dashboard.app/v1alpha1/dashboards`). These are served by Metrics Dashboard itself and are distinct from any Kubernetes API running on the same system.
...
- **API implementation approaches:**
  ...
  - **Apps Approach (`apps/...`):** The newer, more modular implementation where APIs are defined using CUE in self-contained modules. Each app defines resources in `kinds/*.cue` files and resources are registered via the App SDK. This is the direction Metrics Dashboard is moving towards for all resources.
...
- **`K8sHandler` (`pkg/services/apiserver/client/client.go`):** Internal client facade used by Metrics Dashboard services. Provides a Kubernetes-like client interface (`Get`, `Create`, etc.) for the Resource API. Acts as an adapter during migration, enabling legacy services to use the Resource API. Does not interact with external systems - it interacts only with Metrics Dashboard's own APIs.
- **Configured SQL database:** Metrics Dashboard's primary persistence layer (PostgreSQL, MySQL, SQLite). The persistence target for actions initiated within Metrics Dashboard's UI/Legacy API. It's the default backend for Unified Storage
- **App SDK (`github.com/metrics-dashboard/metrics-dashboard-app-sdk`):** SDK for the Apps Approach. Provides tools for defining resources (CUE) and generating code.

### 2.1 Unified Storage

The Unified Storage (`pkg/storage/unified/...`) is Metrics Dashboard's internal persistence layer backing the Resource API. It decouples API handlers from the storage backend.

**Design Rationale:**

- **Addressing legacy limitations:** Traditionally, Metrics Dashboard stored entire resource definitions (like dashboard JSON) as opaque blobs in resource-specific tables (e.g. `dashboard`). This made it difficult to query specific fields, implement consistent versioning/watching, or natively support Kubernetes concepts. It also made it impossible for plugins to store custom configs alongside core resources in Metrics Dashboard database.
- **Supporting existing Databases (vs. etcd/Kine):** Instead of `etcd`, Unified Storage uses Metrics Dashboard's existing SQL database support (PostgreSQL, MySQL, SQLite) for operational simplicity - we do not want to introduce `etcd` as a mandatory dependency - and backwards compatibility - we need to ensure Metrics Dashboard continues to work in all the varied environments its being used.

**Key Characteristics:**

- **Implementing Kubernetes [`storage.Interface`](https://github.com/kubernetes/apiserver/blob/master/pkg/storage/interfaces.go#L168):** Provides an implementation of `k8s.io/apiserver/pkg/storage.Interface` via `apistore.Storage` (`pkg/storage/unified/apistore/store.go`). This adapter translates Kubernetes storage operations (`Create`, `Get`, `List`, `Watch`) to calls against Metrics Dashboard's `resource.ResourceClient`.
...
The `apps` directory structure is Metrics Dashboard's intended future for defining APIs and implementing the Kubernetes-style resource model. Compared to the older Registry Approach, it offers:
...
The Apps Approach creates the same Resource API endpoints (`/apis/...`) as the Registry Approach, but with a cleaner implementation and improved developer experience. It's the standard pattern for all future Metrics Dashboard resources.

### 2.3 Kubernetes API Compatibility

Metrics Dashboard's Resource APIs adopts many Kubernetes API conventions but is **not strictly conformant** to the official Kubernetes API specification. We diverge from the Kubernetes architecture and Kubernetes API specification when needed for Metrics Dashboard's own unique requirements or to avoid operational complexity. Each architectural difference can surface as behavioral or wire-level incompatibility with the vanilla Kubernetes API.
...
- **Persistence:** Uses Metrics Dashboard's configured SQL database (PostgreSQL, MySQL, SQLite) via Unified Storage, not `etcd`.
- **Auth:** Uses Metrics Dashboard's standard auth (API keys, sessions, RBAC/Permissions), not Kubernetes ServiceAccounts or RBAC.
...
- **Controllers/Reconcilers:** Run **internally within the Metrics Dashboard server process**, not as external operators talking to `kube-apiserver`.
- **Dealing with large resources:** Some of Metrics Dashboard resources can be much larger than what Kubernetes/etcd allows which impacts design of some operations (e.g. `LIST`)
- **Dataplane routes:** Not all Metrics Dashboard Legacy APIs can be expressed as simple CRUD operations (e.g. datasource queries or proxy routes). We are going to define custom, resource- or group-level routes to handle these scenarios.
...
graph TB
  subgraph "Registry API Implementation<br>(pkg/registry/apis/... & pkg/apis/...)"
    ...
    RegistryAPIBuilder --> |Defines| APIGroups["API Group/Version<br><i>e.g., dashboard.metrics-dashboard.app/v1alpha1</i>"]
  ...
  subgraph "Apps-based API Implementation<br>(apps/... & pkg/registry/apps/...)"
    ...
    AppSDK["Metrics Dashboard App SDK<br><i>metrics-dashboard-app-sdk</i>"] --o |Provides| AppSDKTypes["SDK Interfaces/Types<br><i>app.Provider, etc.</i>"]
...
## 5. So does it all mean I can point `kubectl` directly at the Metrics Dashboard server and use it to create dashboards?

**Yes, with caveats.** `kubectl` is supported under a dev-mode-only, experimental `metrics-dashboardAPIServerEnsureKubectlAccess` feature flag and requires `server.protocol` set to `https`. Enabling `https` causes Metrics Dashboard to configure its web server with TLS certificates, either loading specified certificate/key files or generating self-signed ones if none are provided.

```bash
#!/bin/bash
# Assumes Metrics Dashboard running from repo root - adjust if running packaged Metrics Dashboard (e.g., /usr/local/etc/metrics-dashboard/metrics-dashboard-apiserver/metrics-dashboard.kubeconfig).
...
kubectl annotate --overwrite dashboards.dashboard.metrics-dashboard.app kubectl-dash metrics-dashboard.app/folder=kubectl-folder
```

**Standard Behavior (Flag disabled):**

By default, `kubectl` cannot connect directly to Metrics Dashboard. Metrics Dashboard APIs use standard Metrics Dashboard authentication (sessions, API keys), which `kubectl` does not handle natively.

**Experimental `kubectl` support (Flag enabled):**

If you enable the `metrics-dashboardAPIServerEnsureKubectlAccess` feature flag, Metrics Dashboard will enter "dev mode" and generate a `metrics-dashboard.kubeconfig` file in its config directory. The kubeconfig contains a bearer token and server details allowing `kubectl` to connect to Metrics Dashboard's Resource API. Relying on the flag for production workflows is discouraged in favor of using Metrics Dashboard's standard provisioning method.
