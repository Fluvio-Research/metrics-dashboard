...
- `${var_name:<format>}` This format gives you more control over how Metrics Dashboard interpolates values. Refer to [Advanced variable format options](#advanced-variable-format-options) for more detail on all the formatting types.
...
Test the formatting options on the [Metrics Dashboard Play site](https://play.metrics-dashboard.org/d/cJtIfcWiz/template-variable-formatting-options?orgId=1).
...
Metrics Dashboard typically converts the variable values as follows to accommodate Prometheus:
