---
aliases:
  - ../reference/share_dashboard/ # /docs/metrics-dashboard/latest/reference/share_dashboard/
  - ../reference/share_panel/ # /docs/metrics-dashboard/latest/reference/share_panel/
  - ../share-dashboards-panels/ # /docs/metrics-dashboard/latest/share-dashboards-panels/
  - ../sharing/ # /docs/metrics-dashboard/latest/sharing/
  - ../sharing/playlists/ # /docs/metrics-dashboard/latest/sharing/playlists/
  - ../sharing/share-dashboard/ # /docs/metrics-dashboard/latest/sharing/share-dashboard/
  - ../sharing/share-panel/ # /docs/metrics-dashboard/latest/sharing/share-panel/
  - share-dashboard/ # /docs/metrics-dashboard/latest/dashboards/share-dashboard/
keywords:
  - metrics-dashboard
  - dashboard
  - documentation
...
description: Share Metrics Dashboard dashboards and panels within your organization and publicly
weight: 650
refs:
  image-rendering:
  ...
  configure-report:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/create-reports/#create-a-report
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/visualizations/dashboards/create-reports/#create-a-report
  image-rendering-config:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#configuration
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#configuration
  max-width:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#viewport-maximum-width
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#viewport-maximum-width
  max-height:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#viewport-maximum-height
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#viewport-maximum-height
  max-scale:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#maximum-device-scale-factor
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/image-rendering/#maximum-device-scale-factor
---
...
Metrics Dashboard enables you to share dashboards and panels with other users within your organization and in certain situations, publicly on the web. You can share using:
...
You must have an authorized viewer permission to see an image rendered by a direct link. The same permission is also required to view embedded links unless you have anonymous access permission enabled for your Metrics Dashboard instance.

{{< admonition type="note" >}}
Anonymous access permission is not available in Metrics Dashboard Cloud. This feature is only supported for Metrics Dashboard Enterprise and Metrics Dashboard Open Source.
{{< /admonition >}}
...
1. Click **Copy link**.
1. Send the copied link to a Metrics Dashboard user with authorization to view the link.
...
Externally shared dashboards allow you to share your Metrics Dashboard dashboard with anyone. This is useful when you want to make your dashboard available to the world without requiring access to your Metrics Dashboard organization.
...
{{< admonition type="note" >}}
This feature is only available on Metrics Dashboard Enterprise.
{{< /admonition >}}
...
You can publish snapshots to your local instance or to [snapshots.raintank.io](http://snapshots.raintank.io). The latter is a free service provided by Metrics Dashboard Labs that enables you to publish dashboard snapshots to an external Metrics Dashboard instance. Anyone with the link can view it. You can set an expiration time if you want the snapshot removed after a certain time period.

{{< admonition type=note >}}
The snapshots.raintank.io option is disabled by default in Metrics Dashboard Cloud. You can update [your config file](https://metrics-dashboard.com/docs/metrics-dashboard/latest/setup-metrics-dashboard/configure-metrics-dashboard/#external_enabled) to enable this functionality.
{{< /admonition >}}
...
1. Click **Publish snapshot** or **Publish to snapshots.raintank.io**.
   ...
   Metrics Dashboard generates the link of the snapshot. Note that you can't publish dashboard snapshots containing custom panels to snapshot.raintank.io.
...
Export a Metrics Dashboard JSON file that contains everything you need, including layout, variables, styles, data sources, queries, and so on, so that you can later import the dashboard. To export a JSON file, follow these steps:
...
1. Select the theme for the dashboard. Choose from **Current**, **Dark**, or **Light**.
1. Click **Copy link**.
1. Send the copied link to a Metrics Dashboard user with authorization to view it.
...
1. Send the copied image to a Metrics Dashboard user with authorization to view it.
1. Click the **X** at the top-right corner to close the share drawer.

#### Query string parameters for server-side rendered images

When you click **Generate image** in the panel link settings, Metrics Dashboard generates a PNG image of the panel with the following default parameters:
...
```bash
https://play.metrics-dashboard.org/render/d-solo/ktMs4D6Mk?from=2024-09-03T11:55:44.442Z&to=2024-09-03T17:55:44.442Z&panelId=panel-13&__feature.dashboardSceneSolo&width=1000&height=500&tz=UTC
```

### Share an embed

You can share a panel by embedding it on another website using an iframe. Users must be signed into Metrics Dashboard to view the panel unless you have anonymous access permission enabled for your Metrics Dashboard instance.

{{< admonition type="note" >}}
Panel embedding and anonymous access permissions are not available in Metrics Dashboard Cloud, even for panels in [externally shared dashboards](ref:shared-dashboards). These capabilities are only supported in Metrics Dashboard Enterprise and Metrics Dashboard Open Source.
{{< /admonition >}}
...
The result is an interactive Metrics Dashboard visualization embedded in an iframe.
...
1. Click **Publish snapshot** or **Publish to snapshots.raintank.io**.
   ...
   Metrics Dashboard generates the link of the snapshot. Note that you can't publish snapshots that include custom panels to snapshot.raintank.io.
