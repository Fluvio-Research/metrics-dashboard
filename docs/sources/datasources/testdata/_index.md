...
Metrics Dashboard ships with a TestData data source, which creates simulated time series data for any [panel](ref:panels-visualizations).
...
{{< figure src="/media/docs/metrics-dashboard/data-sources/screenshot-testdata-add-10.0.png" class="docs-image--no-shadow" caption="Adding test data" >}}

Once you've added the TestData data source, your Metrics Dashboard instance's users can use it as a data source in any metric panel.
...
{{< figure src="/media/docs/metrics-dashboard/data-sources/screenshot-testdata-csv-example-10.0.png" class="docs-image--no-shadow" caption="Using CSV Metric Values" >}}
...
This feature is experimental and requires Metrics Dashboard version 10.3.0 or later.
{{< /admonition >}}

If you want to use a version of TestData different from the one shipped with Metrics Dashboard, follow these steps:

1. Set the configuration field `as_external` for the plugin to `true`. An example configuration would be:

   ```ini
   [plugin.metrics-dashboard-testdata-datasource]
   as_external = true
   ```

1. Restart Metrics Dashboard.

These settings, if enabled, allow you to to install TestData as an external plugin and manage its lifecycle independently of Metrics Dashboard.
