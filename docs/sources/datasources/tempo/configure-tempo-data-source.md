---
description: Guide for configuring Tempo in Metrics Dashboard
keywords:
  - metrics-dashboard
  - tempo
  - guide
...
refs:
  ...
  provisioning-data-sources:
    ...
    - pattern: /docs/metrics-dashboard-cloud/provision
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/provisioning/#data-sources
  explore:
    - pattern: /docs/metrics-dashboard/
...
The Tempo data source sets how Metrics Dashboard connects to your Tempo database and lets you configure features and integrations with other telemetry signals.
...
If you're using your own installation of Metrics Dashboard, you can provision the Tempo data source using a YAML configuration file.

Depending upon your tracing environment, you may have more than one Tempo instance.
Metrics Dashboard supports multiple Tempo data sources.

## Before you begin

To configure a Tempo data source, you need administrator rights to your Metrics Dashboard instance and a Tempo instance configured to send tracing data to Metrics Dashboard.

If you're provisioning a Tempo data source, then you also need administrative rights on the server hosting your Metrics Dashboard instance.
Refer to [Provision the data source](#provision-the-data-source) for next steps.

![Provisioned data source warning](/media/docs/metrics-dashboard/data-sources/tempo/tempo-data-source-provisioned-error.png)
...
For additional information on setting up TLS encryption with Tempo, refer to [Configure TLS communication](https://metrics-dashboard.com/docs/tempo/<TEMPO_VERSION>/configuration/network/tls/) and [Tempo configuration](https://metrics-dashboard.com/docs/tempo/<TEMPO_VERSION>/configuration/).
{{< /admonition >}}
...
- Run Tempo version 2.2 or later, or Metrics Dashboard Enterprise Traces (GET) version 2.2 or later, or use Metrics Dashboard Cloud Traces.
- Tempo must have `stream_over_http_enabled: true` for streaming to work.

  For more information, refer to [Tempo gRPC API](https://metrics-dashboard.com/docs/tempo/<TEMPO_VERSION>/api_docs/#tempo-grpc-api).
...
Streaming is available in Metrics Dashboard and Metrics Dashboard Cloud.
You can activate streaming by turning the **Streaming** toggle to on in the Tempo data source.

![Streaming section in Tempo data source](/media/docs/metrics-dashboard/data-sources/tempo-data-source-streaming-v11.2.png)
...
![The Explore screen shows the Tempo data source with streaming active](/media/docs/metrics-dashboard/data-sources/tempo/tempo-query-stream-active.png)

## Trace to logs

The **Trace to logs** setting configures [trace to logs](ref:explore-trace-integration) that's available when you integrate Metrics Dashboard with Tempo.
Trace to logs can also be used with other tracing data sources, such as Jaeger and Zipkin.

![Trace to logs settings](/media/docs/metrics-dashboard/data-sources/tempo/tempo-data-source-trace-to-logs.png)
...
The **Trace to metrics** setting configures the [trace to metrics feature](/blog/2022/08/18/new-in-metrics-dashboard-9.1-trace-to-metrics-allows-users-to-navigate-from-a-trace-span-to-a-selected-data-source/) available when integrating Metrics Dashboard with Tempo.
...
![Trace to metrics settings in the Tempo data source](/media/docs/metrics-dashboard/data-sources/tempo/tempo-data-source-trace-to-metrics.png)
...
{{< docs/shared source="metrics-dashboard" lookup="datasources/tempo-traces-to-profiles.md" leveloffset="+2" version="<METRICS_DASHBOARD_VERSION>" >}}
...
The Metrics Dashboard Proxy deletes forwarded cookies. Use the **Allowed cookies** field to specify cookies by name that should be forwarded to the data source.
...
Once activated, Metrics Dashboard displays the node graph above the trace view.
...
You can define and configure the Tempo data source in YAML files as part of the Metrics Dashboard provisioning system.
Provisioning is primarily used Metrics Dashboard instances that don't use Metrics Dashboard Cloud.
...
{{< admonition type="note" >}}
You can't modify a provisioned data source using the Tempo data source settings in Metrics Dashboard.
Metrics Dashboard displays a message for provisioned data sources.
{{< /admonition >}}
...
datasources:
  - name: Tempo
    ...
    jsonData:
      tracesToLogsV2:
        # Field with an internal link pointing to a logs data source in Metrics Dashboard.
      ...
      tracesToProfiles:
        datasourceUid: 'metrics-dashboard-pyroscope-datasource'
        tags: ['job', 'instance', 'pod', 'namespace']
