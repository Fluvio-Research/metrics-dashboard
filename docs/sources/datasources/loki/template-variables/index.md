...
You can use this variable type to specify any number of key/value filters, and Metrics Dashboard applies them automatically to all of your Loki queries.
...
Consider using the `$__auto` variable in your Loki metric queries, which will automatically be substituted with the [step value](https://metrics-dashboard.com/docs/metrics-dashboard/next/datasources/loki/query-editor/#options) for range queries, and with the selected time range's value (computed from the starting and ending times) for instant queries.
