...
When you select a label, Metrics Dashboard retrieves available values from the server.
...
To reduce the repetition of selecting and removing the same labels when examining multiple log context windows, Metrics Dashboard stores your selected labels and applies them to each open context window. This lets you seamlessly navigate through various log context windows without having to reapply your filters.
...
Live tailing relies on two Websocket connections: one between the browser and Metrics Dashboard server, and another between the Metrics Dashboard server and Loki server.
...
**Using Apache2 for proxying between the browser and the Metrics Dashboard server:**
...
It assumes that the Metrics Dashboard server is available at `http://localhost:3000/`, the Loki server is running locally without proxy, and your external site uses HTTPS.
...
Metrics Dashboard automatically uses log content as annotation text and your log stream labels as tags.
