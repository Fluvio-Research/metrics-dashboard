...
description: Guide for using Google Cloud Monitoring in Metrics Dashboard
keywords:
  - metrics-dashboard
  - stackdriver
  - google
...
Metrics Dashboard ships with built-in support for Google Cloud Monitoring.
...
If Metrics Dashboard is running on a Google Compute Engine (GCE) virtual machine, when you [Configure a GCE Default Service Account](google-authentication/#configure-a-gce-default-service-account), you must also grant that Service Account access to the "Cloud Monitoring API" scope.
...
After Metrics Dashboard loads the dashboard, you can select a project from the dropdown list.
