...
refs:
  ...
  configure-standard-options-display-name:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/panels-visualizations/configure-standard-options/#display-name
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/panels-visualizations/configure-standard-options/#display-name
...
Metrics Dashboard can format the response from MS SQL as either a table or as a time series.
...
Code mode supports autocompletion of tables, columns, SQL keywords, standard SQL functions, Metrics Dashboard template variables, and Metrics Dashboard macros.
...
In the **Dataset** dropdown, select the MSSQL database to query. Metrics Dashboard populates the dropdown with all databases that the user can access.
Once you select a database, Metrics Dashboard populates the dropdown with all available tables.
...
{{< docs/shared source="metrics-dashboard" lookup="datasources/sql-query-builder-macros.md" version="<METRICS_DASHBOARD_VERSION>" >}}
...
| `$__timeGroup(dateColumn,'5m', 0)`                    | Same as above but with a fill parameter so missing points in that series will be added by metrics-dashboard and 0 will be used as value.                                                                                                                                         |
...
Store timestamps in UTC to avoid issues with time shifts in Metrics Dashboard when using non-UTC timezones.
{{< /admonition >}}
...
A time series query result is returned in a [wide data frame format](https://metrics-dashboard.com/developers/plugin-tools/key-concepts/data-frames#wide-format).
