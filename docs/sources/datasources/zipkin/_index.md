...
description: Guide for using Zipkin in Metrics Dashboard
keywords:
  - metrics-dashboard
  - zipkin
  - tracing
...
Metrics Dashboard ships with built-in support for Zipkin, an open source, distributed tracing system. This topic explains configuration and queries specific to the Zipkin data source.
...
Once you've added the Zipkin data source, you can [configure it](#configure-the-data-source) so that your Metrics Dashboard instance's users can create queries in its [query editor](#query-the-data-source) when they [build dashboards](ref:build-dashboards) and use [Explore](ref:explore).
...
The **Trace to logs** setting configures the [trace to logs feature](ref:trace-integration) that is available when you integrate Metrics Dashboard with Zipkin.
...
The **Trace to metrics** setting configures the [trace to metrics feature](/blog/2022/08/18/new-in-metrics-dashboard-9.1-trace-to-metrics-allows-users-to-navigate-from-a-trace-span-to-a-selected-data-source/) available when integrating Metrics Dashboard with Zipkin.
...
Once enabled, Metrics Dashboard displays the Node Graph after loading the trace view.
