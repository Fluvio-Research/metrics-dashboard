...
**Standard options** in the panel editor pane let you change how field data is displayed in your visualizations. Options that you apply don't change the data, they just change how Metrics Dashboard _displays_ the data.
...
![A custom currency unit called Gems in the Unit drop-down](/media/docs/metrics-dashboard/panels-visualizations/custom_unit_currency_v11.0.png)
...
![A thumbs up emoji as a custom unit](/media/docs/metrics-dashboard/panels-visualizations/custom_unit_thumbsup_v11.0.png)

![A time series visualization using custom thumbs up emoji units](/media/docs/metrics-dashboard/panels-visualizations/thumbsup_panel_v11.0.png)

##### Time format units

All **Date & time** format units in Metrics Dashboard (such as **Datetime ISO** or **Datetime US**) expect input values to be in milliseconds since the Unix epoch (January 1, 1970). If your data source provides timestamps in seconds, these will be incorrectly interpreted as dates very close to January 1, 1970.
...
By default, Metrics Dashboard automatically scales the unit based on the magnitude of the value. For example, if you have values of 0.14kW and 3000kW, Metrics Dashboard displays them as 140W and 3MW, respectively. You can use custom units to control this behavior by setting a prefix, suffix, or custom SI unit.

#### String units

Sometimes Metrics Dashboard is too aggressive in interpreting strings and displaying them as numbers. To configure Metrics Dashboard to show the original string value, select **Misc > String** in the **Unit** drop-down.
...
By default, the calculated **Min** and **Max** are based on the minimum and maximum of all series and fields. When you enable **Field min/max**, Metrics Dashboard calculates the min or max of each field individually, based on the minimum or maximum value of the field.

### Decimals

Specify the number of decimals Metrics Dashboard includes in the rendered value. If you leave this field empty, Metrics Dashboard automatically truncates the number of decimals based on the value. For example 1.1234 displays as 1.12 and 100.456 displays as 100.
...
| ---------------------------- | ----------------------- | ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `${__field.displayName}`     | Same as syntax          | `Temp {Loc="PBI", Sensor="3"}` | Displays the field name, and labels in `{}` if they are present. If there is only one label key in the response, then for the label portion, Metrics Dashboard displays the value of the label without the enclosing braces. |
...
| Shades of a color                     | Metrics Dashboard selects shades of a single color.                                                                                                                                                                                                                                                                                                                                                                   |
...
| Classic palette                       | Metrics Dashboard automatically assigns a color for each field or series based on its order. If the order of a field changes in your query, the color also changes. Useful for graphs, pie charts, and other categorical data visualizations.                                                                                                                                                                         |
| Classic palette (by series name)      | Metrics Dashboard automatically assigns colors based on the name of the series. Useful when the series names to be visualized can change based on the available data.                                                                                                                                                                                                                                                 |
| Multiple continuous colors (by value) | Metrics Dashboard automatically assigns colors based on the percentage of a value relative to the min and the max of the field or series. For some visualizations, you also need to choose if the color is set by the **Last**, **Min**, or **Max** value of the field or series. Select from: **Green-Yellow-Red**, **Red-Yellow-Green**, **Blue-Yellow-Red**, **Yellow-Red**, **Blue-Purple**, and **Yellow-Blue**. |
| Single continuous color (by value)    | Metrics Dashboard automatically assigns shades of one color based on the percentage of a value relative to the min and the max of the field or series. For some visualizations, you also need to choose if the color is set by the **Last**, **Min**, or **Max** value of the field or series. Select from: **Blues**, **Reds**, **Greens**, and **Purples**.                                                         |
...
Enter what Metrics Dashboard should display if the field value is empty or null. The default value is a hyphen (-).
