...
description: Manipulate and transform data in Metrics Dashboard using SQL expressions.
weight: 45
refs:
  expressions:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/panels-visualizations/query-transform-data/expression-queries/
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/visualizations/panels-visualizations/query-transform-data/expression-queries/
---
...
In Metrics Dashboard, a server-side expression is a way to transform or calculate data after it has been retrieved from the data source, but before it is sent to the frontend for visualization. Metrics Dashboard evaluates these expressions on the server, not in the browser or at the data source.

For general information on Metrics Dashboard expressions, refer to [Write expression queries](ref:expressions).
...
- Enable SQL expressions under the feature toggle `sqlExpressions`.
  - If you self-host Metrics Dashboard, you can find feature toggles in the configuration file `metrics-dashboard.ini`.
...
- If you are using Metrics Dashboard Cloud, contact [Support](https://metrics-dashboard.com/help/) to enable this feature.
...
To work with SQL expressions, you must use data from a backend data source. In Metrics Dashboard, a backend data source refers to a data source plugin or integration that communicates with a database, service, or API through the Metrics Dashboard server, rather than directly from the browser (frontend).
...
The SQL expression workflow in Metrics Dashboard is designed with the following behaviors:

- **Unhidden queries are visualized automatically.** If an input query is not hidden, Metrics Dashboard will attempt to render it alongside your SQL expression. This can clutter the output, especially in table visualizations.
...
- If the frame type is present and is either numeric, wide time series, or multi-frame time series (for example, labeled formats), Metrics Dashboard automatically converts the data into a table structure.

## Known limitations

- Currently, only one SQL expression is supported per panel or alert.
- Metrics Dashboard supports certain data sources. Refer to [compatible data sources](#compatible-data-sources) for a current list.
...
Metrics Dashboard supports three types of data source response formats:
...
For more information on Dataplane formats, refer to [Metrics Dashboard Dataplane Documentation](https://metrics-dashboard.com/developers/dataplane).
...
- For time series data, Metrics Dashboard includes a `time` column with timestamps
...
2. Add the SQL expression `SELECT * from A`. After you add a SQL expression that selects from RefID A, Metrics Dashboard converts it to a table response:
