...
The Alerting Provisioning HTTP API can be used to create, modify, and delete resources relevant to Metrics Dashboard-managed alerts. This API is the one used by our [Metrics Dashboard Terraform provider](https://registry.terraform.io/providers/metrics-dashboard/metrics-dashboard/latest/docs).

For more information on the differences between Metrics Dashboard-managed and data source-managed alerts, refer to [Introduction to alert rules](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/fundamentals/alert-rules/).

> If you are running Metrics Dashboard Enterprise, you need to add specific permissions for some endpoints. For more information, refer to [Role-based access control permissions](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/roles-and-permissions/access-control/custom-role-actions-scopes/).

## Metrics Dashboard-managed endpoints
...
The `export` endpoints allow you to export alerting resources in a JSON format suitable for [provisioning via files](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/set-up/provision-alerting-resources/file-provisioning/). However, this format cannot be used to update resources via the HTTP API.
...
Template groups enable you to define multiple notification templates (`{{ define "" }}`) within a single group. They can be managed from the Metrics Dashboard Alerting UI.
...
### Edit resources in the Metrics Dashboard UI

By default, you cannot edit API-provisioned alerting resources in Metrics Dashboard.

To enable editing these resources in the Metrics Dashboard UI, add the **`X-Disable-Provenance: true`** header to the following API requests:
...
To reset the notification policy tree to the default and unlock it for editing in the Metrics Dashboard UI, use:

- [`DELETE /api/v1/provisioning/policies`](#route-reset-policy-tree)

## Data source-managed resources

The Alerting Provisioning HTTP API can only be used to manage Metrics Dashboard-managed alert resources. To manage resources related to [data source-managed alerts](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/alerting-rules/create-data-source-managed-rule/), consider the following tools:

- [mimirtool](https://metrics-dashboard.com/docs/mimir/<MIMIR_VERSION>/manage/tools/mimirtool/): to interact with the Mimir alertmanager and ruler configuration.
- [cortex-tools](https://github.com/metrics-dashboard/cortex-tools#cortextool): to interact with the Cortex alertmanager and ruler configuration.
- [lokitool](https://metrics-dashboard.com/docs/loki/<LOKI_VERSION>/alert/#lokitool): to configure the Loki Ruler.

Alternatively, the [Metrics Dashboard Alerting API](https://editor.swagger.io/?url=https://raw.githubusercontent.com/metrics-dashboard/metrics-dashboard/main/pkg/services/ngalert/api/tooling/post.json) can be used to access data from data source-managed alerts. This API is primarily intended for internal usage, with the exception of the `/api/v1/provisioning/` endpoints. It's important to note that internal APIs may undergo changes without prior notice and are not officially supported for user consumption.
...
| `X-Disable-Provenance: true` | header | string | string  |          |         | Allows editing of provisioned resources in the Metrics Dashboard UI |
...
{{< docs/shared lookup="alerts/alerting-provisioning-export-produces.md" source="metrics-dashboard" version="<METRICS_DASHBOARD_VERSION>" >}}
...
| `FolderUID`                  | path   | string                              | string                  |    ✓     |         |                                                                                                                         |
| `Group`                      | path   | string                              | string                  |    ✓     |         |                                                                                                                         |
| `X-Disable-Provenance: true` | header | string                              | string                  |          |         | Allows editing of provisioned resources in the Metrics Dashboard UI. This also applies to all alert rules within the alert group. |
...
| ------------------- | ----------------------------------------- | ------------------- | :------: | ------- | -------------------------------------------------------------------------------------------------------------------- | ------- |
| `datasourceUid`     | string                                    | string              |          |         | Metrics Dashboard data source unique identifier; it should be '**expr**' for a Server Side Expression operation.               |         |
...
EmbeddedContactPoint is the contact point type used by Metrics Dashboard-managed alerts.
