---
aliases:
  - ../administration/configuration/ # /docs/metrics-dashboard/latest/administration/configuration/
  - ../installation/configuration/ # /docs/metrics-dashboard/latest/installation/configuration/
description: Learn how to configure Metrics Dashboard and understand configuration options.
labels:
  products:
    - enterprise
    - oss
...
Metrics Dashboard has default and custom configuration files.
You can customize your Metrics Dashboard instance by modifying the custom configuration file or by using environment variables.
To see the list of settings for a Metrics Dashboard instance, refer to [View server settings](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/stats-and-license#view-server-settings).
...
Restart Metrics Dashboard for your changes to take effect.
{{< /admonition >}}

## Configuration file location

The default settings for a Metrics Dashboard instance are stored in the `<WORKING DIRECTORY>/conf/defaults.ini` file.
_Don't_ change this file.

Depending on your OS, your custom configuration file is either the `<WORKING DIRECTORY>/conf/custom.ini` file or the `/usr/local/etc/metrics-dashboard/metrics-dashboard.ini` file.
You can use a custom configuration path with the `--config` option.

### Linux

If you installed Metrics Dashboard using the deb or RPM packages, then your configuration file is located at `/etc/metrics-dashboard/metrics-dashboard.ini` and a separate `custom.ini` is not used.
This path is specified in the Metrics Dashboard init.d script using `--config` option.

### Docker

Refer to [Configure a Metrics Dashboard Docker image](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-docker/) for information about environmental variables, persistent storage, and building custom Docker images.
...
By default, the configuration file is located at `/opt/homebrew/etc/metrics-dashboard/metrics-dashboard.ini` or `/usr/local/etc/metrics-dashboard/metrics-dashboard.ini`.
For a Metrics Dashboard instance installed using Homebrew, edit the `metrics-dashboard.ini` file directly.
...
There is no local configuration file for Metrics Dashboard Cloud stacks, but many of these settings are still configurable. To edit configurable settings, open a support ticket.

## Remove comments in the .ini files

Metrics Dashboard uses semicolons (`;`) to comment out lines in the INI file.
To uncomment a line, remove the semicolon (`;`) from the beginning of that line.

Metrics Dashboard ignores all configuration lines that begin with a semicolon.
...
[plugin.metrics-dashboard-image-renderer]
...
export GF_PLUGIN_METRICS_DASHBOARD_IMAGE_RENDERER_RENDERING_IGNORE_HTTPS_ERRORS=true
export GF_FEATURE_TOGGLES_ENABLE=newNavigation
```

## Variable expansion

If any of your options contains the expression `$__<PROVIDER>{<ARGUMENT>}`or `${<ENVIRONMENT VARIABLE>}`, then Metrics Dashboard evaluates them.
...
```ini
[paths]
logs = $__env{LOGDIR}/metrics-dashboard
```
...
{{< admonition type="note" >}}
The `vault` provider is only available in Metrics Dashboard Enterprise.

For more information, refer to [Integrate Metrics Dashboard with Hashicorp Vault](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-security/configure-database-encryption/integrate-with-hashicorp-vault).

{{< /admonition >}}

## Configuration options

The following headings describe the sections and configuration options of the Metrics Dashboard configuration file.

### `app_mode`

Options are `production` and `development`.
Default is `production`. _Don't_ change this option unless you are working on Metrics Dashboard development.

### `instance_name`

Set the name of the Metrics Dashboard server instance.
Used in logging, internal metrics, and clustering info.
Defaults to: `${HOSTNAME}`, which uses the value of the environment variable `HOSTNAME`, if that is empty or doesn't exist Metrics Dashboard tries to use system calls to get the machine name.
...
Path to where Metrics Dashboard stores the sqlite3 database (if used), file-based sessions (if used), and other data.
This path is usually specified via command line in the init.d script or the systemd service file.

**macOS:** The default SQLite database is located at `/usr/local/var/lib/metrics-dashboard`
...
Path to where Metrics Dashboard stores logs.
This path is usually specified via command line in the init.d script or the systemd service file.
You can override it in the configuration file or in the default environment variable file.

{{< admonition type="note" >}}
When overriding the default log path in the configuration file or environment variable file, Metrics Dashboard still logs to the default log path until it has fully started.
{{< /admonition >}}

Override log path using the command line argument `cfg:default.paths.logs`:

```bash
./metrics-dashboard-server --config /custom/config.ini --homepath /custom/homepath cfg:default.paths.logs=/custom/path
```

**macOS:** By default, the log file should be located at `/usr/local/var/log/metrics-dashboard/metrics-dashboard.log`.

#### `plugins`

Directory where Metrics Dashboard automatically scans and looks for plugins. For information about manually or automatically installing plugins, refer to [Install Metrics Dashboard plugins](../../administration/plugin-management/#install-metrics-dashboard-plugins).

**macOS:** By default, the Mac plugin location is: `/usr/local/var/lib/metrics-dashboard/plugins`.

#### `provisioning`

Directory that contains [provisioning](../../administration/provisioning/) configuration files that Metrics Dashboard applies on startup.
...
If your machine has more than one network interface, you can use this setting to expose the Metrics Dashboard service on only one network interface and not have it available on others, such as the loopback interface.
An empty value is equivalent to setting the value to `0.0.0.0`, which means the Metrics Dashboard service binds to all interfaces.
...
The port to bind to, defaults to `3000`. To use port 80 you need to either give the Metrics Dashboard binary permission for example:

```bash
sudo setcap 'cap_net_bind_service=+ep' /usr/sbin/metrics-dashboard-server
```

Or redirect port 80 to the Metrics Dashboard port using:

```bash
sudo iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 3000
```

Another way is to put a web server like Nginx or Apache in front of Metrics Dashboard and have them proxy requests to Metrics Dashboard.
...
This is the full URL used to access Metrics Dashboard from a web browser. This is
important if you use Google or GitHub OAuth authentication (for the
callback URL to be correct).

{{< admonition type="note" >}}
This setting is also important if you have a reverse proxy in front of Metrics Dashboard that exposes it through a sub-path.
...
Serve Metrics Dashboard from sub-path specified in `root_url` setting. By default it is set to `false` for compatibility reasons.

By enabling this setting and using a sub-path in `root_url` like `root_url = http://localhost:3000/metrics-dashboard`, Metrics Dashboard is accessible on `http://localhost:3000/metrics-dashboard`.
If accessed without sub-path Metrics Dashboard redirects the request to the sub-path.

#### `router_logging`

Set to `true` for Metrics Dashboard to log all HTTP requests (not just errors). These are logged as Info level events to the Metrics Dashboard log.

#### `static_root_path`

The path to the directory where the frontend files (HTML, JS, and CSS
files). Defaults to `public` which is why the Metrics Dashboard binary needs to be
...
Make sure that the target group is in the group of Metrics Dashboard process and that Metrics Dashboard process is the file owner before you change this setting.
It is recommended to set the GID as HTTP server user GID.
Not set when the value is `-1`.

#### `socket_mode`

Mode where the socket should be set when `protocol=socket`. Make sure that Metrics Dashboard process is the file owner before you change this setting.

#### `socket`

Path where the socket should be created when `protocol=socket`. Make sure Metrics Dashboard has appropriate permissions for that path before you change this setting.

#### `cdn_url`

Specify a full HTTP URL address to the root of your Metrics Dashboard CDN assets. Metrics Dashboard adds edition and version paths.

For example, given a CDN URL like `https://cdn.myserver.com`, Metrics Dashboard tries to load a JavaScript file from
`http://cdn.myserver.com/metrics-dashboard-oss/7.4.0/public/build/app.<HASH>.js`.
...
Metrics Dashboard needs a database to store users and dashboards (and other
things). By default it is configured to use [`sqlite3`](https://www.sqlite.org/index.html) which is an
embedded database (included in the main Metrics Dashboard binary).
...
For example, for MySQL running on the same host as Metrics Dashboard: `host = 127.0.0.1:3306` or with Unix sockets: `host = /var/run/mysqld/mysqld.sock`

#### `name`

The name of the Metrics Dashboard database. Leave it set to `metrics-dashboard` or some
other name.
...
The maximum number of open connections to the database. For MYSQL, configure this setting on both Metrics Dashboard and the database. For more information, refer to [`sysvar_max_connections`](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html#sysvar_max_connections).
...
Only the MySQL driver supports isolation levels in Metrics Dashboard. In case the value is empty, the driver's default isolation level is applied. Available options are "READ-UNCOMMITTED", "READ-COMMITTED", "REPEATABLE-READ" or "SERIALIZABLE".
...
Caches authentication tokens and other temporary authentication-related data in the configured database, Redis, or Memcached. This setting doesn't configure [Query Caching in Metrics Dashboard Enterprise](../../administration/data-source-management/#query-and-resource-caching).
...
Leave empty when using `database` and Metrics Dashboard uses the primary database.

##### `redis`

Example connection string: `addr=127.0.0.1:6379,pool_size=100,db=0,username=metrics-dashboard,password=metrics-dashboardRocks,ssl=false`
...
The length of time that Metrics Dashboard waits for a successful TLS handshake with the data source.
...
The length of time that Metrics Dashboard waits for the first response headers from a data source after fully writing the request headers, if the request has an `Expect: 100-continue` header.
...
The maximum number of idle connections that Metrics Dashboard maintains. Default is `100`.
...
The length of time that Metrics Dashboard maintains idle connections before closing them. Default is `90` seconds.
...
If enabled and user is not anonymous, data proxy adds the `X-Metrics Dashboard-User` header with username into the request. Default is `false`.

#### `response_limit`

Limits the amount of bytes that Metrics Dashboard reads from responses of outgoing HTTP requests. Default is `0` which means disabled.

#### `row_limit`

Limits the number of rows that Metrics Dashboard processes from SQL data sources. Default is `1000000`.

#### `user_agent`

Sets a custom value for the `User-Agent` header for outgoing data proxy requests. If empty, the default value is `Metrics Dashboard/<BuildVersion>` (for example `Metrics Dashboard/9.0.0`).
...
This option is also known as _usage analytics_. When `false`, this option disables the writers that write to the Metrics Dashboard database and the associated features, such as dashboard and data source insights, presence indicators, and advanced dashboard search. The default value is `true`.

#### `reporting_enabled`

When enabled Metrics Dashboard sends anonymous usage statistics to `stats.metrics-dashboard.org`.
Metrics Dashboard doesn't track IP addresses, only counters of running instances, versions, dashboards, and errors.

The anonymous usage statistics help inform the future development of Metrics Dashboard.
Counters are sent every 24 hours.
Default value is `true`.

#### `check_for_updates`

Set to `false` to disable checking for new versions of Metrics Dashboard in GitHub.
When enabled, the check for a new version runs every ten minutes.
It notifies, via the UI, when a new version is available.
The check itself doesn't cause automatic updates of the Metrics Dashboard software, nor does it send any sensitive information.

#### `check_for_plugin_updates`

Set to `false` to disable checking for new versions of installed plugins from https://metrics-dashboard.com.
...
If you want to track Metrics Dashboard usage with Google Analytics specify _your_ Universal Analytics ID here.
By default this feature is disabled.

#### `google_analytics_4_id`

If you want to track Metrics Dashboard usage with Google Analytics 4 specify _your_ GA4 ID here.
By default this feature is disabled.
...
If you want to track Metrics Dashboard usage via RudderStack specify _your_ RudderStack
...
If you want to track Metrics Dashboard usage via Azure Application Insights, then specify _your_ Application Insights connection string. Since the connection string contains semicolons, you need to wrap it in backticks (`). By default, tracking usage is disabled.
...
Disable creation of a Metrics Dashboard Admin user on first start of Metrics Dashboard. Default is `false`.

#### `admin_user`

The name of the default Metrics Dashboard Admin user, who has full permissions.
Default is `admin`.

#### `admin_password`

The password of the default Metrics Dashboard Admin. Set once on first-run. Default is `admin`.

#### `admin_email`

The email of the default Metrics Dashboard Admin, created on startup. Default is `admin@localhost`.
...
Define a allowlist of IP addresses or domains with ports, that can be used in data source URLs with the Metrics Dashboard data source proxy.
...
Set to `true` if you host Metrics Dashboard behind HTTPS. Default is `false`.
...
When `false`, the HTTP header `X-Frame-Options: deny` is set in Metrics Dashboard HTTP responses which instructs browsers to not allow rendering Metrics Dashboard in a `<frame>`, `<iframe>`, `<embed>` or `<object>`.
...
`actions_allow_post_url=/api/plugins/metrics-dashboard-special-app`
...
Set root URL to a Metrics Dashboard instance where you want to publish external snapshots (defaults to https://snapshots.raintank.io).
...
Set to true to enable this Metrics Dashboard instance to act as an external snapshot server and allow unauthenticated requests for creating and deleting snapshots. Default is `false`.
...
Default is `false`.
A Metrics Dashboard Admin can still create users.
...
This is a comma-separated list of usernames. Users specified here are hidden in the Metrics Dashboard UI. They are still visible to Metrics Dashboard administrators and to themselves.

<hr>

### `[auth]`

Metrics Dashboard provides many ways to authenticate users. Refer to the Metrics Dashboard [Authentication overview](../configure-security/configure-authentication/) and other authentication documentation for detailed instructions on how to set up and configure authentication.

#### `login_cookie_name`

The cookie name for storing the auth token. Default is `metrics-dashboard_session`.
...
{{< admonition type="warning" >}}
Currently if no organization role mapping is found for a user, Metrics Dashboard doesn't update the user's organization role.
...
With `skip_org_role_sync` set to `true`, when a user logs in for the first time, Metrics Dashboard sets the organization role based on the value specified in `auto_assign_org_role` and forces the organization to `auto_assign_org_id` when specified, otherwise it falls back to OrgID `1`.

{{< admonition type="note" >}}
Enabling `skip_org_role_sync` also disables the synchronization of Metrics Dashboard Admins from the external provider, as such `allow_assign_metrics-dashboard_admin` is ignored.
{{< /admonition >}}

Use this setting when you want to manage the organization roles of your users from within Metrics Dashboard and be able to manually assign them to multiple organizations, or to prevent synchronization conflicts when they can be synchronized from another provider.
...
| --------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| false                             | false                | Synchronize user organization role with metrics-dashboard.com role. If no role is provided, `auto_assign_org_role` is set.                    | false      |
...
| false                             | true                 | Skips organization role synchronization for metrics-dashboard.com users. Role is set to `auto_assign_org_role`.                               | true       |
| true                              | true                 | Skips organization role synchronization for metrics-dashboard.com users and all other OAuth providers. Role is set to `auto_assign_org_role`. | true       |
...
| true                              | false                | User organization role is set to `auto_assign_org_role` and can be changed in Metrics Dashboard. | true       |
| false                             | true                 | User organization role is set to `auto_assign_org_role` and can be changed in Metrics Dashboard. | true       |
| true                              | true                 | User organization role is set to `auto_assign_org_role` and can be changed in Metrics Dashboard. | true       |

{{< admonition type="note" >}}
For GitLab, GitHub, Okta, Generic OAuth providers, Metrics Dashboard synchronizes organization roles and sets Metrics Dashboard Admins. The `allow_assign_metrics-dashboard_admin` setting is also accounted for, to allow or not setting the Metrics Dashboard Admin role from the external provider.
{{< /admonition >}}
...
| true                              | false                | Skips organization role synchronization for all OAuth providers' users. Role is set to `auto_assign_org_role`.                                                   | true       |
| false                             | true                 | Skips organization role and Metrics Dashboard Admin synchronization for GitHub users. Role is set to `auto_assign_org_role`.                                               | true       |
| true                              | true                 | Skips organization role synchronization for all OAuth providers and skips Metrics Dashboard Admin synchronization for GitHub users. Role is set to `auto_assign_org_role`. | true       |
...
| true                              | false                | Skips organization role synchronization for all OAuth providers' users. Role is set to `auto_assign_org_role`.                                                   | true       |
| false                             | true                 | Skips organization role and Metrics Dashboard Admin synchronization for GitLab users. Role is set to `auto_assign_org_role`.                                               | true       |
| true                              | true                 | Skips organization role synchronization for all OAuth providers and skips Metrics Dashboard Admin synchronization for GitLab users. Role is set to `auto_assign_org_role`. | true       |
...
| true                              | false                | Skips organization role synchronization for all OAuth providers' users. Role is set to `auto_assign_org_role`.                                                           | true       |
| false                             | true                 | Skips organization role and Metrics Dashboard Admin synchronization for the provider's users. Role is set to `auto_assign_org_role`.                                               | true       |
| true                              | true                 | Skips organization role synchronization for all OAuth providers and skips Metrics Dashboard Admin synchronization for the provider's users. Role is set to `auto_assign_org_role`. | true       |
...
| true                              | false                | Skips organization role synchronization for all OAuth providers' users. Role is set to `auto_assign_org_role`.                                                 | true       |
| false                             | true                 | Skips organization role and Metrics Dashboard Admin synchronization for Okta users. Role is set to `auto_assign_org_role`.                                               | true       |
| true                              | true                 | Skips organization role synchronization for all OAuth providers and skips Metrics Dashboard Admin synchronization for Okta users. Role is set to `auto_assign_org_role`. | true       |
...
| --------------------------------- | -------------------- | --------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| false                             | false                | Synchronized with Google Auth organization roles                                        | A user logs in to Metrics Dashboard using their Google account and their organization role is automatically set based on their role in Google.                                                     |
| true                              | false                | Skipped synchronization of organization roles from all OAuth providers                  | A user logs in to Metrics Dashboard using their Google account and their organization role is _not_ set based on their role. But Metrics Dashboard Administrators can modify the role from the UI.           |
| false                             | true                 | Skipped synchronization of organization roles Google                                    | A user logs in to Metrics Dashboard using their Google account and their organization role is _not_ set based on their role in Google. But Metrics Dashboard Administrators can modify the role from the UI. |
| true                              | true                 | Skipped synchronization of organization roles from all OAuth providers including Google | A user logs in to Metrics Dashboard using their Google account and their organization role is _not_ set based on their role in Google. But Metrics Dashboard Administrators can modify the role from the UI. |
...
> Only available in Metrics Dashboard 11.3+.
...
Refer to [Anonymous authentication](../configure-security/configure-authentication/metrics-dashboard/#anonymous-authentication) for detailed instructions.
...
Metrics Dashboard supports additional integration with Azure services when hosted in the Azure Cloud.

#### `cloud`

Azure cloud environment where Metrics Dashboard is hosted:
...
Specifies whether Metrics Dashboard is running in Azure with Managed Identity configured (for example, running in a Azure Virtual Machines instance). Disabled by default, needs to be explicitly enabled.
...
By default, this includes all Metrics Dashboard Labs owned Azure plugins or those that use Azure settings (Azure Monitor, Azure Data Explorer, Prometheus, MSSQL).
...
Enable this to allow Metrics Dashboard to send email. Default is `false`.
...
Address used when sending out emails, default is `admin@metrics-dashboard.localhost`.

#### `from_name`

Name to be used when sending out emails, default is `Metrics Dashboard`.
...
Metrics Dashboard logging options.
...
Use this configuration option to set the default error message shown to users. This message is displayed instead of sensitive backend errors, which should be obfuscated. The default message is `Please inspect the Metrics Dashboard server log for details.`.
...
Custom HTTP endpoint to send events captured by the Faro agent to. Default, `/log-metrics-dashboard-javascript-agent`, logs the events to standard output.

#### `log_endpoint_requests_per_second_limit`

Requests per second limit enforced per an extended period, for Metrics Dashboard backend log ingestion endpoint, `/log-metrics-dashboard-javascript-agent`. Default is `3`.

#### `log_endpoint_burst_limit`

Maximum requests accepted per short interval of time for Metrics Dashboard backend log ingestion endpoint, `/log-metrics-dashboard-javascript-agent`. Default is `15`.
...
Turn on error instrumentation. Only affects Metrics Dashboard JavaScript Agent.

#### `instrumentations_console_enabled`

Turn on console instrumentation. Only affects Metrics Dashboard JavaScript Agent

#### `instrumentations_webvitals_enabled`

Turn on web vitals instrumentation. Only affects Metrics Dashboard JavaScript Agent

#### `instrumentations_tracing_enabled`

Turns on tracing instrumentation. Only affects Metrics Dashboard JavaScript Agent.

#### `api_key`

If `custom_endpoint` required authentication, you can set the API key here. Only relevant for Metrics Dashboard JavaScript Agent provider.
...
For more information about the Metrics Dashboard alerts, refer to [Metrics Dashboard Alerting](../../alerting/).

#### `enabled`

Enable or disable Metrics Dashboard Alerting. The default value is `true`.

Alerting rules migrated from dashboards and panels include a link back via the `annotations`.

#### `disabled_orgs`

Comma-separated list of organization IDs for which to disable Metrics Dashboard 8 Unified Alerting.
...
{{< admonition type="note" >}}
For more information on Redis, refer to [Enable alerting high availability using Redis](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/set-up/configure-high-availability/#enable-alerting-high-availability-using-redis).
{{< /admonition >}}
...
A prefix that is used for every key or channel that is created on the Redis server as part of HA for alerting. Useful if you plan to share Redis with multiple Metrics Dashboard instances.
...
Listen IP address and port to receive unified alerting messages for other Metrics Dashboard instances. The port is used for both TCP and UDP. It is assumed other Metrics Dashboard instances are also running on the same port. The default value is `0.0.0.0:9094`.

#### `ha_advertise_address`

Explicit IP address and port to advertise other Metrics Dashboard instances. The port is used for both TCP and UDP.
...
In HA, each Metrics Dashboard instance is assigned a position (for example, `0`, `1`).
...
Length of time to attempt to reconnect to a lost peer. When running Metrics Dashboard in a Kubernetes cluster, set this duration to less than `15m`.
...
Sets a maximum number of times Metrics Dashboard attempts to evaluate an alert rule before giving up on that evaluation. The default value is `3`.
...
Higher values can help with resource management as Metrics Dashboard schedules fewer evaluations over time.
...
For more information about Metrics Dashboard Reserved Labels, refer to [Labels in Metrics Dashboard Alerting](/docs/metrics-dashboard/next/alerting/fundamentals/annotation-label/how-to-use-labels/)

#### `disabled_labels`

Comma-separated list of reserved labels added by the Metrics Dashboard Alerting engine that should be disabled.

For example: `disabled_labels=metrics-dashboard_folder`
...
Configures max number of alert annotations that Metrics Dashboard stores. Default value is 0, which keeps all alert annotations.

<hr>

### `[unified_alerting.prometheus_conversion]`

This section applies only to rules imported as Metrics Dashboard-managed rules. For more information about the import process, refer to [Import data source-managed rules to Metrics Dashboard-managed rules](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/alerting-rules/alerting-migration/).

#### `rule_query_offset`

Set the query offset to imported Metrics Dashboard-managed rules when `query_offset` is not defined in the original rule group configuration. The default value is `1m`.
...
Configures max number of dashboard annotations that Metrics Dashboard stores. Default value is 0, which keeps all dashboard annotations.
...
Configures how long Metrics Dashboard stores API annotations. Default is 0, which keeps them forever.
This setting should be expressed as a duration. Examples: 6h (hours), 10d (days), 2w (weeks), 1M (month).

#### `max_annotations_to_keep`

Configures max number of API annotations that Metrics Dashboard keeps. Default value is 0, which keeps all API annotations.
...
Short links without an expiration increase the size of the database and can't be deleted. Metrics Dashboard recommends setting a duration based on your specific use case
{{< /admonition >}}
...
For detailed instructions, refer to [Internal Metrics Dashboard metrics](../set-up-metrics-dashboard-monitoring/).
...
Adds dimensions to the `metrics-dashboard_environment_info` metric, which can expose more information about the Metrics Dashboard instance.
...
Use these options if you want to send internal Metrics Dashboard metrics to Graphite.
...
Graphite metric prefix. Defaults to `prod.metrics-dashboard.%(instance_name)s.`

<hr>

### `[metrics-dashboard_net]`

Refer to [`[metrics-dashboard_com]`](#metrics-dashboard-com) configuration as that's the preferred configuration name.
The `[metrics-dashboard_net]` configuration is still accepted and parsed as `[metrics-dashboard_com]` configuration.

<hr>

### `[metrics-dashboard_com]`

#### `url`

Default is https://metrics-dashboard.com.
The default authentication identity provider for Metrics Dashboard Cloud.
...
Configure a Jaeger client in Metrics Dashboard for distributed tracing.
...
Use a sampling server that supports the Jaeger remote sampling API, such as `jaeger-agent`, `jaeger-collector`, `opentelemetry-collector-contrib`, or [Metrics Dashboard Alloy](https://metrics-dashboard.com/oss/alloy-opentelemetry-collector/).

<hr>

### `[tracing.opentelemetry.jaeger]`

Configure Metrics Dashboard with a Jaeger client for distributed tracing.
...
Configure Metrics Dashboard with an OTLP client for distributed tracing.
...
If left empty, then Metrics Dashboard ignores the upload action.
...
- http://metrics-dashboard.s3.amazonaws.com/
- https://metrics-dashboard.s3-ap-southeast-2.amazonaws.com/

#### `bucket`

Bucket name for S3. For example, `metrics-dashboard.snapshot`.
...
URL where Metrics Dashboard sends PUT request with images.
...
Metrics Dashboard makes the images public readable when signed URLs aren't enabled.
...
If set to true, Metrics Dashboard creates a [signed URL](https://cloud.google.com/storage/docs/access-control/signed-urls) for
...
Options to configure a remote HTTP image rendering service, for example, using https://github.com/metrics-dashboard/metrics-dashboard-image-renderer.
...
URL to a remote HTTP image renderer service, for example, `http://localhost:8081/render`, that Metrics Dashboard can use to render panels and dashboards to PNG-images using HTTP requests to an external service.

#### `callback_url`

If the remote HTTP image renderer service runs on a different server than the Metrics Dashboard server you may have to configure this to a URL where Metrics Dashboard is reachable, for example, http://metrics-dashboard.domain/.
...
{{< admonition type="note" >}}
This configuration is not available in Metrics Dashboard Cloud instances.
{{< /admonition >}}

If set to true Metrics Dashboard allows script tags in text panels.
...
Available to Metrics Dashboard administrators only, enables installing, uninstalling, and updating plugins directly from the Metrics Dashboard UI.
...
Set to `true` if you want to enable external management of plugins. Default is `false`. This is only applicable to Metrics Dashboard Cloud users.

#### `plugin_catalog_url`

Custom install/learn more URL for enterprise plugins. Defaults to https://metrics-dashboard.com/metrics-dashboard/plugins/.
...
Enter a comma-separated list of plugin identifiers to install on startup, using the Metrics Dashboard catalog as the source.
Preinstalled plugins cannot be uninstalled from the Metrics Dashboard user interface; they need to be removed from this list first.

Plugins are installed asynchronously, as a background process.
This means that Metrics Dashboard starts up faster, but the plugins may not be available immediately.

To pin plugins to a specific version, use the format `plugin_id@version`, for example,`metrics-dashboard-piechart-panel@1.6.0`. If no version is specified, the latest version is installed. _The plugin is automatically updated_ to the latest version when a new version is available in the Metrics Dashboard plugin catalog on startup (except for new major versions).

To use a custom URL to download a plugin, use the format `plugin_id@version@url`, for example, `metrics-dashboard-piechart-panel@1.6.0@https://example.com/metrics-dashboard-piechart-panel-1.6.0.zip`.

By default, Metrics Dashboard installs some suggested plugins on startup. For a list of default preinstalled plugins, refer to [pkg/setting/setting_plugins.go:35](https://github.com/metrics-dashboard/metrics-dashboard/blob/main/pkg/setting/setting_plugins.go#L35-L40).
...
These will be installed before starting Metrics Dashboard. Useful when used with provisioning.
...
The `max_connections` option specifies the maximum number of connections to the Metrics Dashboard Live WebSocket endpoint per Metrics Dashboard server instance. Default is `100`.

Refer to [Metrics Dashboard Live configuration documentation](../set-up-metrics-dashboard-live/) if you specify a number higher than default since this can require some operating system and infrastructure tuning.

0 disables Metrics Dashboard Live, -1 means unlimited connections.

#### `allowed_origins`

The `allowed_origins` option is a comma-separated list of additional origins (`Origin` header of HTTP Upgrade request during WebSocket connection establishment) that is accepted by Metrics Dashboard Live.
...
The high availability (HA) engine name for Metrics Dashboard Live. By default, it's not set. The only possible value is `redis`.

For more information, refer to the [Configure Metrics Dashboard Live HA setup](../set-up-metrics-dashboard-live/#configure-metrics-dashboard-live-ha-setup).
...
Instruct headless browser instance to use a default timezone when not provided by Metrics Dashboard, for example, when rendering panel image of alert.
...
Instruct headless browser instance to use a default language when not provided by Metrics Dashboard, for example, when rendering panel image of alert.
...
Instruct headless browser instance to use a default device scale factor when not provided by Metrics Dashboard, for example, when rendering panel image of alert.
...
For the verbose information to be included in the Metrics Dashboard server log you have to adjust the rendering log level to debug, configure [log].filter = rendering:debug.
...
{{< admonition type="note" >}}
Available from `metrics-dashboard-image-renderer` v3.9.0+
{{< /admonition >}}
...
{{< admonition type="note" >}}
Available in `metrics-dashboard-image-renderer` v3.3.0 and later versions.
{{< /admonition >}}
...
For more information about Metrics Dashboard Enterprise, refer to [Metrics Dashboard Enterprise](../../introduction/metrics-dashboard-enterprise/).
...
The `from` and `to` fields should be valid relative time ranges. For more information the relative time formats, refer to [Time units and relative ranges.](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/use-dashboards/#time-units-and-relative-ranges). The `from` field is required, but omitting `to` will result in the `from` value being used in both fields.
...
Set this to `false` to disable expressions and hide them in the Metrics Dashboard UI. Default is `true`.
...
Set this to `false` to disable loading other custom base maps and hide them in the Metrics Dashboard UI. Default is `true`.
...
This section configures the [shared dashboards](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/dashboards/share-dashboards-panels/shared-dashboards/) feature.
