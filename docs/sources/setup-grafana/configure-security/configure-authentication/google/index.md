...
aliases:
  - ../../../auth/google/
description: Metrics Dashboard Google OAuth Guide
labels:
  products:
    - cloud
    - enterprise
...
1. Create a new project if you don't have one already.
   ...
   1. Fill out the requested information using the URL of your Metrics Dashboard Cloud instance.
...
1. Enter the following:
   ...
   - **Authorized JavaScript origins**: `https://<YOUR_METRICS_DASHBOARD_URL>`
   - **Authorized redirect URIs**: `https://<YOUR_METRICS_DASHBOARD_URL>/login/google`
   - Replace `<YOUR_METRICS_DASHBOARD_URL>` with the URL of your Metrics Dashboard instance.
     {{< admonition type="note" >}}
     The URL you enter is the one for your Metrics Dashboard instance home page, not your Metrics Dashboard Cloud portal URL.
     {{< /admonition >}}
1. Click Create
1. Copy the Client ID and Client Secret from the 'OAuth Client' modal

## Configure Google authentication client using the Metrics Dashboard UI

As a Metrics Dashboard Admin, you can configure Google OAuth client from within Metrics Dashboard using the Google UI. To do this, navigate to **Administration > Authentication > Google** page and fill in the form. If you have a current configuration in the Metrics Dashboard configuration file then the form will be pre-populated with those values otherwise the form will contain default values.
...
If you need to reset changes made in the UI back to the default values, click **Reset**. After you have reset the changes, Metrics Dashboard will apply the configuration from the Metrics Dashboard configuration file (if there is any configuration) or the default values.
...
```terraform
resource "metrics-dashboard_sso_settings" "google_sso_settings" {
...
## Configure Google authentication client using the Metrics Dashboard configuration file
...
### Enable Google OAuth in Metrics Dashboard

Specify the Client ID and Secret in the [Metrics Dashboard configuration file](../../../configure-metrics-dashboard/#configuration-file-location). For example:
...
Restart the Metrics Dashboard backend. You should now see a Google login button
...
{{< admonition type="note" >}}
You can disable PKCE in Metrics Dashboard by setting `use_pkce` to `false` in the`[auth.google]` section.
{{< /admonition >}}
...
By default, Metrics Dashboard includes the `access_type=offline` parameter in the authorization request to request a refresh token.

Refresh token fetching and access token expiration check is enabled by default for the Google provider since Metrics Dashboard v10.1.0. If you would like to disable access token expiration check then set the `use_refresh_token` configuration value to `false`.
...
1. Add the `https://www.googleapis.com/auth/cloud-identity.groups.readonly` scope to your Metrics Dashboard `[auth.google]` configuration:
...
1. Configure team sync in your Metrics Dashboard team's `External group sync` tab.
   The external group ID for a Google group is the group's email address, such as `dev@metrics-dashboard.com`.
...
{{< admonition type="note" >}}
Add the `https://www.googleapis.com/auth/cloud-identity.groups.readonly` scope to your Metrics Dashboard `[auth.google]` scopes configuration to retrieve groups.
{{< /admonition >}}
...
{{< admonition type="note" >}}
By default the `skip_org_role_sync` option is enabled. The `skip_org_role_sync` option defaults to false in Metrics Dashboard v10.3.0 and later versions.
{{< /admonition >}}
...
In this example, the user with email `admin@company.com` is granted the `Admin` organization role as well as the Metrics Dashboard server admin role.
All other users are granted the `Viewer` role.

```ini
allow_assign_metrics-dashboard_admin = true
skip_org_role_sync = false
role_attribute_path = email=='admin@company.com' && 'Metrics DashboardAdmin' || 'Viewer'
```
...
| `name`                       | No       | Yes                | Name that refers to the Google authentication from the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                                  | `Google`                                           |
| `icon`                       | No       | Yes                | Icon used for the Google authentication in the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                                          | `google`                                           |
...
| `allow_sign_up`              | No       | Yes                | Controls Metrics Dashboard user creation through the Google login. Only existing Metrics Dashboard users can log in with Google if set to `false`.                                                                                                                                                                                                                                                                                                                                                                  | `true`                                             |
...
| `validate_hd`                | No       | Yes                | Set to `false` to disable the validation of the `hd` parameter from the Google ID token. For more informatiion, refer to [Enable Google OAuth in Metrics Dashboard](#enable-google-oauth-in-metrics-dashboard).                                                                                                                                                                                                                                                                                                     | `true`                                             |
...
| `use_refresh_token`          | No       | Yes                | Enables the use of refresh tokens and checks for access token expiration. When enabled, Metrics Dashboard automatically adds the `promp=consent` and `access_type=offline` parameters to the authorization request.                                                                                                                                                                                                                                                                                       | `true`                                             |
