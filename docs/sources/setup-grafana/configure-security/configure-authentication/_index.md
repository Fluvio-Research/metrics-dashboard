...
aliases:
  - ../../auth/
  - ../../auth/overview/
description: Learn about all the ways in which you can configure Metrics Dashboard to authenticate
  users.
labels:
  products:
    - cloud
...
Metrics Dashboard provides many ways to authenticate users. Some authentication integrations also enable syncing user permissions and org memberships.

The following table shows all supported authentication methods and the features available for them. [Team sync](../configure-team-sync/) and [active sync](enhanced-ldap/#active-ldap-synchronization) are only available in Metrics Dashboard Enterprise.

| Authentication method               | Multi Org Mapping | Enforce Sync | Role Mapping | Metrics Dashboard Admin Mapping | Team Sync | Allowed groups | Active Sync | Skip OrgRole mapping | Auto Login | Single Logout | SCIM support |
| :---------------------------------- | :---------------- | :----------- | :----------- | :-------------------- | :-------- | :------------- | :---------- | :------------------- | :--------- | :------------ | :----------- |
...
| [Basic auth](metrics-dashboard/)              | yes               | N/A          | yes          | yes                   | N/A       | N/A            | N/A         | N/A                  | N/A        | N/A           | N/A          |
...
| [metrics-dashboard.com OAuth](metrics-dashboard-cloud/) | no                | no           | yes          | no                    | N/A       | N/A            | N/A         | yes                  | yes        | yes           | N/A          |
...
**Metrics Dashboard Admin Mapping:** Able to map a user’s admin role in the default org
...
**Single Logout:** Logging out from Metrics Dashboard also logs you out of provider session
...
Metrics Dashboard allows you to configure more than one authentication provider, however it is not possible to configure the same type of authentication provider twice.
...
> Note: Metrics Dashboard does not support multiple identity providers resolving the same user. Ensure there are no user account overlaps between the different providers

In scenarios where you have multiple identity providers of the same type, there are a couple of options:

- Use different Metrics Dashboard instances each configured with a given identity provider.
...
If users want to use the same email address with multiple identity providers (for example, Metrics Dashboard.Com OAuth and Google OAuth), you can configure Metrics Dashboard to use the email address as the unique identifier for the user. This is done by enabling the `oauth_allow_insecure_email_lookup` option, which is disabled by default. Please note that enabling this option can lower the security of your Metrics Dashboard instance. If you enable this option, you should also ensure that the `Allowed organization`, `Allowed groups` and `Allowed domains` settings are configured correctly to prevent unauthorized access.

To enable this option, refer to the [Enable email lookup](#enable-email-lookup) section.

## Multi-factor authentication (MFA/2FA)

Metrics Dashboard and the Metrics Dashboard Cloud portal currently do not include built-in support for multi-factor authentication (MFA).

We strongly recommend integrating an external identity provider (IdP) that supports MFA, such as Okta, Azure AD, or Google Workspace. By configuring your Metrics Dashboard instances to use an external IdP, you can leverage MFA to protect your accounts and resources effectively.

## Login and short-lived tokens

> The following applies when using Metrics Dashboard's basic authentication, LDAP (without Auth proxy) or OAuth integration.

Metrics Dashboard uses short-lived tokens as a mechanism for verifying authenticated users.
...
This means that a user can close a Metrics Dashboard window and return before `now + login_maximum_inactive_lifetime_duration` to continue their session.
...
When using SSO (Single Sign-On) authentication methods, Metrics Dashboard handles sessions differently based on the configuration:

### OAuth/OpenID Connect

- Without refresh tokens (default):
  - Metrics Dashboard creates a session valid for up to `login_maximum_lifetime_duration` (default: 30 days).
  - During this time, the session remains valid even if the user loses access at the IdP.
- With refresh tokens enabled:
  - The user receives a JWT refresh token. When the JWT expires and the refresh token is used to obtain a new token, Metrics Dashboard will revalidate access with the IdP.
...
- After successful SAML authentication, Metrics Dashboard creates a session with the default session lifetime.
...
# Login cookie name
login_cookie_name = metrics-dashboard_session
...
By default, Metrics Dashboard identifies users based on the unique ID provided by the identity provider (IdP).
...
**Important note**: While it is possible to configure Metrics Dashboard to allow email-based user lookups, we strongly recommend against this approach in most cases due to potential security risks.
...
Available in [Metrics Dashboard Enterprise](../../../introduction/metrics-dashboard-enterprise/) and [Metrics Dashboard Cloud](../../../introduction/metrics-dashboard-cloud/) since Metrics Dashboard v10.4.
{{< /admonition >}}

```
curl --request PUT \
  --url http://{slug}.metrics-dashboard.com/api/admin/settings \
...
1. Add `disableAutoLogin=true` as a query parameter to your Metrics Dashboard URL.
   - Example: `metrics-dashboard.example.net/login?disableAutoLogin=true` or `metrics-dashboard.example.net/login?disableAutoLogin`
...
URL to redirect the user to after signing out from Metrics Dashboard. This can for example be used to enable signout from an OAuth provider.
...
a Metrics Dashboard admin user, you can also do the same for any user from the Server Admin / Edit User view.

### Protected roles

{{< admonition type="note" >}}
Available in [Metrics Dashboard Enterprise](../../../introduction/metrics-dashboard-enterprise/) and [Metrics Dashboard Cloud](../../../introduction/metrics-dashboard-cloud/).
{{< /admonition >}}

By default, after you configure an authorization provider, Metrics Dashboard will adopt existing users into the new authentication scheme. For example, if you have created a user with basic authentication having the login `jsmith@example.com`, then set up SAML authentication where `jsmith@example.com` is an account, the user's authentication type will be changed to SAML if they perform a SAML sign-in.
