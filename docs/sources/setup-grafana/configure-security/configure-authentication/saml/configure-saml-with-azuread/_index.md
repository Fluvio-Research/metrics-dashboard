...
Metrics Dashboard supports user authentication through Microsoft Entra ID. This topic shows you how to configure SAML authentication in Metrics Dashboard with [Entra ID](https://www.microsoft.com/en-us/security/business/identity-access/microsoft-entra-id).

{{< admonition type="note" >}}
If an Entra ID user belongs to more than 150 groups, a Graph API endpoint is used instead.

Metrics Dashboard versions 11.1 and below, do not support fetching the groups from the Graph API endpoint. As a result, users with more than 150 groups will not be able to retrieve their groups. Instead, it is recommended that you use the Azure AD connector.

As of Metrics Dashboard 11.2, the SAML integration offers a mechanism to retrieve user groups from the Graph API.
...
Ensure you have permission to administer SAML authentication. For more information about roles and permissions in Metrics Dashboard, refer to [Roles and permissions](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/roles-and-permissions/).
...
In order to validate Entra ID users with Metrics Dashboard, you need to configure the SAML Toolkit application endpoints by creating a new SAML integration in the Entra ID organization.

> For the following configuration, we will use `https://localhost` as the Metrics Dashboard URL. Replace it with your Metrics Dashboard URL.
...
1. Select **Add**.
1. Copy the value of the secret. This value is used in the `client_secret` field in the [SAML configuration](https://metrics-dashboard.com/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-security/configure-authentication/saml/saml-configuration-options/).
...
While an Entra ID tenant can be configured in Metrics Dashboard via SAML, some additional information is only accessible via the Graph API. To retrieve this information, create a new application in Entra ID and grant it the necessary permissions.

> [Entra ID SAML limitations](https://learn.microsoft.com/en-us/entra/identity-platform/id-token-claims-reference#groups-overage-claim)

> For the following configuration, the URL `https://localhost` will be used as the Metrics Dashboard URL. Replace it with your Metrics Dashboard instance URL.
...
{{< figure src="/media/docs/metrics-dashboard/saml/graph-api-app-permissions.png" caption="Screen shot of the permissions listed in Entra ID for the App registration" >}}
