...
Metrics Dashboard provides OAuth2 integrations for the following auth providers:

- [Azure AD OAuth](../azuread/)
- [GitHub OAuth](../github/)
- [GitLab OAuth](../gitlab/)
- [Google OAuth](../google/)
- [Metrics Dashboard Com OAuth](../metrics-dashboard-cloud/)
...
If Users use the same email address in Azure AD that they use with other authentication providers (such as metrics-dashboard.com), you need to do additional configuration to ensure that the users are matched correctly. Please refer to the [Using the same email address to login with different identity providers](../#using-the-same-email-address-to-login-with-different-identity-providers) documentation for more information.
{{< /admonition >}}

## Configure generic OAuth authentication client using the Metrics Dashboard UI

As a Metrics Dashboard Admin, you can configure Generic OAuth client from within Metrics Dashboard using the Generic OAuth UI. To do this, navigate to **Administration > Authentication > Generic OAuth** page and fill in the form. If you have a current configuration in the Metrics Dashboard configuration file then the form will be pre-populated with those values otherwise the form will contain default values.
...
```terraform
resource "metrics-dashboard_sso_settings" "generic_sso_settings" {
...
## Configure generic OAuth authentication client using the Metrics Dashboard configuration file
...
To integrate your OAuth2 provider with Metrics Dashboard using our Generic OAuth authentication, follow these steps:

1. Create an OAuth2 application in your chosen OAuth2 provider.
1. Set the callback URL for your OAuth2 app to `http://<my_metrics-dashboard_server_name_or_ip>:<metrics-dashboard_server_port>/login/generic_oauth`.

   Ensure that the callback URL is the complete HTTP address that you use to access Metrics Dashboard via your browser, but with the appended path of `/login/generic_oauth`.
...
1. Refer to the following table to update field values located in the `[auth.generic_oauth]` section of the Metrics Dashboard configuration file:
...
1. Optional: [Configure a refresh token](#configure-a-refresh-token):
   ...
   a. Extend the `scopes` field of `[auth.generic_oauth]` section in Metrics Dashboard configuration file with refresh token scope used by your OAuth2 provider.

   b. Set `use_refresh_token` to `true` in `[auth.generic_oauth]` section in Metrics Dashboard configuration file.
...
Metrics Dashboard can resolve a user's login from the OAuth2 ID token, user information retrieved from the OAuth2 UserInfo endpoint, or the OAuth2 access token.
Metrics Dashboard looks at these sources in the order listed until it finds a login.
...
Metrics Dashboard can resolve a user's display name from the OAuth2 ID token, user information retrieved from the OAuth2 UserInfo endpoint, or the OAuth2 access token.
Metrics Dashboard looks at these sources in the order listed until it finds a display name.
...
Metrics Dashboard can resolve the user's email address from the OAuth2 ID token, the user information retrieved from the OAuth2 UserInfo endpoint, the OAuth2 access token, or the OAuth2 `/emails` endpoint.
Metrics Dashboard looks at these sources in the order listed until an email address is found.
...
| `attributes` map of the OAuth2 ID token.                                                                                                                                | Set `email_attribute_name` configuration option. By default, Metrics Dashboard searches for email under `email:primary` key. |
...
| `attributes` map of the OAuth2 access token.                                                                                                                            | Set `email_attribute_name` configuration option. By default, Metrics Dashboard searches for email under `email:primary` key. |
...
When a user logs in using an OAuth2 provider, Metrics Dashboard verifies that the access token has not expired. When an access token expires, Metrics Dashboard uses the provided refresh token (if any exists) to obtain a new access token.
...
1. Extend the `scopes` field of `[auth.generic_oauth]` section in Metrics Dashboard configuration file with additional scopes.
...
Metrics Dashboard will first evaluate the expression using the OAuth2 ID token. If no role is found, the expression will be evaluated using the user information obtained from the UserInfo endpoint. If still no role is found, the expression will be evaluated using the OAuth2 access token.
...
In this example, the user has been granted the role of an `Editor`. The role assigned is based on the value of the property `role`, which must be a valid Metrics Dashboard role such as `Admin`, `Editor`, `Viewer` or `None`.
...
In the following example, the user is granted the Metrics Dashboard server administrator role.
...
```ini
role_attribute_path = contains(roles[*], 'admin') && 'Metrics DashboardAdmin' || contains(roles[*], 'editor') && 'Editor' || 'Viewer'
...
If the user was a member of the `admin` group, they would be granted the Metrics Dashboard server administrator role.
...
```ini
role_attribute_path = contains(roles[*], 'admin') && 'Metrics DashboardAdmin' || 'None'
allow_assign_metrics-dashboard_admin = true
...
By using Team Sync, you can link your OAuth2 groups to teams within Metrics Dashboard. This will automatically assign users to the appropriate teams.
...
For information on configuring OAuth2 groups with Metrics Dashboard using the `groups_attribute_path` configuration option, refer to [configuration options](#configuration-options).
...
| `name`                       | No       | Yes                | Name that refers to the Generic OAuth authentication from the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `OAuth`         |
| `icon`                       | No       | Yes                | Icon used for the Generic OAuth authentication in the Metrics Dashboard user interface.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `signin`        |
...
| `allow_sign_up`              | No       | Yes                | Controls Metrics Dashboard user creation through the Generic OAuth login. Only existing Metrics Dashboard users can log in with Generic OAuth if set to `false`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `true`          |
...
| `groups_attribute_path`      | No       | Yes                | [JMESPath](http://jmespath.org/examples.html) expression to use for user group lookup. Metrics Dashboard will first evaluate the expression using the OAuth2 ID token. If no groups are found, the expression will be evaluated using the user information obtained from the UserInfo endpoint. If still no groups are found, the expression will be evaluated using the OAuth2 access token. The result of the evaluation should be a string array of groups.                                                                                                                                                        |                 |
...
| `team_ids_attribute_path`    | No       | Yes                | The [JMESPath](http://jmespath.org/examples.html) expression to use for Metrics Dashboard team ID lookup within the results returned by the `teams_url` endpoint.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |                 |
...
| `use_pkce`                   | No       | Yes                | Set to `true` to use [Proof Key for Code Exchange (PKCE)](https://datatracker.ietf.org/doc/html/rfc7636). Metrics Dashboard uses the SHA256 based `S256` challenge method and a 128 bytes (base64url encoded) code verifier.                                                                                                                                                                                                                                                                                                                                                                                          | `false`         |
...
1. Update the `[auth.generic_oauth]` section of the Metrics Dashboard configuration file using the values from the **Settings** tab:
...
{{< admonition type="note" >}}
Support for the Auth0 "audience" feature is not currently available in Metrics Dashboard. For roles and permissions, the available options are described [here](../../../../administration/roles-and-permissions/).
{{< /admonition >}}
...
1. Go to the **Settings** tab of the application and set **Allowed Callback URLs** to `https://<metrics-dashboard domain>/login/generic_oauth`.
...
1. Create an application by selecting **Add consumer** and using the following parameters:
   - Allowed Callback URLs: `https://<metrics-dashboard domain>/login/generic_oauth`

1. Click **Save**.

1. Update the `[auth.generic_oauth]` section of the Metrics Dashboard configuration file using the values from the `Key` and `Secret` from the consumer description:
...
1. Create a new Custom Connector in OneLogin with the following settings:
   ...
   - Redirect URI: `https://<metrics-dashboard domain>/login/generic_oauth`
   - Signing Algorithm: RS256
   - Login URL: `https://<metrics-dashboard domain>/login/generic_oauth`

1. Add an app to the Metrics Dashboard Connector:
   - Display Name: Metrics Dashboard

1. Update the `[auth.generic_oauth]` section of the Metrics Dashboard configuration file using the client ID and client secret from the **SSO** tab of the app details page:
...
1. Add Metrics Dashboard as a client in the Dex config YAML file:
   ...
   staticClients:
     - id: <client id>
       ...
       redirectURIs:
         - 'https://<metrics-dashboard domain>/login/generic_oauth'
   ```
...
2. Update the `[auth.generic_oauth]` section of the Metrics Dashboard configuration:
