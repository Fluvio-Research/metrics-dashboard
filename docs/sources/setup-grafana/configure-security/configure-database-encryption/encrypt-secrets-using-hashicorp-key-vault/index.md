...
description: Learn how to use Hashicorp Vault to encrypt secrets in the Metrics Dashboard database.
labels:
  products:
    - cloud
    - enterprise
...
You can use an encryption key from Hashicorp Vault to encrypt secrets in the Metrics Dashboard database.
...
4. From within Metrics Dashboard, turn on envelope encryption.

5. Add your Hashicorp Vault details to the Metrics Dashboard configuration file; depending on your operating system, is usually named `metrics-dashboard.ini`:
   ...
   An example of a Hashicorp Vault provider section in the `metrics-dashboard.ini` file is as follows:
   ...
   # Key ring name
   ;key_ring = metrics-dashboard-encryption-key
...
6. Update the `[security]` section of the `metrics-dashboard.ini` configuration file with the new Encryption Provider key that you created:
...
7. [Restart Metrics Dashboard](/docs/metrics-dashboard/latest/installation/restart-metrics-dashboard/).

8. (Optional) From the command line and the root directory of Metrics Dashboard Enterprise, re-encrypt all of the secrets within the Metrics Dashboard database with the new key using the following command:
