...
- [Loki](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/loki/)
- [Elasticsearch](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/elasticsearch/)
- [Cloudwatch](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/aws-cloudwatch/)
- [InfluxDB](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/influxdb/)
- [Azure Monitor](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/azure-monitor/)
...
If the log doesn't have a specified level label, Metrics Dashboard attempts to determine if its content matches any of the supported expressions.
Refer to the following table for more information. The log level is always determined by the first match. If Metrics Dashboard isn't able to infer a log level field, it gets visualized as an unknown log level.
...
Metrics Dashboard provides data links or correlations, allowing you to convert any part of a log message into an internal or external link. These links enable you to navigate to related data or external resources, offering a seamless and convenient way to explore additional information.
...
Toggle **Wrap lines** if you encounter long lines of text that make it difficult to read and analyze the context around log entries. By enabling this toggle, Metrics Dashboard automatically wraps long lines of text to fit within the visible width of the viewer, making the log entries easier to read and understand.
...
Linking log lines in Metrics Dashboard allows you to quickly navigate to specific log entries for detailed and precise analysis. Click **Copy shortlink** to generate and copy a [short URL](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/developers/http_api/short_url/) that provides direct access to the exact log entry within an absolute time range. When you open the link, Metrics Dashboard automatically scrolls to the corresponding log line and highlights it in blue, making it easy to identify and focus on relevant information.
...
For example, after switching to the Loki data source, the Prometheus query `metrics-dashboard_alerting_active_alerts{job="metrics-dashboard"}` changes to `{job="metrics-dashboard"}`. This will retrieve a set of logs within the specified time range, which can be searched using grep or text search.
