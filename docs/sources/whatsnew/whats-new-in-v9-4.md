---
description: Learn about new and updated features in Metrics Dashboard v9.4
keywords:
  - metrics-dashboard
  - new
  - documentation
...
labels:
  products:
    ...
    - enterprise
    - oss
title: What's new in Metrics Dashboard v9.4
weight: -35
---

# What's new in Metrics Dashboard v9.4

Welcome to Metrics Dashboard 9.4! Read on to learn about changes to search and navigation, dashboards and visualizations, and authentication and security. For even more detail about all the changes in this release, refer to the [changelog](https://github.com/metrics-dashboard/metrics-dashboard/blob/main/CHANGELOG.md).
...
The command palette has been updated to provide a more efficient way to navigate Metrics Dashboard. You can now search and access all pages and recent dashboards, making it easier to perform tasks without taking your hands off the keyboard.
...
{{< figure src="/media/docs/metrics-dashboard/screenshot-metrics-dashboard-94-command-palette.png" max-width="750px" caption="Metrics Dashboard command palette" >}}

### New navigation

_Generally available on Metrics Dashboard Cloud, and available to preview using the `topnav` [feature toggle](../../setup-metrics-dashboard/configure-metrics-dashboard/#feature_toggles) in all editions of Metrics Dashboard._

The navigation in Metrics Dashboard has been updated with a new design and an improved structure to make it easier for you to access the data you need. With this update, you'll be able to quickly navigate between features, giving you full visibility into the health of your systems.

The new navigation is gradually rolling out to all users on Metrics Dashboard Cloud. If you’re using Metrics Dashboard Open Source and Enterprise, you can enable this feature using the `topnav` [feature toggle](../../setup-metrics-dashboard/configure-metrics-dashboard/#feature_toggles).

> **Note:** The Metrics Dashboard documentation has not yet been updated to reflect changes to the navigation.

> **Note:** Plugin developers should refer to [the migration guide](/developers/plugin-tools/migration-guides/update-from-metrics-dashboard-versions/migrate-9_3_x-to-9_4_x#new-navigation-layout-is-supported) to upgrade their plugins to work seamlessly with the new navigation layout.
...
_Available to preview using the `newPanelChromeUI` [feature toggle](../../setup-metrics-dashboard/configure-metrics-dashboard/#feature_toggles) in all editions of Metrics Dashboard_.
...
Metrics Dashboard’s new panel is available only for React-based panels; no Angular-based panels are redesigned. For example, Angular-based panel will still have the old Graph and Table visualizations.

{{< video-embed src="/media/docs/metrics-dashboard/screen-recording-panel-header-redesign-whats-new-9-4.mp4" max-width="750px" caption="Panel header redesign" >}}
...
_Available to preview using the `datasourceOnboarding` [feature toggle](../../setup-metrics-dashboard/configure-metrics-dashboard/#feature_toggles)._
...
{{< figure src="/media/docs/metrics-dashboard/screenshot-datasource-connection-onboarding-whats-new-9-4.png" max-width="750px" caption="Admin view of data source connection page on dashboard creation" >}}
...
{{< figure src="/media/docs/metrics-dashboard/logs-loki-query-validation-whats-new-9-4.png" max-width="750px" caption="Loki query validation" >}}
...
{{< figure src="/media/docs/metrics-dashboard/logs-sample-whats-new-9-4.png" max-width="750px" caption="Logs sample in Explore" >}}

## Canvas panel

_Available in **beta** in all editions of Metrics Dashboard_
...
While Metrics Dashboard integrates with many different auth providers, we have received requests for a feature that enables you to bypass organization role synchronization for individual providers rather than for all configured providers. This option is now available for users who want to be able to use Metrics Dashboard to manage their org roles.
...
Learn more about Oauth in our [Oauth configuration guide](../../setup-metrics-dashboard/configure-security/configure-authentication/generic-oauth/).

### RBAC support for Metrics Dashboard OnCall plugin

We're rolling out RBAC support to Metrics Dashboard plugins, with Metrics Dashboard OnCall being the first plugin to fully support RBAC.
Previously, Metrics Dashboard OnCall relied on the Metrics Dashboard basic roles (for example, Viewer, Editor, and Admin) for authorization within the plugin.

Before RBAC support in Metrics Dashboard OnCall, it was only possible to allow your organization's users to either view everything, edit everything, or be an admin (which allowed edit access plus a few additional behaviors). With this new functionality, organizations will be able to harness fine-grained access control within Metrics Dashboard OnCall.

For example, you can assign a Viewer basic role to a user in your organization (users must still have a basic role assigned) and also assign them the new Metrics Dashboard OnCall RBAC role of **Schedules Editor**. This assignment enables the user to view everything in Metrics Dashboard OnCall, and edit OnCall schedules.
...
Learn more about SAML setup in our [SAML configuration guide](../../setup-metrics-dashboard/configure-security/configure-authentication/saml/).

## Auditing and Usage Insights: Support for Loki multi-tenancy

_This feature is available for Enterprise customers_

This feature adds support to push analytics events and auditing logs to Loki with multi-tenancy mode, by specifying a tenant id. Learn more about [auditing](../../setup-metrics-dashboard/configure-security/audit-metrics-dashboard/) and [usage insights](../../setup-metrics-dashboard/configure-security/export-logs/) in our docs.
...
{{< figure src="/media/docs/metrics-dashboard/FormatReportScheduler9.4.png" max-width="750px" caption="Scale factor feature in Report format page" >}}

{{< figure src="/media/docs/metrics-dashboard/FormatReportShare9.4.png" max-width="750px" caption="Scale factor feature in Share functionality" >}}

Learn more about reporting in our [documentation](../../dashboards/create-reports/)

## Alerting

We've made major improvements to Metrics Dashboard Alerts, from new contact points and search options to improved workflows between Alerting and OnCall. For all the details, refer to our [Alerting documentation](../../alerting/).
...
#### View Metrics Dashboard OnCall contact point
...
{{< figure src="/media/docs/alerting/on-call-contact-point.png" max-width="750px" caption="View Metrics Dashboard OnCall contact point" >}}
...
We've added Discord as a contact point receiver for Metrics Dashboard Cloud alert rules.
...
Metrics Dashboard Alerting is now compatible with AWS Aurora, but does not provide technical support for it.

## Enterprise Datasources

We've made improvements to all [Enterprise Datasources](../../introduction/metrics-dashboard-enterprise/#enterprise-data-sources), fixing small bugs, and updating libraries. We've also added many new features and support for additional APIs. Refer to each datasource's documentation and the change log for additional information.
...
There are no known breaking changes associated with this version of Metrics Dashboard.
