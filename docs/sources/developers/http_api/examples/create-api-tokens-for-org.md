---
aliases:
  - ../../../http_api/create-api-tokens-for-org/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/http_api/create-api-tokens-for-org/
  - ../../../tutorials/api_org_token_howto/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/tutorials/api_org_token_howto/
canonical: /docs/metrics-dashboard/latest/developers/http_api/examples/create-api-tokens-for-org/
keywords:
  - metrics-dashboard
  - tutorials
  - API
...
Use the Metrics Dashboard API to set up new Metrics Dashboard organizations or to add dynamically generated dashboards to an existing organization.

## Authentication

There are two authentication methods to access the API:

- Basic authentication: A Metrics Dashboard Admin user can access some parts of the Metrics Dashboard API through basic authentication.
...
1. [Create the org](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/http_api/org/#create-organization). Here is an example using curl:
...
1. Optional step. If the org was created previously and/or step 3 fails then first [add your Admin user to the org](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/http_api/org/#add-user-in-organization):
...
1. [Switch the org context for the Admin user to the new org](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/http_api/user/#switch-user-context-for-signed-in-user):
...
1. [Add a dashboard](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/http_api/dashboard/#create-update-dashboard) using the key (or bearer token as it is also called):
...
> **Note:** If you export a dashboard for sharing externally using the Share > Export menu in the Metrics Dashboard UI, you cannot import that dashboard. Instead, click **View JSON** and save it to a file or fetch the JSON output through the API.
