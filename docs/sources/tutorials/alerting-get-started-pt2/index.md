...
tags:
  - beginner
title: Get started with Metrics Dashboard Alerting - Multi-dimensional alerts and how to route them
weight: 63
killercoda:
  title: Get started with Metrics Dashboard Alerting - Multi-dimensional alerts and how to route them
  description: Learn to use alert instances and route notifications by labels to contacts.
...
This tutorial is a continuation of the [Metrics Dashboard Alerting - Create and receive your first alert](http://www.metrics-dashboard.com/tutorials/alerting-get-started/) tutorial.

In this guide, we dig into more complex yet equally fundamental elements of Metrics Dashboard Alerting: **alert instances** and **notification policies**.
...
- **Interactive learning environment**
  - Alternatively, you can try out this example in our interactive learning environment: [Get started with Metrics Dashboard Alerting - Alert routing](https://killercoda.com/metrics-dashboard-labs/course/metrics-dashboard/alerting-get-started-pt2/). It's a fully configured environment with all the dependencies already installed.
...
An [alert instance](https://metrics-dashboard.com/docs/metrics-dashboard/latest/alerting/fundamentals/#alert-instances) is an event that matches a metric returned by an alert rule query.

Let's consider a scenario where you're monitoring website traffic using Metrics Dashboard. You've set up an alert rule to trigger an alert instance if the number of page views exceeds a certain threshold (more than `1000` page views) within a specific time period, say, over the past `5` minutes.
...
In the above diagram, alert instances and notification policies are matched by labels. For instance, the label `team=operations` matches the alert instance “**Pod stuck in CrashLoop**” and “**Disk Usage -80%**” to child policies that send alert notifications to a particular contact point (operations@metrics-dashboard.com).
...
1. In your browser, **sign in** to your Metrics Dashboard Cloud account.

   OSS and interactive learning environment users: To log in, navigate to [http://localhost:3000](http://localhost:3000), where Metrics Dashboard should be running.
...
1. From the **Contact point** drop-down, choose **Webhook**.
   {{< admonition type="note" >}}
   If you don’t have any contact points, add a [Contact point](https://metrics-dashboard.com/tutorials/alerting-get-started/#create-a-contact-point).
   {{</ admonition >}}
...
{{< figure alt="Screenshot showing a preview of a query in Metrics Dashboard that returns two alert instances, including the query results and relevant alert details" src="/media/docs/alerting/firing-instances.png" max-width="1200px" caption="Preview of a query returning two alert instances in Metrics Dashboard." >}}
...
In the [life cycle](http://metrics-dashboard.com/docs/metrics-dashboard/next/alerting/fundamentals/alert-rule-evaluation/) of alert instances, when an alert condition (threshold) is not met, the alert instance state is **Normal**. Similarly, when the condition is breached (for longer than the pending period, which in this tutorial will be 0), the alert instance state switches back to **Alerting**, which means that the alert rule state is **Firing**, and a notification is sent.
...
1. Click **Preview routing**. Based on the existing labels, you should see a preview of what policies are matching with the alerts. There should be two alert instances matching the labels that were previously setup in each notification policy: `device=desktop`, `device=mobile`.
   ...
   These [types of labels](https://metrics-dashboard.com/docs/metrics-dashboard/latest/alerting/fundamentals/alert-rules/annotation-label/#label-types) are generated by the data source query and they can be leveraged to match our notification policies without needing to manually add them to the alert rule.
...
Now that the alert rule has been configured, you should receive alert [notifications](http://metrics-dashboard.com/docs/metrics-dashboard/next/alerting/fundamentals/alert-rule-evaluation/state-and-health/#notifications) in the contact point whenever the alert triggers and gets resolved. In our example, each alert instance should be routed separately as we configured labels to match notification policies. Once the evaluation interval has concluded (1m), you should receive an alert notification in the Webhook endpoint.
...
In this tutorial, you have learned how Metrics Dashboard Alerting can route individual alert instances using the labels generated by the data-source query and match these labels with notification policies, which in turn routes alert notifications to specific contact points.

If you run into any problems, you are welcome to post questions in our [Metrics Dashboard Community forum](https://community.metrics-dashboard.com/).

## Learn more in [Metrics Dashboard Alerting: Group alert notifications](http://www.metrics-dashboard.com/tutorials/alerting-get-started-pt3/)

<!-- INTERACTIVE ignore START -->

{{< admonition type="tip" >}}

In [Get started with Metrics Dashboard Alerting: Group alert notifications](http://www.metrics-dashboard.com/tutorials/alerting-get-started-pt3/) you learn how to group alert notifications effectively.
