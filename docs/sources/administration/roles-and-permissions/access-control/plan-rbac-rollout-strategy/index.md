...
title: Plan your Metrics Dashboard RBAC rollout strategy
weight: 20
refs:
  ...
  rbac-fixed-basic-role-definitions-fixed-role-definitions:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/roles-and-permissions/access-control/rbac-fixed-basic-role-definitions/#fixed-role-definitions
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/account-management/authentication-and-permissions/access-control/rbac-fixed-basic-role-definitions/#fixed-role-definitions
...
- **Assign roles to users** when you have a one-off scenario where a small number of users require access to a resource or when you want to assign temporary access. If you have a large number of users, this approach can be difficult to manage as you scale your use of Metrics Dashboard. For example, a member of your IT department might need the `fixed:licensing:reader` and `fixed:licensing:writer` roles so that they can manage your Metrics Dashboard Enterprise license.
...
You can take advantage of your current authentication provider to manage user and team permissions in Metrics Dashboard. When you map users and teams to SAML and LDAP groups, you can synchronize those assignments with Metrics Dashboard.

For example:

1. Map SAML, LDAP, or Oauth roles to Metrics Dashboard basic roles (viewer, editor, or admin).

2. Use the Metrics Dashboard Enterprise team sync feature to synchronize teams from your SAML, LDAP, or Oauth provider to Metrics Dashboard. For more information about team sync, refer to [Team sync](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-security/configure-team-sync/).

3. Within Metrics Dashboard, assign RBAC permissions to users and teams.
...
- **Modify basic roles** when Metrics Dashboard's definitions of what viewers, editors, and admins can do does not match your definition of these roles. You can add or remove permissions from any basic role.

  {{< admonition type="note" >}}
  Changes that you make to basic roles impact the role definition for all [organizations](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/administration/organization-management/) in the Metrics Dashboard instance. For example, when you add the `fixed:users:writer` role's permissions to the viewer basic role, all viewers in any org in the Metrics Dashboard instance can create users within that org.
  {{< /admonition >}}
...
- **Metrics Dashboard UI:** Use the Metrics Dashboard UI when you want to assign a limited number of RBAC roles to users and teams. The UI contains a role picker that you can use to select roles.
- **Metrics Dashboard HTTP API:** Use the Metrics Dashboard HTTP API if you would like to automate role assignment.
- **Terraform:** Use Terraform to assign and manage user and team role assignments if you use Terraform for provisioning.
- **Metrics Dashboard provisioning:** Metrics Dashboard provisioning provides a robust approach to assigning, removing, and deleting roles. Within a single YAML file you can include multiple role assignment and removal entries.

## Permissions scenarios

We've compiled the following permissions rollout scenarios based on current Metrics Dashboard implementations.
...
1. In Metrics Dashboard, create a team with the name `Internal employees`.
1. Assign the `fixed:datasources:explorer` role to the `Internal employees` team.
1. Add internal employees to the `Internal employees` team, or map them from a SAML, LDAP, or Oauth team using [Team Sync](/docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/setup-metrics-dashboard/configure-security/configure-team-sync/).
...
To see an alert rule in Metrics Dashboard, the user must have read access to the folder that stores the alert rule, permission to read alerts in the folder, and permission to query all data sources that the rule uses.
...
By default, only a Metrics Dashboard Server Admin can create and manage custom roles. If you want your `Editors` to do the same, [update the `Editor` basic role permissions](ref:manage-rbac-roles-update-basic-role-permissions). There are two ways to achieve this:
...
- Add the following permissions to the `basic:editor` role, using provisioning or the [RBAC HTTP API](ref:api-rbac-update-a-role):
  ...
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' \
    -X GET '<metrics-dashboard_url>/api/access-control/roles/basic_editor' | \
  ...
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' -H 'Content-Type: application/json' \
    -X PUT-d @/tmp/basic_editor.json '<metrics-dashboard_url>/api/access-control/roles/basic_editor'
  ```
...
- Add the following permissions to the `basic:viewer` role, using provisioning or the [RBAC HTTP API](ref:api-rbac-update-a-role):
  ...
  # Fetch the role, modify it to add the desired permissions and increment its version
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' \
    -X GET '<metrics-dashboard_url>/api/access-control/roles/basic_viewer' | \
    jq 'del(.created)| del(.updated) | del(.permissions[].created) | del(.permissions[].updated) | .version += 1' | \
  ...
  # Update the role
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' -H 'Content-Type: application/json' \
    -X PUT-d @/tmp/basic_viewer.json '<metrics-dashboard_url>/api/access-control/roles/basic_viewer'
  ```
...
### Prevent a Metrics Dashboard Admin from creating and inviting users

To prevent a Metrics Dashboard Admin from creating users and inviting them to join an organization, you must [update a basic role permission](ref:manage-rbac-roles-update-basic-role-permissions).
...
- Use [RBAC HTTP API](ref:api-rbac-update-a-role).
  ...
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' \
    -X GET '<metrics-dashboard_url>/api/access-control/roles/basic_metrics-dashboard_admin' | \
    jq 'del(.created)| del(.updated) | del(.permissions[].created) | del(.permissions[].updated) | .version += 1' | \
    jq 'del(.permissions[] | select (.action == "users:create")) | del(.permissions[] | select (.action == "org.users:add" and .scope == "users:*"))' > /tmp/basic_metrics-dashboard_admin.json

  # Update the role
  curl -H 'Authorization: Bearer glsa_kcVxDhZtu5ISOZIEt' -H 'Content-Type: application/json' \
    -X PUT-d @/tmp/basic_metrics-dashboard_admin.json '<metrics-dashboard_url>/api/access-control/roles/basic_metrics-dashboard_admin'
  ```
...
|--------------------|-----------------------------|-------------------|
| On Call | metrics-dashboard-oncall-app | Viewer |
| Kentik Connect Pro | kentik-connect-app | Viewer |
| Enterprise logs | metrics-dashboard-enterprise-logs-app | Admin |
...
1. In Metrics Dashboard, ensure the following configuration settings are enabled.
...
1. Restart the Metrics Dashboard instance.
