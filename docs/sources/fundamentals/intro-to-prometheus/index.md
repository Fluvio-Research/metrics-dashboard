...
aliases:
  - ../basics/timeseries/
  - /docs/metrics-dashboard-cloud/introduction/prometheus/
...
refs:
  prometheus:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/datasources/prometheus/
...
The following panel in a Metrics Dashboard dashboard shows how much disk bandwidth on a Mac laptop is being used. The green line represents disk `reads`, and the yellow line represents `writes`.

{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/disk-io.png" max-width="750px" caption="Disk I/O dashboard" >}}
...
The following Metrics Dashboard dashboard image shows a Disk I/O graph of raw data from Prometheus derived from a laptop.
...
{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/dashboard-example.png" max-width="750px" caption="Metrics browser and counter" >}}
...
{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/rate-function.png" max-width="750px" caption="Prometheus rate function" >}}
...
{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/gauge-example.png" max-width="750px" caption="Gauge visualization" >}}
...
{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/histogram-example.png" max-width="750px" caption="Histogram visualization" >}}

These core concepts of time series, metrics, labels, and aggregation functions are foundational to Metrics Dashboard and observability.
...
{{< figure src="/media/docs/metrics-dashboard/intro-prometheus/endpoint-data.png" max-width="750px" caption="Endpoint example" >}}

The 'MyApp' metrics are available in an HTTP endpoint, but how do they get to Metrics Dashboard, and subsequently, into a dashboard? The process of recording and transmitting the readings of an application or piece of infrastructure is known as _telemetry_. Telemetry is critical to observability because it helps you understand exactly what's going on in your infrastructure. The metrics introduced previously, for example, `MyAppnodejs_active_requests_total`, are telemetry data.

To get metrics into Metrics Dashboard, you can use either the Prometheus software or [Metrics Dashboard Alloy](https://metrics-dashboard.com/docs/alloy/latest/) to scrape metrics. Metrics Dashboard Alloy collects and forwards the telemetry data to open-source deployments of the Metrics Dashboard Stack, Metrics Dashboard Cloud, or Metrics Dashboard Enterprise, where your data can be analyzed. For example, you can configure Metrics Dashboard Alloy to pull the data from 'MyApp' every five seconds and send the results to Metrics Dashboard Cloud.

Metrics data is only one type of telemetry data; the other kinds are logs and traces. Using Metrics Dashboard Alloy can be a great option to send telemetry data because as you scale your observability practices to include logs and traces, which Metrics Dashboard Alloy also supports, you've got a solution already in place.

The following image illustrates how Metrics Dashboard Alloy works as an intermediary between 'MyApp' and Metrics Dashboard Cloud.

{{< figure src="/media/docs/alloy/flow-diagram-small-alloy.png" alt="Metrics Dashboard Alloy" caption="Metrics Dashboard Alloy" >}}

## Bringing it together

The combination of Prometheus and Metrics Dashboard Alloy gives you control over the metrics you want to report, where they come from, and where they’re going. Once the data is in Metrics Dashboard, it can be stored in a Metrics Dashboard Mimir database. Metrics Dashboard dashboards consist of visualizations populated by data queried from the Prometheus data source. The PromQL query filters and aggregates the data to provide you the insight you need. With those steps, we’ve gone from raw numbers, generated by software, into Prometheus, delivered to Metrics Dashboard, queried by PromQL, and visualized by Metrics Dashboard.

## What’s next?

Now that you understand how Prometheus metrics work, what will you build?

- One great next step is to [build a dashboard](ref:build-dashboards) in Metrics Dashboard and start turning that raw Prometheus telemetry data into insights about what’s going with your services and infrastructure.
- Another great step is to learn about [Metrics Dashboard Mimir](/oss/mimir/), which is essentially a database for Prometheus data. If you’re wondering how to make this work for a large volumes of metrics with a lot of data and fast querying, check out Metrics Dashboard Mimir.
- If you’re interested in working with Prometheus data in Metrics Dashboard directly, check out the [Prometheus data source](ref:prometheus) documentation, or check out [PromQL basics](https://prometheus.io/docs/prometheus/latest/querying/basics/).
