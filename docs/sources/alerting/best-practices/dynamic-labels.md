---
canonical: https://metrics-dashboard.com/docs/metrics-dashboard/latest/alerting/best-practices/dynamic-labels
...
refs:
  ...
  labels:
    ...
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/alerting-and-irm/alerting/fundamentals/alert-rules/alert-rules/annotation-label/#labels
...
First, it's important to understand how Metrics Dashboard Alerting treats [labels](ref:labels).
...
In this example, the `minor` and `warning` alerts likely represent the same underlying issue, but Metrics Dashboard treats them as distinct alert instances. As a result, this scenario generates two firing notifications and two resolved notifications, one for each instance.
...
1. Observe alert state changes.
   ...
   {{< docs/play title="this alert example" url="https://play.metrics-dashboard.org/alerting/metrics-dashboard/eep7oslk5u680e/view" >}}
...
- [Multi-dimensional alerting example](ref:multi-dimensional-example) – Explore how Metrics Dashboard creates separate alert instances for each unique set of labels.
...
- [Stale alert instances](ref:stale-alert-instances) – Understand how Metrics Dashboard resolves and removes stale alert instances.
- [Handle missing data](ref:missing-data-guide) – Learn how Metrics Dashboard distinguishes between missing series and `NoData`.
...
- [Dynamic label example in Metrics Dashboard Play](https://play.metrics-dashboard.org/alerting/metrics-dashboard/eep7oslk5u680e/view) - View this example in Metrics Dashboard Play to explore alert instances and state transitions with dynamic labels.
