...
Having installed either the image rendering plugin, or set up Metrics Dashboard to use a remote rendering service, set `capture` in `[unified_alerting.screenshots]` to `true`:
