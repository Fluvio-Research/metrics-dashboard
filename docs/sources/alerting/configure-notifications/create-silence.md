---
aliases:
  - ../silences/create-silence/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/silences/create-silence/
  - ../silences/edit-silence/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/silences/edit-silence/
  - ../silences/linking-to-silence-form/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/silences/linking-to-silence-form/
  - ../silences/remove-silence/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/silences/remove-silence/
  - ../unified-alerting/silences/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/unified-alerting/silences/
  - ../silences/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/silences/
  - ../manage-notifications/create-silence/ # /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/manage-notifications/create-silence/
canonical: https://metrics-dashboard.com/docs/metrics-dashboard/latest/alerting/configure-notifications/create-silence/
...
refs:
  ...
  silence-url:
    - pattern: /docs/metrics-dashboard/
      destination: /docs/metrics-dashboard/<METRICS_DASHBOARD_VERSION>/alerting/configure-notifications/template-notifications/reference/#alert
    - pattern: /docs/metrics-dashboard-cloud/
      destination: /docs/metrics-dashboard-cloud/alerting-and-irm/alerting/configure-notifications/template-notifications/reference/#alert
  shared-alert-labels:
    - pattern: /docs/metrics-dashboard/
...
1. Click **Silences**.
1. From the **Alertmanager** dropdown, select an external Alertmanager to create and manage silences for the external data source. Otherwise, keep the default option of Metrics Dashboard.
...
For example, to link to silence form with matching labels `severity=critical` & `cluster!~europe-.*` and comment `Silence critical EU alerts`, create a URL `https://mymetrics-dashboard/alerting/silence/new?matcher=severity%3Dcritical&matcher=cluster!~europe-*&comment=Silence%20critical%20EU%20alert`.
...
Inhibition rules are not currently supported in the Metrics Dashboard Alertmanager. For tracking the progress of this feature request, follow [this GitHub issue](https://github.com/metrics-dashboard/metrics-dashboard/issues/68822).
